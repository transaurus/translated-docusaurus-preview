...
后续章节将介绍常见托管服务商及其高效部署 Docusaurus 站点的配置方法。Docusaurus 与这些服务无商业关联，所提供信息仅供参考。部分配置说明由第三方提供，可能未及时同步最新API变更。若发现内容过时，欢迎提交PR修正。

由于我们仅能提供有限支持，现已停止接受新增托管方案的PR。您可将配置指南发布在其他平台（如个人博客或服务商官网），并申请在此添加相关链接。
...
正确配置后，站点将在合并至部署分支（默认为 `main`）时自动触发部署/重新部署。

:::warning

部分 Docusaurus 站点采用旧版目录结构（常见于v1版本），`docs` 文件夹独立于 `website` 目录外：
...
若将 `website` 设为 Netlify 基础目录，需配置 [自定义忽略命令](https://docs.netlify.com/configure-builds/common-configurations/ignore-builds/) 以避免 `docs` 更新时未触发构建：
...
建议关闭 Netlify 的 `后期处理 > 资源优化 > 美化URL` 功能，以避免URL小写化、多余重定向及404错误。

**特别注意**：全局复选框 `禁用资源优化` 存在缺陷，实际不会关闭 `美化URL` 功能。请务必单独取消勾选该选项。

如需保留 Netlify 的 `美化URL` 功能，请相应调整 Docusaurus 的 `trailingSlash` 配置。

更多细节请参考 [slorber/trailing-slash-guide](https://github.com/slorber/trailing-slash-guide)。

:::

## 部署至 Vercel {#deploying-to-vercel}

将 Docusaurus 项目部署至 [Vercel](https://vercel.com/) 可显著提升 [性能表现](https://vercel.com/) 并简化运维流程。

通过 [Vercel Git 集成](https://vercel.com/docs/concepts/git) 部署前，请确保项目已推送至Git仓库。

通过 [导入流程](https://vercel.com/import/git) 将项目导入 Vercel。在导入过程中，您会发现所有相关选项都已预先配置好；不过，您也可以选择更改这些 [选项](https://vercel.com/docs/build-step#build-&-development-settings)。
...
Docusaurus 提供了简便的方式发布到 [GitHub Pages](https://pages.github.com/)，该服务免费提供给每个 GitHub 仓库使用。

### 概述 {#github-pages-overview}

通常发布流程涉及两个仓库（至少两个分支）：包含源文件的分支，以及包含构建输出供 GitHub Pages 服务使用的分支。在以下教程中，它们将分别被称为 **"源"** 和 **"部署"**。

每个 GitHub 仓库都关联一个 GitHub Pages 服务。如果部署仓库名为 `my-org/my-project`（其中 `my-org` 是组织名或用户名），部署后的站点将出现在 `https://my-org.github.io/my-project/`。如果部署仓库名为 `my-org/my-org.github.io`（即 _组织 GitHub Pages 仓库_），站点将出现在 `https://my-org.github.io/`。

:::info

如需为 GitHub Pages 使用自定义域名，请在 `static` 目录创建 `CNAME` 文件。`static` 目录下的所有内容都会被复制到部署用的 `build` 目录根目录。使用自定义域名时，您可以将 `baseUrl: '/projectName/'` 改回 `baseUrl: '/'`，并将 `url` 设置为您的自定义域名。

更多细节可参考 GitHub Pages 文档 [用户、组织和项目页面](https://help.github.com/en/articles/user-organization-and-project-pages)。

:::

GitHub Pages 会从默认分支（通常是 `master`/`main`）或 `gh-pages` 分支，以及根目录或 `/docs` 文件夹中获取部署就绪的文件（即 `docusaurus build` 的输出）。您可以通过仓库的 `Settings > Pages` 进行配置。该分支被称为"部署分支"。

我们提供了 `docusaurus deploy` 命令，帮助您通过一条命令完成从源分支到部署分支的部署：克隆、构建和提交。
...
默认情况下，GitHub Pages 会使用 [Jekyll](https://jekyllrb.com/) 处理发布的文件。由于 Jekyll 会丢弃以下划线 `_` 开头的文件，建议您在 `static` 目录添加一个名为 `.nojekyll` 的空文件来禁用 Jekyll。
...
GitHub 企业版安装与 github.com 的工作方式相同；您只需将组织的 GitHub Enterprise 主机设置为环境变量：
...
1. 当向 `main` 分支发起新的拉取请求时，执行一个确保站点构建成功的动作（不实际部署）。该任务称为 `test-deploy`。
2. 当拉取请求被合并到 `main` 分支或直接向 `main` 分支推送时，执行构建并部署到 GitHub Pages。该任务称为 `deploy`。

以下是两种通过 GitHub Actions 部署文档的方法。根据部署仓库的位置选择对应标签页：
...
持续集成（CI）服务通常用于在代码提交到版本控制时执行例行任务，包括运行单元测试/集成测试、自动化构建、发布 npm 包以及部署网站变更。要实现网站自动部署，只需在网站更新时调用 `yarn deploy` 脚本。以下章节将介绍如何通过流行 CI 服务提供商 [Travis CI](https://travis-ci.com/) 实现这一过程。
...
2. 使用 GitHub 账号为目标仓库[添加 Travis CI 应用](https://github.com/marketplace/travis-ci)。
3. 打开 Travis CI 仪表盘（形如 `https://travis-ci.com/USERNAME/REPO`），导航至仓库的 `More options > Setting > Environment Variables` 部分。
4. 创建名为 `GH_TOKEN` 的环境变量（值为新生成的令牌），以及 `GH_EMAIL`（您的邮箱地址）和 `GH_NAME`（GitHub 用户名）。
...
此后，每当有新提交推送到 `main` 分支时，Travis CI 将运行测试套件，若全部通过则会通过 `yarn deploy` 脚本自动部署网站。
...
4. 在项目页面（形如 `https://dev.azure.com/ORG_NAME/REPO_NAME/_build`）中，使用以下文本创建新流水线。点击编辑添加名为 `GH_TOKEN` 的环境变量（值为新生成令牌），以及 `GH_EMAIL`（您的邮箱）和 `GH_NAME`（GitHub 用户名）。务必将其标记为机密。您也可在仓库根目录创建 `azure-pipelines.yml` 文件。
...
4. 登录 Drone.io 控制台（形如 `https://cloud.drone.io/USERNAME/REPO`）。
...
[Koyeb](https://www.koyeb.com) 是一个对开发者友好的无服务器平台，用于全球部署应用程序。该平台允许您无缝运行 Docker 容器、Web 应用程序和 API，支持基于 Git 的部署、原生自动扩展、全球边缘网络以及内置的服务网格和发现功能。查看 [Koyeb 的 Docusaurus 部署指南](https://www.koyeb.com/tutorials/deploy-docusaurus-on-koyeb) 以开始使用。

## 部署到 Render {#deploying-to-render}

[Render](https://render.com) 提供 [免费的静态网站托管服务](https://render.com/docs/static-sites)，包括完全托管的 SSL、自定义域名、全球 CDN 以及从您的 Git 仓库持续自动部署。只需几分钟即可开始，按照 [Render 的 Docusaurus 部署指南](https://render.com/docs/deploy-docusaurus) 操作即可。
...
1. 创建一个 Qovery 账户。访问 [Qovery 控制台](https://console.qovery.com) 注册账户（如果您还没有）。
2. 创建一个项目。
   - 点击 **创建项目** 并为项目命名。
   - 点击 **下一步**。
3. 创建一个新环境。
...
4. 添加一个应用程序。
   - 点击 **创建应用程序**，命名并选择您的 GitHub 或 GitLab 仓库（存放 Docusaurus 应用程序的位置）。
   - 定义主分支名称和应用程序根路径。
   - 点击 **创建**。应用程序创建完成后：
   - 导航到应用程序的 **设置**
   - 选择 **端口**
...
完成。观察状态并等待应用程序部署完成。要在浏览器中打开应用程序，请在应用程序概览中点击 **操作** 和 **打开**。
...
3. 配置构建设置。
   ...
   - 接下来会出现**网站定制**窗口。从框架列表中选择**静态网站**选项。
...
4. 部署。
   ...
   - 开始后您将进入部署日志界面。如果代码存在问题，日志会显示警告或错误信息并指明原因。通常日志包含所有需要的调试数据。

   - 部署完成后，您将收到邮件通知并看到日志条目。大功告成！您的项目已上线运行。

## 部署到Surge {#deploying-to-surge}

Surge是一个[静态网站托管平台](https://surge.sh/help/getting-started-with-surge)，可通过命令行在数秒内完成Docusaurus项目部署。该平台提供免费服务（包括自定义域名和SSL证书）。
...
确认要发布的站点位于`build`目录下。系统会自动分配一个随机子域名`*.surge.sh`（可编辑修改）。
...
您的站点现已免费部署在`subdomain.surge.sh`或`your-domain.com`（取决于所选方法）。
...
未来任何变更都可通过`surge`命令直接部署。

## 部署到Stormkit {#deploying-to-stormkit}

您可以将Docusaurus项目部署至[Stormkit](https://www.stormkit.io)——专为静态网站、单页应用(SPA)和无服务器函数设计的部署平台。具体操作请参考本[指南](https://www.stormkit.io/blog/how-to-deploy-docusarous)。
...
更多部署至 QuantCDN 的用例和示例，请参阅[文档](https://docs.quantcdn.io/docs/cli/continuous-integration)和[博客](https://www.quantcdn.io/blog)。

## 部署到 Layer0 {#deploying-to-layer0}

[Layer0](https://www.layer0.co) 是一个一体化平台，用于开发、部署、预览、实验、监控和运行无头前端。它专注于大型动态网站，通过 EdgeJS（基于 JavaScript 的内容分发网络）、预测性预取和性能监控提供一流的性能。Layer0 提供免费套餐，只需几分钟即可开始使用，请按照 [Layer0 的 Docusaurus 部署指南](https://docs.layer0.co/guides/docusaurus)操作。

## 部署到 Cloudflare Pages {#deploying-to-cloudflare-pages}

[Cloudflare Pages](https://pages.cloudflare.com/) 是一个面向前端开发者的 Jamstack 平台，用于协作和部署网站。按照[这篇文章](https://dev.to/apidev234/deploying-docusaurus-to-cloudflare-pages-565g)的指引，几分钟内即可开始使用。

## 部署到 Azure Static Web Apps {#deploying-to-azure-static-web-apps}

[Azure Static Web Apps](https://docs.microsoft.com/en-us/azure/static-web-apps/overview) 是一项服务，可直接从代码仓库自动构建和部署全栈 Web 应用到 Azure，简化了 CI/CD 的开发者体验。Static Web Apps 将 Web 应用的静态资源与动态（API）端点分离。静态资源从全球分布的内容服务器提供，使客户端能够通过附近的服务器更快地检索文件。动态 API 使用基于事件驱动的无服务器架构进行扩展，这种方法更具成本效益，并能按需扩展。按照[这份分步指南](https://dev.to/azure/11-share-content-with-docusaurus-azure-static-web-apps-30hc)，几分钟内即可开始使用。
...
按照我们的 [Kinsta 上的 Docusaurus](https://kinsta.com/docs/docusaurus-example/) 文章指引，只需点击几下即可开始使用。