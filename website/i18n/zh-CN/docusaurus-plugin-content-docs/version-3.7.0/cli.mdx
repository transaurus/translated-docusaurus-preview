...
当需要从远程服务器或虚拟机（如 GitHub Codespaces）转发 3000 端口时，可指定开发服务器监听 `0.0.0.0` 以绑定本地IP地址。
...
该交互式命令行将引导您完成完整的[替换流程](./swizzling.mdx)。
...
使用 [GitHub Pages](https://pages.github.com/) 部署您的站点。更多详情请参阅[部署文档](deployment.mdx#deploying-to-github-pages)。
...
建议在报告问题、升级版本后或遇到 Docusaurus 站点异常时运行此命令。
