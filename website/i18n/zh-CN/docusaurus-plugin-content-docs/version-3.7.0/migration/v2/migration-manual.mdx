...
将以下预设配置添加至`docusaurus.config.js`：
...
我们建议将 `docs` 文件夹移至 `website` 文件夹内，这也是 v2 版本的默认目录结构。若 `docs` 目录位于 `website` 中，[Vercel](https://vercel.com) 可[直接支持 Docusaurus 项目部署](https://github.com/vercel/vercel/tree/main/examples/docusaurus)。将文档与网站代码集中存放在同一个 `website` 目录下通常更利于项目管理。
...
已弃用。我们为 Docusaurus 2 开发了名为 [Infima](https://infima.dev/) 的 CSS 框架，采用 CSS 变量实现主题化。相关文档尚未完备，后续将在此更新。要覆盖 Infima 的 CSS 变量，请创建自定义 CSS 文件（如 `./src/css/custom.css`）并通过 `@docusaurus/preset-classic` 的配置选项全局引入：
...
也可使用下方工具生成网站色阶变量，并复制到 `src/css/custom.css` 中：
...
您的 Algolia DocSearch v1 配置（位于[此处](https://github.com/algolia/docsearch-configs/blob/master/configs)）需针对 Docusaurus v2 更新（参见[示例](https://github.com/algolia/docsearch-configs/tree/master/configs/docusaurus-2.json)）。

可联系 DocSearch 团队 (@shortcuts, @s-pace) 获取支持。他们将协助更新配置并触发站点重新爬取（否则需等待最长 24 小时的下次计划爬取）
...
已弃用。请将其作为选项传递给 `@docusaurus/preset-classic` 文档插件：
...
- `defaultVersionShown` - 版本功能尚未移植。若您使用版本功能，暂时无法迁移至 Docusaurus 2。请保持关注。
- `disableHeaderTitle`
- `disableTitleTagline`
...
出于 SEO 考量及避免链接失效，应在托管服务商处配置服务端重定向规则。
...
旧版本中不允许嵌套侧边栏分类，且分类仅能包含文档ID。而v2支持无限嵌套侧边栏，并提供多种[侧边栏项类型](../../guides/docs/sidebar/items.mdx)（不仅限于文档）。
...
该操作会将当前主题使用的`<Footer />`组件复制到站点根目录下的`src/theme/Footer`目录中，您可在此编辑组件以实现自定义。

请勿仅为在左侧添加Logo而swizzle页脚组件。v2版本已特意移除Logo并将其移至底部，只需在`docusaurus.config.js`中通过`themeConfig.footer`配置即可：
...
在v2中需改用`useDocusaurusContext`钩子获取`siteConfig`对象。

v2版本要求在每个页面外层应用主题布局。Layout组件接收元数据props。
...
Docusaurus 2采用[MDX](https://mdxjs.com/)语法，需检查现有文档中可能存在的语法冲突。例如HTML中有效的自闭合标签（如`<img>`和`<br>`）现在必须显式闭合（`<img/>`和`<br/>`），MDX文档中的所有标签必须符合JSX规范。
...
**提示**：可使用 [HTML 转 JSX](https://transform.tools/html-to-jsx) 等在线工具简化迁移过程。
...
博客的 Docusaurus front matter 字段已从驼峰式改为蛇形命名以保持与文档的一致性。
