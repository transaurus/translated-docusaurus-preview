...
有时需要在网站发布公告。为此可添加公告栏——这是一个位于导航栏上方、非固定且可选是否可关闭的面板。所有配置都在`announcementBar`对象中。
...
Logo可放置在[静态文件夹](static-assets.mdx)中。Logo URL默认设置为站点基础URL。虽然可以指定自定义URL，但如果是外部链接将在新标签页打开。此外可覆盖logo链接的target属性值，当文档网站托管在主站点的子目录时特别有用——这种情况下可能不需要logo链接在主站新标签页打开。

为增强深色模式支持，还可为此模式设置不同的logo。
...
根据`type`字段的不同，项目可具有不同行为。以下章节将介绍所有可用的导航栏项目类型。

#### 导航栏链接 {#navbar-link}

默认情况下，导航栏项目是常规链接（内部或外部）。

React Router 应自动为链接应用活动链接样式，但在边缘情况下可使用 `activeBasePath`。若某个链接需在多个不同路径下保持活动状态（例如同一侧边栏下存在多个文档目录），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更灵活的替代方案且优先级更高——Docusaurus 会将其解析为正则表达式来匹配当前URL。

外链会自动添加 `target="_blank" rel="noopener noreferrer"` 属性。
...
类型为 `dropdown` 的导航栏项目具有额外的 `items` 字段，即嵌套的导航栏项目数组。

导航栏下拉菜单项目仅接受以下**类链接项目类型**：

- [导航栏链接](#navbar-link)
- [导航栏文档链接](#navbar-doc-link)
- [导航栏文档版本](#navbar-docs-version)
- [导航栏文档侧边栏](#navbar-doc-sidebar)
- [自定义HTML导航栏](#navbar-with-custom-html)

注意下拉菜单基项本身也是可点击链接，因此可接收[普通导航栏链接](#navbar-link)的任何属性。
...
若需链接至特定文档，此特殊导航栏项目类型会渲染指向 `docId` 所指定文档的链接。只要浏览同一侧边栏下的文档，该链接就会获得 `navbar__link--active` 类。
...
可将导航栏项目链接至给定侧边栏的第一个文档链接（可以是文档链接或生成的分类索引），而无需硬编码文档ID。
...
若使用版本化文档，此特殊导航栏项目类型会渲染包含所有可用版本的下拉菜单。

用户可在保持当前文档不变的情况下切换版本（需确保文档ID在不同版本间保持一致）。
...
若使用版本化文档，此特殊导航栏项目类型会链接至当前浏览的文档版本（取决于URL），并回退至最新版本。
...
若启用了[国际化功能](../../i18n/i18n-introduction.mdx)，此特殊导航栏项会渲染包含所有可用语言的下拉菜单。

用户可在保持当前页面的同时切换不同语言版本。
...
可启用此UI特性：当用户向下滚动页面时自动隐藏导航栏，向上滚动时重新显示。
...
可在保留主题切换功能的同时设置静态导航栏样式。所选样式将始终生效，不受用户选择的主题影响。
...
默认使用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)作为语法高亮主题。可从[可用主题列表](https://github.com/FormidableLabs/prism-react-renderer/tree/master/packages/prism-react-renderer/src/themes)选择自定义主题，也可为暗色模式指定不同的语法高亮主题。
...
若使用行高亮Markdown语法，可能需要为暗色模式语法高亮主题指定不同的高亮背景色。具体指导参见[文档](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)。
...
通过`themeConfig.footer`可添加页脚logo和版权信息。logo文件应置于[静态文件夹](static-assets.mdx)，其URL配置方式与导航栏logo相同。
...
您可以通过 `themeConfig.footer.links` 添加页脚链接。页脚配置有两种类型：**多栏页脚**和**简易页脚**。

多栏页脚链接需要为每列配置一个 `title` 和一组 `FooterItem` 列表。
...
您可以通过 `themeConfig.tableOfContents` 调整默认目录设置。
...
用于访问颜色模式的React钩子。该上下文包含设置浅色/深色模式的函数，并暴露布尔值变量指示当前使用的模式。
