...
此插件在开发环境中始终处于禁用状态，**仅在生产环境生效**，因其作用于构建输出产物。
...
若您使用 `@docusaurus/preset-classic` 预设，则无需额外安装此插件依赖。

您可通过[预设配置](../../using-plugins.mdx#docusauruspreset-classic)来调整插件参数。
...
`lastmod` 配置项仅在插件提供[路由元数据](../plugin-methods/lifecycle-apis.mdx#addRoute)属性 `sourceFilePath` 和/或 `lastUpdatedAt` 时，才会输出站点地图的 `<lastmod>` 标签。

所有官方内容插件都会为基于内容文件（Markdown、MDX或React页面组件）的路由提供元数据，但第三方插件作者可能未提供这些信息，此时将无法为其路由输出 `<lastmod>` 标签。
