...
用于生成**客户端重定向**的Docusaurus插件。

该插件会向静态站点写入额外的HTML页面，通过JavaScript将用户重定向至现有的Docusaurus页面。

:::warning[仅生产环境生效]

此插件在开发环境中始终处于非活跃状态，**仅在生产环境生效**，因其功能基于构建输出实现。
...
本插件会读取[`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes)配置，当多个文件将输出至相同位置时调整其日志级别。
...
"from"与"to"的概念是本插件的核心。"from"表示您希望_创建_的路径（即待写入的额外HTML文件）；"to"表示您希望重定向_至_的路径（通常是Docusaurus已识别的路由）。

因此可以对同一个"to"设置多个"from"：我们将创建多个重定向至同一目标的HTML文件。反之，一个"from"不能对应多个"to"：生成的HTML文件必须具有确定的重定向目标。
