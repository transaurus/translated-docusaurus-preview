...
[订阅功能](../../blog.mdx#feed)通过解析构建输出实现，因此**仅在生产环境生效**。
...
用于通过 [XSLT](https://developer.mozilla.org/en-US/docs/Web/XSLT) 技术美化博客 XML 订阅源的浏览器渲染效果。

- 设为 `true` 使用插件内置的 `.xsl` 和 `.css` 文件样式
- 设为假值（`undefined | null | false`）禁用此功能
- 设为字符串可指定博客内容目录下自定义 `.xsl` 文件路径（需配套同名的 `.css` 文件）
...
默认会在博客内容目录根目录查找 `authors.yml` 文件。

该文件可包含预定义的[全局博客作者](../../blog.mdx#global-authors)，在 Markdown 文件中可通过 [`authors` 前置元数据](#markdown-front-matter)引用这些作者标识符。
