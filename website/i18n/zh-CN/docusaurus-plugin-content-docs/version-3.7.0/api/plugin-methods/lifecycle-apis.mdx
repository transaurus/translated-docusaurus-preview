...
插件应使用此生命周期从数据源（文件系统、远程 API、无头 CMS 等）获取数据或进行一些服务器端处理。返回值是插件所需的内容。
...
一个声明式回调，用于创建静态数据（通常是 JSON 或字符串），这些数据稍后可以作为 props 提供给您的路由。接收文件名和要存储的数据，并返回实际数据文件的路径。

例如，下面的插件创建一个 `/friends` 页面，显示 `你的朋友是：Yangshun, Sebastien`：
...
修改内部 webpack 配置。如果返回值是一个 JavaScript 对象，它将使用 [`webpack-merge`](https://github.com/survivejs/webpack-merge) 合并到最终配置中。如果是一个函数，它将被调用并接收 `config` 作为第一个参数，`isServer` 标志作为第二个参数。
...
`configureWebpack` 被调用时会传入根据客户端/服务器端构建生成的 `config`。您可以将其视为要合并的基础配置。

### `isServer` {#isServer}

`configureWebpack` 在服务端构建和客户端构建时都会被调用。服务端构建传入 `true`，客户端构建传入 `false` 作为 `isServer` 参数。
...
可以指定合并策略。例如，如果您希望某个 webpack 规则前置而非追加：
...
更多细节请参阅 [webpack-merge 策略文档](https://github.com/survivejs/webpack-merge#merging-with-strategies)。
...
- `headTags` 将在 `</head>` 结束标签前插入，位于配置添加的脚本之后。
- `preBodyTags` 将在 `<body>` 开始标签后插入，位于所有子元素之前。
- `postBodyTags` 将在 `</body>` 结束标签前插入，位于所有子元素之后。

## `getClientModules()` {#getClientModules}

返回要导入客户端包的[客户端模块](../../advanced/client.mdx#client-modules)路径数组。

例如，要让您的主题从用户传入的 `options` 中加载 `customCss` 或 `customJs` 文件路径：
