...
该服务对开发者文档和技术博客**完全免费**：只需确保阅读[准入清单](https://docsearch.algolia.com/docs/who-can-apply/)并[申请加入DocSearch计划](https://docsearch.algolia.com/apply)。
...
您可以通过[我们的博客文章](/blog/2021/11/21/algolia-docsearch-migration)或[DocSearch迁移文档](https://docsearch.algolia.com/docs/migrating-from-legacy)了解从旧版DocSearch基础设施迁移的更多信息。
...
若更新了`initialIndexSettings`配置，虽然可通过界面手动更新索引，但[Algolia团队建议删除原有索引后重新爬取](https://github.com/facebook/docusaurus/issues/9200#issuecomment-1667338492)，以确保完全按新设置重新初始化。
...
当浏览 v2 文档时，出现 v1 的搜索结果会显得不合时宜。有时 v1 和 v2 文档内容高度相似，可能导致同一查询返回重复结果（每个版本各出现一次）。

同理，浏览法语站点时展示英文文档结果也不合理。

上下文搜索通过动态创建查询过滤器，自动识别您当前浏览的文档版本和语言。
...
DocSearch 默认采用符合无障碍标准的精心调校主题，确保色彩对比度达标。
...
Algolia DocSearch 支持通过 `docusaurus.config.js` 中的 `algolia` 字段传递[配置参数](https://docsearch.algolia.com/docs/api/)。
...
对于搜索索引较小且可被用户浏览器下载的网站，您可以使用本地搜索插件。

社区支持的[本地搜索插件列表可在此查看](https://docusaurus.io/community/resources#search)。

## 👥 使用自定义搜索 {#using-your-own-search}

要使用自定义搜索，请从`@docusaurus/theme-classic`中拆分（swizzle）`SearchBar`组件

```bash npm2yarn
npm run swizzle @docusaurus/theme-classic SearchBar
```

这将在项目文件夹中创建`src/theme/SearchBar`文件。重启开发服务器并编辑该组件后，Docusaurus将使用您自定义的`SearchBar`组件。

**注意**：您也可以[从Algolia SearchBar拆分](#editing-the-algolia-search-component)并基于此创建自定义搜索组件。