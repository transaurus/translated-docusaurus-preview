...
不过您通常不需要逐个安装常见插件：它们可以以[预设包](#using-presets)的形式分发。对于大多数用户而言，插件是通过预设配置进行管理的。
...
如果您充满热情，也可以阅读[插件开发指南](./advanced/plugins.mdx)或[插件方法参考文档](./api/plugin-methods/README.mdx)来自行开发插件。
...
但插件支持通过"Babel风格"的配置方式——用二元组包裹插件名称和选项对象来指定参数：
...
所有Docusaurus内容插件都支持多实例运行。例如，您可能需要使用[多文档插件实例](./guides/docs/docs-multi-instance.mdx)或[多博客](./blog.mdx#multiple-blogs)。这种情况下必须为每个插件实例分配唯一ID，默认插件ID为`default`。
...
`themes`和`plugins`选项会导致不同的[模块简写解析规则](#module-shorthands)，如需使用简写功能，请确保使用正确的配置项！
...
预设（Presets）是插件和主题的捆绑包。例如，与其让您在配置文件中逐个注册和配置`@docusaurus/plugin-content-docs`、`@docusaurus/plugin-content-blog`等插件，我们提供了`@docusaurus/preset-classic`预设，允许您在一个集中的位置配置它们。
...
如果名称不包含具体包名（如`@jquery`），则会加载`[scope]/docusaurus-[moduleType]`格式的模块（例如`@jquery/docusaurus-plugin`）。否则将尝试以下路径：
