...
同样地，您可以通过 `require` 现有资源并在 `video` 标签、`a` 锚链接等中使用返回的 URL 来链接到它们。
...
如果使用 Markdown 图片或链接语法，Docusaurus 会将所有资源路径解析为文件路径并自动转换为 `require()` 调用。除非使用 JSX 语法（这种情况下需要自行处理），否则无需在 Markdown 中显式使用 `require()`。
...
当您需要通过 CSS 修改 SVG 图片的某些部分时，这非常有用。例如，可以根据当前主题更改 SVG 的某一种颜色。
...
GitHub 采用其独有的[图片主题化方案](https://github.blog/changelog/2021-11-24-specify-theme-context-for-images-in-markdown/)，通过路径片段实现，您可以轻松自行实现类似效果。

要通过路径片段切换图片显示（GitHub 使用的是 `#gh-dark-mode-only` 和 `#gh-light-mode-only`），请将以下代码添加到您的自定义 CSS 中（如果不希望与 GitHub 耦合，也可以使用自己的后缀）：
...
如果 Markdown 链接或图片使用绝对路径，该路径将被视为文件路径，并从静态资源目录中解析。例如，若您配置了[静态目录](../../static-assets.mdx)为 `['public', 'static']`，则对于以下图片：

```md title="my-doc.md"
![An image from the static](/img/docusaurus.png)
```

Docusaurus 会依次在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 中查找。该链接最终会被转换为 `require()` 调用而非保留为 URL，这在以下两个方面具有优势：

1. 您无需关心基础URL，Docusaurus会在提供资源时自动处理；
2. 图片会进入Webpack的构建流程，其文件名将被附加哈希值，这使得浏览器能积极缓存图片并提升站点性能。
