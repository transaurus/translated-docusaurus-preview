...
通过[front matter](./guides/markdown-features/markdown-features-intro.mdx#front-matter)可为文章添加作者等元数据，但Docusaurus也能自动推断必要信息。完整字段参见[API文档](api/plugins/plugin-content-blog.mdx#markdown-front-matter)。
...
默认每页显示10篇文章，可通过插件配置中的`postsPerPage`参数调整分页。设为`postsPerPage: 'ALL'`将禁用分页并展示所有文章。还可为列表页添加SEO元描述：
...
侧边栏默认展示5篇最新文章，通过`blogSidebarCount`参数可调整数量。设为`0`会完全移除侧边栏容器以增加主内容区宽度。特殊值`'ALL'`会展示全部文章。
...
此命名约定是可选的，您也可以通过 front matter 提供日期。由于 front matter 遵循支持日期时间标记的 YAML 语法，若需要更精确的发布日期，可以使用 front matter。例如，若同一天发布多篇文章，可按当天时间排序：
...
使用 `authors` front matter 字段声明博文作者。作者至少需包含 `name` 或 `image_url` 属性。Docusaurus 会使用 `url`、`email` 和 `title` 等信息，但也允许其他自定义信息。
...
在博文 front matter 中，可引用全局配置文件声明的作者：
...
无论是通过 front matter 还是作者映射声明的作者，都必须包含姓名或头像（或两者兼备）。若某篇博文的所有作者均未设置姓名，Docusaurus 会紧凑显示其头像。效果可参见[此测试文章](/tests/blog/2022/01/20/image-only-authors)。

:::warning[Feed 生成]

[RSS 订阅源](#feed)需要设置作者的 email 才能显示作者信息。
...
您可以通过在[全局作者配置](#global-authors)中添加 `page: true` 属性，为每位作者单独启用此功能：
...
`readingTime`回调接收三个参数：博客内容文本字符串、front matter键值记录，以及默认阅读时长函数。返回数字（分钟数）或`undefined`（禁用该页面的阅读时长）。

默认阅读时长函数可接受额外选项：`wordsPerMinute`（数字，默认300）和`wordBound`（字符串到布尔值的函数）。当传入字符串应被视为单词边界时（默认为空格、制表符和换行符），函数应返回`true`。
...
您可以将博客文章列表页设为网站首页，而非使用独立着陆页。将`routeBasePath`设置为`'/'`后，博客将通过根路由`example.com/`提供服务，而非子路由`example.com/blog/`。
...
若禁用文档插件，请确保同时删除配置文件中其他对文档插件的引用，特别是导航栏中的文档相关条目。

:::

:::tip

针对仅需文档功能的用户，我们还提供"纯文档模式"。详见[纯文档模式](./guides/docs/docs-introduction.mdx)获取详细配置说明及对`routeBasePath`参数的深入解析。

:::

### 多博客系统 {#multiple-blogs}

经典主题默认假设单站点仅承载一个博客，因此仅包含单个博客插件实例。若需在单个网站部署多个博客，您可通过在`docusaurus.config.js`的`plugins`选项中配置额外博客插件实现。

为第二个博客设置专属的`routeBasePath`URL路由路径。注意此处`routeBasePath`必须与首个博客不同以避免路径冲突！同时通过`path`参数指定第二个博客内容的存储目录路径。
...
示例可参考我们部署的第二个博客[测试博客](/tests/blog)。