...
...但该语法仅在 2020 年后发布的浏览器版本中被识别。为了兼容早期浏览器版本，当为生产环境构建站点时，我们的 JS 加载器会将代码转译为更冗长的语法：
...
但这会惩罚其他用户，因为 29 个字符的代码行现在变成了 168 个字符——增加了 6 倍！（实际情况下会好一些，因为使用的名称会更短。）作为权衡，JS 加载器仅将语法转译到浏览器列表中所有版本都支持的程度。

默认情况下，浏览器列表通过 `package.json` 文件中的根级 `browserslist` 字段提供。

:::warning

在旧版浏览器上，编译输出会使用不受支持（过于现代）的 JS 语法，导致 React 初始化失败，最终呈现一个只有 HTML/CSS 而没有 JS 的静态网站。
...
输出结果是生产环境支持的所有浏览器。以下是 2022 年 1 月的输出示例：
