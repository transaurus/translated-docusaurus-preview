...
在文章中使用`<!--truncate-->`标记来定义列表页显示的摘要内容。该标记前的所有内容将作为摘要显示，且必须是独立可渲染的Markdown。例如：
...
默认每页显示10篇文章，可通过插件配置中的`postsPerPage`控制分页。设为`postsPerPage: 'ALL'`将禁用分页并在首页显示所有文章。还可为博客列表页添加meta描述提升SEO：
...
博客侧边栏默认展示5篇最新文章，可通过`blogSidebarCount`配置数量。设为`0`将完全禁用侧边栏(同时移除容器扩大主内容区宽度)，设为`'ALL'`则显示_所有_文章。

可通过`blogSidebarTitle`修改侧边栏标题。例如当设置`blogSidebarCount: 'ALL'`时，可将默认的"最新文章"改为"全部文章"：
...
Docusaurus支持从多种模式(如`YYYY-MM-DD-my-blog-post-title.md`或`YYYY/MM/DD/my-blog-post-title.md`)提取`YYYY-MM-DD`格式日期，便于按年/月分组或使用扁平结构。
...
此命名约定是可选的，您也可以通过front matter提供日期。由于front matter遵循支持日期时间标记的YAML语法，如需更精确的发布日期，可使用front matter。例如，若同一天发布多篇文章，可按具体时间排序：
...
通过front matter或作者映射声明的作者，需包含姓名或头像（或两者兼备）。若某文章所有作者均无姓名，Docusaurus会紧凑显示其头像。效果可参见[此测试文章](/tests/blog/2022/01/20/image-only-authors)。

:::warning[Feed生成]

[RSS订阅源](#feed)需设置作者邮箱才能在feed中显示作者信息。

:::

### 作者页面 {#authors-pages}

作者页面功能为可选功能，主要适用于多作者博客。

您可通过在[全局作者配置](#global-authors)中添加`page: true`属性为每位作者单独启用该功能：
...
- 为每位作者生成专属页面（[示例](/blog/authors/slorber)），列出其贡献的所有博客文章
- 作者索引页（[示例](/blog/authors)）按`authors.yml`中的顺序展示所有作者
...
Docusaurus会根据字数统计为每篇博客生成阅读时长预估。我们提供了自定义此功能的选项。
...
通过配置 `feedOptions` 可以生成 RSS/Atom/JSON 订阅源。默认情况下会生成 RSS 和 Atom 订阅。如需禁用订阅生成，请将 `feedOptions.type` 设为 `null`。
...
您可以让博客文章列表页替代首页作为站点入口。将 `routeBasePath` 设置为 `'/'` 后，博客将通过根路由 `example.com/` 提供服务（而非默认的子路由 `example.com/blog/`）。
...
若禁用文档插件，请同步移除配置文件中所有对文档插件的引用，尤其注意清理导航栏中的文档相关条目。

:::

:::tip

我们还支持"纯文档模式"，详见[纯文档模式指南](./guides/docs/docs-introduction.mdx)了解 `routeBasePath` 的详细配置说明。

:::

### 多博客系统 {#multiple-blogs}

经典主题默认假设站点仅包含一个博客，因此只内置单博客插件实例。若需在单个站点部署多个博客，可通过在 `docusaurus.config.js` 的 `plugins` 选项中添加额外博客插件实现。

为第二个博客设置专属的 `routeBasePath` 路由路径（需与首个博客路径不同以避免冲突），并通过 `path` 指定其博文目录路径。
