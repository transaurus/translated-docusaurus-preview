...
答案就是：创建 MDX 插件！MDX 内置的[插件系统](https://mdxjs.com/advanced/plugins/)可自定义 Markdown 文件如何被解析并转换为 JSX。MDX 插件主要有三种典型用例：
...
如果您使用过 [MDX 交互式演示](https://mdxjs.com/playground/)，会注意到 MDX 转换有两个中间步骤：Markdown 抽象语法树（MDAST）和超文本抽象语法树（HAST），最终才输出 JSX。MDX 插件也分为两种形式：
...
使用插件为项目中常用的 JSX 元素创建简写语法。我们提供的[警告标注](./markdown-features-admonitions.mdx)语法也是通过 Remark 插件实现的，您可以为自己的用例实现类似功能。
...
MDX 插件通常是 npm 包，因此可以通过 npm 安装。以[数学公式插件](./markdown-features-math-equations.mdx)为例。
...
许多官方 Remark/Rehype 插件仅支持 [ES Modules](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)（JavaScript 模块系统），Docusaurus 支持此规范。我们建议使用 [ES Modules](https://flaviocopes.com/es-modules/) 配置文件来简化这类包的导入。

:::

接下来，在 `docusaurus.config.js` 中导入插件并通过插件或预设配置添加到插件选项：
...
例如，我们创建一个为所有二级标题添加"第X节"前缀的插件。首先在任意位置创建插件源文件（也可发布为独立npm包并通过上述方式安装）。我们将插件放在`src/remark/section-prefix.js`中。remark/rehype插件本质上是一个接收`options`参数并返回AST转换器的函数。
...
例如我们的`transformImage`插件就利用此参数将相对路径图片引用转换为`require()`调用。

:::

:::note

Docusaurus默认插件会在自定义remark插件之前执行，这意味着图片/链接此时已被转换为带`require()`调用的JSX。例如上例中，虽然所有二级标题都添加了"第X节"前缀，但生成的目录仍保持原样，因为目录生成插件在我们的自定义插件之前执行。若需在默认插件前处理MDAST，请使用`beforeDefaultRemarkPlugins`和`beforeDefaultRehypePlugins`：
