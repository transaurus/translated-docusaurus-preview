...
- `/blog`博客目录
- `/static`静态资源目录
- 若站点启用了版本控制，还会迁移`versioned_sidebar.json`和`/versioned_docs`版本化文档
...
可通过添加选项标志让迁移CLI自动转换Markdown内容和页面至v2格式。通常仍需手动调整才能达到理想效果。
...
建议先不使用`--page`和`--mdx`选项进行迁移，提交代码后，再尝试启用这些选项重新运行迁移。
