...
:::warning[部分功能仅在生产环境生效]

[订阅功能](../../blog.mdx#feed) 通过提取构建输出实现，**仅在生产环境激活**。
...
若使用 `@docusaurus/preset-classic` 预设，则无需单独安装此插件。
...
允许通过 [XSLT](https://developer.mozilla.org/en-US/docs/Web/XSLT) 样式化博客 XML 订阅源，使浏览器能优雅渲染。

- 设为 `true` 时，博客将使用内置的 `.xsl` 和 `.css` 文件样式化订阅源
- 设为假值（`undefined | null | false`）时禁用此功能
- 设为 `string` 时，需提供相对于博客内容目录的自定义 `.xsl` 文件路径。按照约定，必须提供同名 `.css` 文件。
...
Markdown 文档可使用以下 [前置元数据](../../guides/markdown-features/markdown-features-intro.mdx#front-matter) 字段，需用 `---` 分隔线包裹。
...
通过 [`authors` 插件选项](#authors) 配置 YAML 作者文件路径。

按照约定，插件会在博客内容目录根目录查找 `authors.yml` 文件。

该文件可包含预定义的 [全局博客作者](../../blog.mdx#global-authors)。在 Markdown 文件中可通过 [`authors` 前置元数据](#markdown-front-matter) 引用这些作者的键名。

### 类型 {#authors-file-types}

提供的作者文件 YAML 内容需符合以下结构：
