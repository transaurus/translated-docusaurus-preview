...
此插件在开发模式下始终处于非活跃状态，**仅在生产环境生效**，因其作用于构建输出阶段。
...
可通过[预设配置](../../using-plugins.mdx#docusauruspreset-classic)进行配置。
...
`lastmod`选项仅在插件提供[路由元数据](../plugin-methods/lifecycle-apis.mdx#addRoute)属性`sourceFilePath`和/或`lastUpdatedAt`时输出站点地图的`<lastmod>`标签。

所有官方内容插件都会为基于内容文件（Markdown、MDX或React页面组件）的路由提供元数据，但第三方插件作者可能未提供此信息，此时插件将无法为其路由输出`<lastmod>`标签。
