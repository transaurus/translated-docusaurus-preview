...
用于生成近乎理想图像（响应式、懒加载、低质量占位符）的 Docusaurus 插件。
...
本插件注册的 [Webpack loader](https://webpack.js.org/loaders/) 会改变导入/require 图像的类型：
...
从 [pnpm 10](https://github.com/pnpm/pnpm/releases/tag/v10.0.0) 开始，运行 `pnpm install` 默认不会执行依赖项的安装脚本。您需要额外的 pnpm 配置（[相关 issue](https://github.com/lovell/sharp/issues/4343)）才能正确安装我们用于图像缩放的 `sharp` 依赖项，例如：
