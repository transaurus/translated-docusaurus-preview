...
"from"与"to"的概念是本插件的核心。"from"表示您希望创建的路径（即待生成的额外HTML文件）；"to"表示重定向目标路径（通常是Docusaurus已知的路由）。

因此可以存在多个"from"指向同一个"to"（生成多个重定向至同一目标的HTML文件），但一个"from"不能对应多个"to"（生成的HTML文件必须有确定的重定向目标）。
