...
不过您通常无需逐个安装常用插件：它们可以以[预设包](#using-presets)的形式分发。对于大多数用户而言，插件是通过预设配置进行管理的。

我们维护了[官方插件列表](./api/plugins/overview.mdx)，社区也开发了一些[非官方插件](/community/resources#community-plugins)。如需添加自动生成文档页面、执行自定义脚本、集成第三方服务等功能，建议先查阅这些列表：可能已有现成解决方案！

若您精力充沛，也可以阅读[插件开发指南](./advanced/plugins.mdx)或[插件方法参考文档](./api/plugin-methods/README.mdx)来自行开发插件。
...
路径应使用绝对路径或相对于配置文件的路径。

## 配置插件 {#configuring-plugins}

最基本的插件使用方式是在配置中直接提供插件名称或路径。

若需配置插件选项，可采用"Babel风格"的二元组形式：将插件名称与选项对象包裹在数组内。
...
所有Docusaurus内容插件都支持多实例运行。例如，您可能需要[配置多个文档插件实例](./guides/docs/docs-multi-instance.mdx)或[运行多个博客](./blog.mdx#multiple-blogs)。每个插件实例必须分配唯一ID，默认ID为`default`。
...
主题的加载方式与插件完全相同。从使用者角度看，在安装和配置时，`themes`与`plugins`选项可互换使用。唯一区别在于主题加载顺序晚于插件，且[主题可以覆盖插件的默认主题组件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes`和`plugins`选项会触发不同的[简写解析规则](#module-shorthands)，如需使用简写形式，请确保选择正确的配置项！
...
预设（Presets）是插件和主题的集合包。例如，与其让您在配置文件中逐个注册和配置`@docusaurus/plugin-content-docs`、`@docusaurus/plugin-content-blog`等插件，我们提供了`@docusaurus/preset-classic`预设，允许您在一个集中位置配置它们。
...
预设通常是一个npm包，因此您可以像安装其他npm包一样使用npm安装它们。
...
Docusaurus支持插件、主题和预设的简写形式。当它看到一个插件/主题/预设名称时，会尝试按以下顺序加载：
...
以下是在`plugins`字段中注册插件的一些示例。请注意，与[ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins)或[Babel](https://babeljs.io/docs/en/options#name-normalization)强制要求插件命名规范不同，Docusaurus允许更自由的命名方式，因此解析结果并不确定，但会遵循上述定义的优先级。
