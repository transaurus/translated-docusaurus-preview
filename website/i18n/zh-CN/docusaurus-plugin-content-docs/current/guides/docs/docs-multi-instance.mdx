...
当有人编辑iOS文档时，重新构建包括未变更的Android文档在内的全部内容是否必要？
...
我们将`product`实例视为最重要实例，通过不分配ID使其成为"默认"实例。
...
每个与文档相关的[主题导航栏项目](../../api/themes/theme-configuration.mdx#navbar)都支持可选的`docsPluginId`属性。

例如，若需要为每个移动端SDK（iOS和Android）分别设置版本下拉菜单，可以这样配置：
