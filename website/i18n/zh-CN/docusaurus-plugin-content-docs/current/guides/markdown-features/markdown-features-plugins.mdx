...
答案是：创建 MDX 插件！MDX 内置的[插件系统](https://mdxjs.com/advanced/plugins/)可自定义 Markdown 文件如何被解析并转换为 JSX。MDX 插件有三种典型使用场景：
...
使用插件为项目中常用的 JSX 元素引入简洁语法。我们提供的[告示栏语法](./markdown-features-admonitions.mdx)也是通过 Remark 插件实现的，您可以为自己的用例实现类似功能。
...
MDX 插件通常是 npm 包，因此您可以通过 npm 像安装其他包一样安装它们。以[数学公式插件](./markdown-features-math-equations.mdx)为例。
...
部分插件支持配置并接受自定义选项。这种情况下请使用 `[plugin, pluginOptions]` 语法，例如：
...
例如，我们创建一个为所有二级标题添加"第X节"前缀的插件。首先在任意位置创建插件源文件（也可发布为独立npm包并通过上述方式安装），此处我们将插件放在`src/remark/section-prefix.js`。remark/rehype插件本质上是接收`options`参数并返回AST转换器的函数。
...
Docusaurus默认插件会优先于自定义remark插件执行，这意味着图片/链接在被自定义插件处理前已通过`require()`转换为JSX。例如上例中，虽然所有二级标题都添加了"第X节"前缀，但生成的目录仍保持原样，因为目录生成插件先于自定义插件执行。若需在默认插件前处理MDAST，请使用`beforeDefaultRemarkPlugins`和`beforeDefaultRehypePlugins`：
