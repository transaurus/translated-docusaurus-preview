...
该服务对开发者文档和技术博客**完全免费**：请确保阅读[准入标准](https://docsearch.algolia.com/docs/who-can-apply/)并[申请加入DocSearch计划](https://docsearch.algolia.com/apply)。

DocSearch每周自动爬取您的网站（可通过网页界面调整频率），将所有内容聚合至Algolia索引库。前端通过Algolia API直接查询这些内容。

如果您的网站[不符合](https://docsearch.algolia.com/docs/who-can-apply)免费托管版DocSearch的准入标准，或网站部署在防火墙后非公开访问，您可以[自行运行](https://docsearch.algolia.com/docs/run-your-own/)DocSearch爬虫。
...
申请通过并部署后，您将收到包含所有配置详情的邮件。通过[网页界面](https://crawler.algolia.com/)可编辑和管理爬取任务。索引库在部署后立即可用，通常无需手动配置。
...
爬虫配置中的`initialIndexSettings`仅在初始化Algolia索引库时生效。

如需更新`initialIndexSettings`配置，可通过界面手动更新索引，但[Algolia团队建议删除旧索引后重新爬取](https://github.com/facebook/docusaurus/issues/9200#issuecomment-1667338492)，以确保新设置完全生效。
...
在`themeConfig`中添加`algolia`字段。[申请DocSearch服务](https://docsearch.algolia.com/apply/)以获取Algolia索引库和API密钥。
...
具体可配置值请参阅 [官方 DocSearch 文档](https://docsearch.algolia.com/docs/api#searchparameters)。

:::

:::warning

在 Algolia 爬虫完成站点抓取前，搜索功能可能无法稳定工作。

若重大变更后搜索失效，请通过 Algolia 控制台**手动触发重新抓取**。
...
当浏览 v2 文档时，展示 v1 版本的搜索结果显然不合逻辑。由于不同版本文档内容可能高度相似，这会导致同一查询返回重复结果（每个版本各出现一次）。

同理，在法语站点浏览时，返回英文文档结果也属异常。

为解决此问题，上下文搜索功能会动态识别当前浏览的文档版本与语言，并自动生成对应的搜索筛选条件。

- 访问 `/en/docs/v1/myDoc` 时，搜索结果仅包含 **v1 版本英文文档**（及未版本化的页面）
- 访问 `/fr/docs/v2/myDoc` 时，搜索结果仅包含 **v2 版本法语文档**（及未版本化的页面）
...
DocSearch 默认采用符合无障碍标准的精心调校主题，确保色彩与对比度达到规范要求。

您仍可通过修改 `/src/css/custom.css` 文件，使用 Docusaurus 提供的 [Infima CSS 变量](styling-layout.mdx#styling-your-site-with-infima)来定制样式。
...
Algolia DocSearch 支持通过 [`docusaurus.config.js` 文件中的 `algolia` 字段](https://docsearch.algolia.com/docs/api/)传递多种配置参数。
...
如需直接修改 Algolia 搜索的 React 组件，可通过[置换(swizzle)](swizzling.mdx)功能处理 `@docusaurus/theme-search-algolia` 中的 `SearchBar` 组件：
...
**注意**：您也可以[从 Algolia SearchBar 开始替换](#editing-the-algolia-search-component)，并基于此创建自己的搜索组件。