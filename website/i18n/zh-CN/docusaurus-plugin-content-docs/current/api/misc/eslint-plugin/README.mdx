...
[ESLint](https://eslint.org/) 是一款静态代码分析工具，通过编辑器提示和命令行报告代码问题或建议最佳实践。Docusaurus 提供了 ESLint 插件来强制执行 Docusaurus 的最佳实践。
...
这将启用 `@docusaurus` ESLint 插件并使用 `recommended` 配置。下方[支持的规则](#supported-rules)列出了该配置会启用的规则列表。

### 手动配置

如需更细粒度的控制，您也可以手动启用插件并直接配置需要使用的规则：
...
- 全部规则：启用**所有**规则。此配置会在次要版本间发生变化，若希望避免意外破坏性变更，不建议使用。
