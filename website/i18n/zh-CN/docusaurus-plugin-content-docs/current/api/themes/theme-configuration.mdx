...
有时需要在网站发布公告。为此可添加公告栏——这是一个位于导航栏上方、非固定且可选关闭的面板。所有配置都在`announcementBar`对象中。
...
标志可放置在[静态文件夹](static-assets.mdx)中。标志URL默认设为站点根路径。虽然可以自定义标志URL，但如果是外部链接将在新标签页打开。此外可覆盖标志链接的target属性值，当文档网站托管在主站点的子目录时特别有用——这种情况下可能不需要标志链接在主站新标签页打开。
...
根据`type`字段的不同，导航栏项可具有不同行为。以下章节将介绍所有可用的导航栏项类型。

#### 导航栏链接 {#navbar-link}

默认情况下，导航栏项目是常规链接（内部或外部）。

React Router 应自动为链接应用活动链接样式，但在边缘情况下可使用 `activeBasePath`。若某链接需在多个不同路径下保持活动状态（例如同一侧边栏下存在多个文档文件夹），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更灵活的替代方案，且优先级更高——Docusaurus 会将其解析为正则表达式并与当前 URL 进行匹配测试。
...
注意下拉菜单基础项目本身也是可点击链接，因此该项目可接收[普通导航栏链接](#navbar-link)的任何属性。
...
若需链接至特定文档，此特殊导航栏项目类型将渲染指向所提供 `docId` 文档的链接。只要浏览同一侧边栏下的文档，该链接就会获得 `navbar__link--active` 类。
...
若使用带版本控制的文档，此特殊导航栏项目类型将渲染包含站点所有可用版本的下拉菜单。
...
若使用带版本控制的文档，此特殊导航栏项目类型将链接至文档的当前活动/浏览版本（取决于当前 URL），并回退至最新版本。
...
若启用了[国际化功能](../../i18n/i18n-introduction.mdx)，此特殊导航栏项类型会渲染包含所有可用语言的下拉菜单。
...
可启用此炫酷的UI特性：当用户向下滚动页面时自动隐藏导航栏，向上滚动时重新显示。
...
目前支持两种样式选项：`dark`（暗色）和`primary`（基于`--ifm-color-primary`主色调）。样式预览可参见[Infima文档](https://infima.dev/docs/components/navbar/)。
...
Docusaurus使用[Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)实现代码块高亮。所有配置均在`prism`对象中设置。
...
默认使用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)作为语法高亮主题。您可以从[可用主题列表](https://github.com/FormidableLabs/prism-react-renderer/tree/master/packages/prism-react-renderer/src/themes)指定自定义主题，还可为暗色模式设置不同的语法高亮主题。
...
若使用行高亮Markdown语法，可能需要为暗色模式语法高亮主题指定不同的高亮背景色。具体请参考[行高亮指南](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)。

:::

### 默认语言 {#default-language}

可为未指定语言的代码块（即仅以```开头的代码块）设置默认语言。注意必须传入有效的[语言名称](https://prismjs.com/#supported-languages)。
...
您可以通过 `themeConfig.footer` 为页脚添加徽标和版权信息。徽标可放置在[静态文件夹](static-assets.mdx)中，其URL使用方式与导航栏徽标相同。
...
- `colorModeChoice: 'light' | 'dark' | null`：用户明确选择的色彩模式。若用户未做选择或重置为系统/默认值时可为`null`
- `setColorMode(colorModeChoice: 'light' | 'dark' | null, options: {persist: boolean}): void`：用户明确选择色彩模式时调用的函数。传入`null`可重置为系统/默认值。默认情况下该选择会持久化至`localStorage`并在页面重载时恢复，但可通过`{persist: false}`禁用此行为

:::warning

不要在渲染 React 组件时使用 `colorMode` 和 `colorModeChoice`。这样做很可能会导致[无样式内容闪烁(FOUC)](https://en.wikipedia.org/wiki/Flash_of_unstyled_content)、布局偏移以及[React 水合](https://18.react.dev/reference/react-dom/client/hydrateRoot)不匹配问题，特别是当你用它们来动态渲染 JSX 内容时。

不过，**在 React 完成水合之后**，这些值可以安全地在 `useEffect` 和事件监听器中使用，就像上面 `MyColorModeButton` 示例中那样。

如果需要根据当前主题动态渲染内容，避免 FOUC、布局偏移和水合不匹配的唯一方法是依赖 CSS 选择器，基于我们在页面显示任何内容之前设置的 `html` 数据属性来动态渲染内容：
...
<details>
 <summary>为什么在渲染时使用 `colorMode` 和 `colorModeChoice` 不安全？</summary>

要理解这个问题，需要先了解 [React 水合](https://18.react.dev/reference/react-dom/client/hydrateRoot)的工作原理。

在静态站点生成阶段，Docusaurus 并不知道用户选择的色彩模式，此时 `useColorMode()` 会返回以下静态值：

- `colorMode = themeConfig.colorMode.defaultMode`
- `colorModeChoice = null`

在 React 客户端首次渲染（水合）时，React 必须生成完全相同的 HTML 标记，同样会使用这些静态值。

正确的 `colorMode` 和 `colorModeChoice` 值只会在第二次 React 渲染时提供。

典型情况下，以下组件会导致 **React 水合不匹配**。标签可能会在 React 水合过程中从 `light` 切换到 `dark`，造成混乱的用户体验。
