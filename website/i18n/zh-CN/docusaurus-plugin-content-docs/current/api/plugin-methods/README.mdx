...
`options`是[使用插件时的第二个可选参数](../../using-plugins.mdx#configuring-plugins)。`options`是插件特定的配置项，由用户在`docusaurus.config.js`中指定。如果导出了[`validateOptions`](./static-methods.mdx#validateOptions)函数，这些选项会先经过验证和规范化处理。

另外，如果预设配置中包含该插件，则预设配置需要负责将正确的选项传递给插件。具体接受哪些选项由各个插件自行定义。
