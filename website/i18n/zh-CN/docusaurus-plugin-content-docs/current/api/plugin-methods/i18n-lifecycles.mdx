...
- `path`：相对于插件本地化文件夹 `i18n/[locale]/[pluginName]` 的路径。扩展名 `.json` 应省略以保持通用性。
- `content`：使用Chrome i18n JSON格式。

这些文件将由 [`write-translations` CLI](../../cli.mdx#docusaurus-write-translations-sitedir) 写入到插件的i18n子文件夹中，并在调用 [`translateContent()`](#translateContent) 和 [`translateThemeConfig()`](#translateThemeConfig) 之前读取适当的本地化内容。
