...
用于向Docusaurus站点添加[Google标签管理器(gtm.js)](https://developers.google.com/tag-platform/tag-manager)的插件。将此插件与标准[gtag插件](./plugin-google-gtag.mdx)配合使用，可深入分析用户如何使用您的站点。
...
该插件在开发环境中始终处于非活动状态，**仅在生产环境激活**，以避免污染分析统计数据。
