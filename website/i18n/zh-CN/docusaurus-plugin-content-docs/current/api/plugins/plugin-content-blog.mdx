...
[订阅功能](../../blog.mdx#feed) 通过解析构建输出实现，因此**仅在生产环境激活**。
...
用于通过 [XSLT](https://developer.mozilla.org/en-US/docs/Web/XSLT) 美化博客 XML 订阅源在浏览器中的渲染效果。

- 设为 `true` 时使用插件内置的 `.xsl` 和 `.css` 文件样式
- 设为假值（`undefined | null | false`）时禁用该功能
- 设为 `string` 时可指定博客内容目录下自定义 `.xsl` 文件路径（需配套同名 `.css` 文件）
...
Markdown 文档可使用以下被 `---` 分隔线包裹的 [前置元数据](../../guides/markdown-features/markdown-features-intro.mdx#front-matter) 字段。
...
通过 [`authors` 插件选项](#authors) 可配置 YAML 作者文件的路径。

默认会在博客内容目录根路径下查找 `authors.yml` 文件。

该文件可预定义 [全局博客作者](../../blog.mdx#global-authors)，通过 Markdown 文件中的 [`authors` 前置元数据](#markdown-front-matter) 可引用这些作者键名。
