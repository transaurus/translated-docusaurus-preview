...
本插件还会读取[`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes)配置，当多个文件将输出至相同位置时调整其日志级别。
...
"from"与"to"的概念是本插件的核心。"from"表示您希望创建的路径（即待生成的额外HTML文件）；"to"表示重定向目标路径（通常是Docusaurus已识别的路由）。
