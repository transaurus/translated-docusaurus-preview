...
- 将`versioned_docs`的ID引用从`version-${version}-${original_id}`(v1)改为`version-${version}/${id}`(v2)。

因为在v1中，很可能有人创建了front matter ID为`"version-${version}-${id}"`的新文件，这会与`versioned_docs`的ID产生冲突。
...
由于我们在v1和v2中都不允许front matter使用`/`字符，因此冲突的可能性更低。
...
在v2中，我们采用快照机制进行文档版本控制。**每个版本的文档不依赖于其他版本**。可能会出现`foo.md`存在于`version-1.0.0`但不存在于`version-1.2.0`的情况。由于Docusaurus v1的回退功能(https://v1.docusaurus.io/docs/en/versioning#fallback-functionality)，这在之前版本中是不可能的。

例如，如果您的v1版本中`versions.json`如下所示：

```json title="versions.json"
["1.1.0", "1.0.0"]
```

Docusaurus v1**仅当文档内容不同时**才会创建版本化文档。如果从v1.0.0到v1.1.0唯一变化的文档是`hello.md`，您的文档结构可能如下所示：
...
在v2中，您需要填充缺失的`versioned_docs`和`versioned_sidebars`（同时确保front matter和ID引用正确）。
...
Docusaurus 2 对文档文件使用 JSX 语法。如果您在 Docusaurus 1 的文档中使用了任何样式属性，需将其转换为样式对象，如下所示：
