...
一种**可行的翻译策略**是通过 Git（或任何其他[版本控制系统](https://en.wikipedia.org/wiki/Version_control)）**对翻译文件进行版本控制**。
...
有关我们的研究笔记和相关链接，请参阅 [Docusaurus 国际化 RFC](https://github.com/facebook/docusaurus/issues/3317)。

:::

## 初始化 {#initialization}

以下是通过 Git 将新初始化的英文 Docusaurus 网站翻译为法文的步骤，假设您已按照[国际化教程](./i18n-tutorial.mdx)操作。
...
保持翻译文件与原始文件**一致可能具有挑战性**，特别是对于 Markdown 文档。

### Markdown 翻译 {#markdown-translations}

当未翻译的 Markdown 文档被编辑时，**您有责任维护相应的翻译文件**，遗憾的是我们目前没有好的方法来帮助您完成此操作。

为了保持翻译站点的一致性，当 `website/docs/doc1.md` 文档被编辑时，您需要**将这些修改反向移植**到 `i18n/fr/docusaurus-plugin-content-docs/current/doc1.md`。
...
您的翻译文件托管在 Git 上，可通过文档和博客插件的 `editLocalizedFiles: true` 选项进行配置。

启用后，编辑按钮将指向本地化文档路径 `i18n/fr/docusaurus-plugin-content-docs/current/doc1.md`。