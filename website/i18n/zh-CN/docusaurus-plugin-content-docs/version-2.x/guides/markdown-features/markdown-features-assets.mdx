...
有时您需要直接从 Markdown 文件链接到资源（如 docx 文件、图片等），将资源文件与使用它的 Markdown 文件放在同一位置会非常方便。
...
同样地，您可以通过 `require` 资源文件并使用返回的 URL 来链接到现有资源，例如在 `video`、`a` 锚点链接等中使用。
...
如果您使用 Markdown 图片或链接语法，Docusaurus 会将所有资源路径解析为文件路径，并自动转换为 `require()` 调用。除非您使用 JSX 语法（这种情况下需要自行处理），否则在 Markdown 中无需使用 `require()`。

:::

## 内联 SVG {#inline-svgs}

Docusaurus 默认支持内联 SVG。
...
如果您希望通过 CSS 修改 SVG 图像的某些部分，这会非常有用。例如，您可以根据当前主题更改 SVG 的某一部分颜色。
...
GitHub 使用其独有的[图片主题化方案](https://github.blog/changelog/2021-11-24-specify-theme-context-for-images-in-markdown/)，通过路径片段实现，您可以轻松自行实现类似功能。

要通过路径片段（GitHub 使用的是 `#gh-dark-mode-only` 和 `#gh-light-mode-only`）切换图片可见性，请将以下内容添加到自定义 CSS 中（如果您不想与 GitHub 耦合，也可以使用自己的后缀）：
...
如果 Markdown 链接或图片使用绝对路径，该路径将被视为文件路径，并从静态目录中解析。例如，如果您配置了[静态目录](../../static-assets.mdx)为 `['public', 'static']`，那么对于以下图片：

```md title="my-doc.md"
![An image from the static](/img/docusaurus.png)
```

Docusaurus 会尝试在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 中查找它。链接随后会被转换为 `require()` 调用，而不是保留为 URL。这在两方面是可取的：

1. 您无需关心基础URL，Docusaurus会在提供资源时自动处理；
2. 图片会进入Webpack的构建流程，其名称会被附加哈希值，这使得浏览器能积极缓存图片并提升站点性能。
