...
`context`是与插件无关的上下文对象，同一个对象会被传递给Docusaurus网站使用的所有插件。`context`对象包含以下字段：
...
`options`是[插件使用时的第二个可选参数](../../using-plugins.mdx#configuring-plugins)。`options`是插件特定的配置，由用户在`docusaurus.config.js`中指定。如果导出了[`validateOptions`](./static-methods.mdx#validateOptions)函数，这些选项会先经过验证和规范化处理。

另外，如果预设(preset)包含该插件，则由预设负责将正确的选项传递给插件。具体接受哪些选项由各个插件自行定义。
