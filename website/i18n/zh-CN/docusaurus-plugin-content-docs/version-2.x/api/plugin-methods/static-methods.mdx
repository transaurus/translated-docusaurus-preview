...
静态方法不属于插件实例——它们被附加到构造函数上。这些方法用于验证和规范化插件选项及主题配置，随后这些参数将作为构造函数的入参来初始化插件实例。
...
为避免 Joi 版本冲突，请使用 `const {Joi} = require("@docusaurus/utils-validation")`
...
在 TypeScript 中，您也可以选择将其作为单独命名导出。
