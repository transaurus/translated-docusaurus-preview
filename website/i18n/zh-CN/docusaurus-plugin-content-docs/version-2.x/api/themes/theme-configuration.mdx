...
有时需要在网站发布公告。为此可添加公告栏——这是一个非固定且可选关闭的面板，位于导航栏上方。所有配置均在`announcementBar`对象中。
...
标志可放置在[静态文件夹](static-assets.mdx)中。标志URL默认设为站点基础URL。虽然可自定义标志URL，但外部链接会在新标签页打开。此外可覆盖标志链接的target属性值，当文档网站托管在主站点的子目录时特别有用——此时可能不需要标志链接在主站新标签页打开。

为增强深色模式支持，还可为此模式设置不同标志。
...
根据`type`字段不同，项可具有不同行为。以下章节将介绍所有可用的导航栏项类型。

#### 导航栏链接 {#navbar-link}

默认情况下，导航栏项是常规链接（内部或外部）。

React Router 通常会自动为链接应用活动链接样式，但在边缘情况下可以使用 `activeBasePath`。当某个链接需要在多个不同路径下保持活动状态时（例如同一侧边栏下存在多个文档目录），可以使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更灵活的替代方案且优先级更高——Docusaurus 会将其解析为正则表达式并与当前 URL 进行匹配测试。

外链（外部链接）会自动添加 `target="_blank" rel="noopener noreferrer"` 属性。
...
除上述字段外，您还可以指定其他任意可应用于 HTML 链接的属性。
...
请注意，下拉菜单的基础项目本身也是可点击链接，因此该项目可以接收[普通导航栏链接](#navbar-link)的任何属性。
...
如需链接到特定文档，此特殊导航栏项目类型会渲染指向所提供 `docId` 文档的链接。只要您浏览同一侧边栏下的文档，该链接就会获得 `navbar__link--active` 类。
...
您可以将导航栏项目链接到给定侧边栏的第一个文档链接（可以是文档链接或生成的分类索引），而无需硬编码文档ID。
...
若使用版本化文档，此特殊导航栏项目类型会渲染包含站点所有可用版本的下拉菜单。

用户可在保持当前文档不变的情况下切换不同版本（前提是文档ID在各版本间保持一致）。
...
若使用版本化文档，此特殊导航栏项目类型会链接到当前浏览的文档版本（取决于当前URL），并默认回退至最新版本。
...
若启用了[i18n国际化功能](../../i18n/i18n-introduction.mdx)，此特殊导航栏项类型会渲染一个下拉菜单，显示站点所有可用语言版本。
...
您还可以使用此导航栏项类型，在导航项中渲染自定义HTML标记。
...
可启用此炫酷的UI特性：当用户开始向下滚动页面时自动隐藏导航栏，向上滚动时重新显示。
...
目前有两种可选样式：`dark`（暗色）和基于`--ifm-color-primary`颜色的`primary`（主色）。样式预览参见[Infima文档](https://infima.dev/docs/components/navbar/)。
...
默认使用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)作为语法高亮主题。您可以从[可用主题列表](https://github.com/FormidableLabs/prism-react-renderer/tree/master/src/themes)指定自定义主题，也可为暗色模式站点设置不同的语法高亮主题。
...
若使用行高亮Markdown语法，可能需要为暗色模式语法高亮主题指定不同的高亮背景色。具体操作请参考[行高亮文档](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)。

:::

### 默认语言 {#default-language}

可为代码块设置默认语言（当开头的三个反引号后未指定语言时生效）。注意必须传递有效的[语言名称](https://prismjs.com/#supported-languages)。
...
可通过`themeConfig.footer`添加页脚Logo和版权信息。Logo可放置在[static静态文件夹](static-assets.mdx)中，其URL处理方式与导航栏Logo相同。
...
多栏页脚链接为每列设置一个 `title` 和一组 `FooterItem` 列表。
...
用于访问颜色模式的React钩子。该上下文包含设置浅色/深色模式的函数，并暴露表示当前模式的布尔变量。
