...
- **灵活的翻译工作流**：可使用 Git（monorepo、fork 或子模块）、SaaS 软件或 FTP
- **灵活的部署选项**：支持单域名、多域名或混合部署
- **模块化**：允许插件作者提供国际化支持
- **低运行时开销**：文档主要是静态的，不需要繁重的 JS 库或 polyfill
...
- **良好的 SEO 默认设置**：自动设置有用的 SEO 头信息，如 [`hreflang`](https://developers.google.com/search/docs/advanced/crawling/localized-versions)
- **RTL 支持**：支持从右到左阅读的语言（阿拉伯语、希伯来语等），且易于实现
...
1. **配置**：在 `docusaurus.config.js` 中声明默认语言和替代语言
...
Markdown 和 MDX 文档整体翻译，以完整保留翻译上下文，而非将每个句子拆分为单独的字符串。
...
对于多实例插件，路径格式为`website/i18n/[locale]/[pluginName]-[pluginId]/...`。

:::

翻译一个简单的法语版Docusaurus站点会生成如下目录结构：
...
JSON文件可通过[`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir)命令行工具初始化。各插件在对应目录下管理自己的翻译内容，而`code.json`文件则定义React代码中使用的所有文本标签。
