...
您可以自由选择**工具和 SaaS 服务**与 Docusaurus 集成，只需确保将**翻译文件放置在正确位置**即可。
...
Crowdin 提供 [CLI 工具](https://support.crowdin.com/cli-tool/)用于**上传源文件**和**下载译文**，支持翻译流程自动化。
...
本教程将演示如何使用 Crowdin 将新建的英文版 Docusaurus 网站翻译为法文版，假设您已完成[国际化教程](./i18n-tutorial.mdx)的准备工作。
...
在 [Crowdin](https://crowdin.com/) 注册账号并创建项目。
...
建议先使用`隐藏字符串`功能，因为Crowdin的预翻译有时会过度处理内容。
...
我们将配置CI在构建时**自动下载Crowdin翻译文件**，并保持这些文件不受Git管理。
...
Crowdin对并发上传/下载支持不佳：建议仅在生产部署中包含翻译内容，保持部署预览版本不翻译。
...
使用简单字符串属性的基础JSX片段（如`<Username name="Sebastien"/>`）可以正常工作；但使用对象/数组属性的复杂片段（如`<User person={{name: "Sebastien"}}/>`）更可能因语法不符合HTML规范而失败。

#### MDX解决方案 {#mdx-solutions}

我们建议将复杂的嵌入式 JSX 代码提取为独立的组件。我们还添加了 `mdx-code-block` 转义语法：
...
- 被 Crowdin 识别为代码块（下载时不会破坏标记结构）
- 被 Docusaurus 识别为常规 JSX（就像未被任何代码块包裹一样）
- 但会失去 MDX 工具链支持（IDE 语法高亮、Prettier 格式化等）
...
Crowdin 提供 `Duplicate Strings` 设置项。
...
不使用 `Hide` 会导致配额中的 `source strings` 数量激增，进而影响计费。
...
当您在 Git 中删除或重命名源文件时，Crowdin CLI 会显示警告：

![Crowdin CLI 下载翻译警告](/img/crowdin/crowdin-download-translations-warning.png)

重构源文件后，应通过 Crowdin 界面手动更新文件：

![Crowdin 文件重命名界面](/img/crowdin/crowdin-files-rename.png)

### 版本控制系统集成 {#vcs-git-integrations}

Crowdin 提供与 [GitHub](https://support.crowdin.com/github-integration/)、GitLab、Bitbucket 的多种集成方案。
...
理论上这能实现 Git 与 Crowdin 的双向编辑，并建立双向同步机制。

但实际应用中存在可靠性问题：

- Crowdin → Git 同步正常（通过拉取请求实现）
- Git → Crowdin 同步需手动触发（需点击按钮）
- Crowdin 匹配现有 Markdown 翻译与源文件的算法并非 100% 可靠，同步后需人工校验
- 用户同时在 Git 和 Crowdin 编辑可能导致翻译丢失
...
目前因技术限制无法正常工作，但我们认为未来可能解决此问题。

Crowdin 会将 Markdown 字符串替换为类似 `crowdin:id12345` 的技术 ID，但替换过程过于激进，影响了 front matter、admonitions 和 JSX 等本应隐藏的内容。
...
您可以通过使用 `editUrl` 函数按语言环境自定义编辑 URL，使编辑按钮链接到 Crowdin 界面。
...
**Docusaurus v2 的配置文件**是使用版本控制和多实例功能的良好示例：
