...
但这会惩罚其他所有用户，因为 29 个字符的代码行现在变成了 168 个字符——增加了 6 倍！（实际情况下会好一些，因为使用的名称会更短。）作为权衡，JS 加载器只会将语法转译到浏览器列表中所有版本都支持的程度。
...
使用默认经典模板初始化的网站在 `package.json` 中包含以下配置：
...
- 市场份额超过 0.5%；且
- 在过去 24 个月内有官方支持或更新（即非"已死亡"浏览器）；且
- 不是 Opera Mini。
