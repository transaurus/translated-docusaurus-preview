...
- Jamstack 提供商（如 [Netlify](#deploying-to-netlify) 和 [Vercel](#deploying-to-vercel)）。我们将以它们为参考，但同样的逻辑适用于其他提供商。
...
后续章节将介绍常见托管服务商及其高效部署 Docusaurus 站点的配置方法。Docusaurus 与这些服务无官方关联，本文档仅作信息参考。部分说明由第三方提供，可能未及时反映 API 变更。若发现内容过时，欢迎提交 PR 修正。

由于我们仅能基于最佳实践提供有限支持，现已停止接收新增托管方案的 PR。您可将部署指南发布在其他平台（如个人博客或服务商官网），并申请在此添加相关链接。
...
若未预先配置这些选项，站点创建后仍可前往「站点设置」→「构建与部署」进行修改。

完成上述配置后，站点将自动部署，并在代码合并至部署分支（默认为 `main`）时触发重新部署。

:::warning

部分 Docusaurus 站点采用旧版 v1 结构，将 `docs` 目录置于 `website` 之外：
...
若将 `website` 目录设为 Netlify 基础目录，则更新 `docs` 文件夹时不会触发构建，需配置 [自定义忽略命令](https://docs.netlify.com/configure-builds/common-configurations/ignore-builds/)：
...
**特别注意**：全局复选框「禁用资源优化」存在缺陷，无法实际关闭「美化 URL」功能。请务必单独取消勾选该选项。
...
将 Docusaurus 项目部署至 [Vercel](https://vercel.com/) 可享受[性能与易用性方面的多重优势](https://vercel.com/)。

如需通过 [Vercel Git 集成](https://vercel.com/docs/concepts/git)部署，请确保项目已推送至 Git 代码库。
...
发布流程通常涉及两个仓库（或至少两个分支）：包含源文件的分支，以及包含构建输出供 GitHub Pages 服务使用的分支。下文将分别称为 **"源分支"** 和 **"部署分支"**。

每个 GitHub 仓库都关联 GitHub Pages 服务。若部署仓库名为 `my-org/my-project`（`my-org` 为组织名或用户名），部署站点将位于 `https://my-org.github.io/my-project/`。若部署仓库名为 `my-org/my-org.github.io`（组织专属 GitHub Pages 仓库），站点将位于 `https://my-org.github.io/`。

:::info

如需使用自定义域名，请在 `static` 目录创建 `CNAME` 文件。`static` 目录下所有内容都会复制到构建目录根部。使用自定义域名时，可将配置从 `baseUrl: '/projectName/'` 改回 `baseUrl: '/'`，并将 `url` 设为自定义域名。

更多细节请参阅 GitHub Pages 文档 [用户、组织和项目页面](https://help.github.com/en/articles/user-organization-and-project-pages)。

:::

GitHub Pages 会从默认分支（通常为 `master`/`main`）或 `gh-pages` 分支的根目录/`docs` 文件夹获取部署文件（`docusaurus build` 的输出）。可通过仓库的 `Settings > Pages` 进行配置，该分支称为"部署分支"。
...
GitHub Pages 默认会给 Docusaurus URL 添加尾部斜杠。建议明确设置 `trailingSlash` 配置（`true` 或 `false`，不要设为 `undefined`）。
...
GitHub Pages 默认会使用 [Jekyll](https://jekyllrb.com/) 处理发布文件。由于 Jekyll 会忽略以下划线 `_` 开头的文件，建议在 `static` 目录添加空文件 `.nojekyll` 来禁用此功能。
...
GitHub 企业版与 github.com 操作方式相同，只需将组织的 GitHub Enterprise 主机设为环境变量：
...
自 2021 年 8 月起，GitHub 要求所有命令行登录必须使用**个人访问令牌**（PAT）而非密码。当 GitHub 提示输入密码时，请输入 PAT 代替。详情请参阅 [GitHub 文档](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token)。
...
以下工作流示例假设您的网站源代码位于仓库的 `main` 分支（即_源分支_为 `main`），且您的[发布源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site)配置为[使用自定义 GitHub Actions 工作流发布](https://docs.github.com/en/pages/getting-started-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site#publishing-with-a-custom-github-actions-workflow)。

我们的目标是：

1. 当向 `main` 分支发起新的拉取请求时，执行一个确保站点能成功构建但不实际部署的操作。该任务称为 `test-deploy`。
2. 当拉取请求被合并到 `main` 分支或有人直接推送到 `main` 分支时，构建并部署到 GitHub Pages。该任务称为 `deploy`。
...
持续集成（CI）服务通常用于在提交新代码到源代码管理时执行例行任务。这些任务可以是运行单元测试和集成测试、自动化构建、将包发布到 npm 以及将更改部署到您的网站的任何组合。要自动化网站的部署，您只需在网站更新时调用 `yarn deploy` 脚本。以下部分介绍如何使用流行的持续集成服务提供商 [Travis CI](https://travis-ci.com/) 来实现这一点。
...
3. 打开您的 Travis CI 仪表板（网址类似 `https://travis-ci.com/USERNAME/REPO`），导航至仓库的 `More options > Setting > Environment Variables` 部分。
...
完成该简单流水线创建后，每次向选定分支推送新提交时，系统将通过 `yarn deploy` 将网站部署至 GitHub Pages。阅读[本指南](https://buddy.works/guides/react-docusaurus)了解如何为 Docusaurus 设置 CI/CD 流水线。
...
4. 在项目页面（形如 `https://dev.azure.com/ORG_NAME/REPO_NAME/_build`）中，使用以下文本创建新流水线。点击编辑并添加名为 `GH_TOKEN` 的环境变量（值为新生成的令牌），以及 `GH_EMAIL`（您的邮箱地址）和 `GH_NAME`（GitHub 用户名）。务必将其标记为机密。您也可在仓库根目录创建名为 `azure-pipelines.yml` 的文件。
...
2. 为私钥和公钥命名以保持特异性，避免覆盖其他[SSH密钥](https://docs.github.com/en/free-pro-team@latest/github/authenticating-to-github/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent)。
...
[Flightcontrol](https://www.flightcontrol.dev/?ref=docusaurus) 是一项可直接从 Git 仓库将 Web 应用自动构建并部署至 AWS Fargate 的服务。它提供完整的基础设施检查与修改权限，突破传统 PaaS 的限制。

请按照 [Flightcontrol 的 Docusaurus 分步指南](https://www.flightcontrol.dev/docs/reference/examples/docusaurus/?ref=docusaurus) 开始操作。

## 部署到 Koyeb {#deploying-to-koyeb}

[Koyeb](https://www.koyeb.com) 是一个对开发者友好的无服务器平台，用于全球部署应用程序。该平台让你无缝运行 Docker 容器、Web 应用和 API，支持基于 Git 的部署、原生自动扩展、全球边缘网络以及内置的服务网格和发现功能。查看 [Koyeb 的 Docusaurus 部署指南](https://www.koyeb.com/tutorials/deploy-docusaurus-on-koyeb) 开始使用。

## 部署到 Render {#deploying-to-render}

[Render](https://render.com) 提供 [免费的静态网站托管](https://render.com/docs/static-sites) 服务，包括完全托管的 SSL、自定义域名、全球 CDN 以及从你的 Git 仓库持续自动部署。只需几分钟即可开始，按照 [Render 的 Docusaurus 部署指南](https://render.com/docs/deploy-docusaurus) 操作。

## 部署到 Qovery {#deploying-to-qovery}

[Qovery](https://www.qovery.com) 是一个完全托管的云平台，运行在你的 AWS、Digital Ocean 和 Scaleway 账户上，你可以在一个地方托管静态网站、后端 API、数据库、定时任务和所有其他应用。

1. 创建一个 Qovery 账户。访问 [Qovery 控制台](https://console.qovery.com) 创建一个账户（如果你还没有）。
2. 创建一个项目。
   - 点击 **创建项目** 并给你的项目命名。
   - 点击 **下一步**。
3. 创建一个新环境。
...
4. 添加一个应用。
   - 点击 **创建应用**，命名并选择你的 GitHub 或 GitLab 仓库（其中包含你的 Docusaurus 应用）。
   - 定义主分支名称和应用的根路径。
   - 点击 **创建**。应用创建后：
   - 导航到你的应用 **设置**
   - 选择 **端口**
...
就这样。观察状态并等待应用部署完成。要在浏览器中打开应用，点击 **操作** 和 **打开** 在你的应用概览中。

## 部署到 Hostman {#deploying-to-hostman}

[Hostman](https://hostman.com/) 允许你免费托管静态网站。Hostman 自动化了一切，你只需要连接你的仓库并按照以下简单步骤操作：
...
2. 选择要部署的项目。
   ...
   - 如果您使用GitHub、GitLab或Bitbucket账号登录Hostman，将看到包含您项目的代码仓库（包括私有仓库）。

   - 选择需要部署的项目，该项目必须包含网站文件目录（例如`website`）。

   - 如需访问其他代码仓库，点击**连接其他仓库**。

   - 如果登录时未使用Git账号凭证，此时可以授权访问所需账号，然后选择项目。
...
3. 配置构建设置。
   ...
   - 如需修改构建命令，可输入多条命令并用`&&`分隔。

4. 部署。

   - 点击**部署**开始构建流程。

   - 启动后，将进入部署日志界面。如果代码存在问题，日志中会显示警告或错误信息并指明原因。通常日志包含所有需要的调试数据。

   - 部署完成后，您将收到邮件通知并看到日志条目。大功告成！您的项目已上线运行。

## 部署到Surge {#deploying-to-surge}

Surge是一个[静态网站托管平台](https://surge.sh/help/getting-started-with-surge)，可通过命令行在数秒内部署Docusaurus项目。部署到Surge简单且免费（含自定义域名和SSL证书）。
...
确认要发布的站点位于`build`目录。系统会自动分配随机子域名`*.surge.sh`（可编辑）。
...
未来任何变更均可通过`surge`命令部署。

## 部署到Stormkit {#deploying-to-stormkit}

您可以将Docusaurus项目部署至[Stormkit](https://www.stormkit.io)——一个面向静态网站、单页应用(SPA)和无服务器函数的部署平台。具体操作请参考本[指南](https://www.stormkit.io/blog/how-to-deploy-docusarous)。
...
查看[文档](https://docs.quantcdn.io/docs/cli/continuous-integration)和[博客](https://www.quantcdn.io/blog)获取更多部署到 QuantCDN 的示例和用例。
...
[Azure Static Web Apps](https://docs.microsoft.com/en-us/azure/static-web-apps/overview) 是一项服务，可直接从代码仓库自动构建和部署全栈 Web 应用到 Azure，简化了 CI/CD 的开发者体验。Static Web Apps 将 Web 应用的静态资源与动态（API）端点分离。静态资源通过全球分布的内容服务器提供，使客户端能够使用附近的服务器更快地检索文件。动态 API 通过基于事件驱动的无服务器架构进行扩展，这种方式更具成本效益且可按需扩展。按照[这份分步指南](https://dev.to/azure/11-share-content-with-docusaurus-azure-static-web-apps-30hc)，几分钟即可开始使用。
