...
现在代码翻译会以 Chrome 国际化 JSON 格式存入 `i18n/[locale]/code.json` 文件。
...
MDX 会将 Markdown 编译为 React 组件，其规则比 v1 解析器更严格，遇到错误时会直接终止构建而非渲染错误内容。

此外，必须使用 JSX 元素替代 HTML 元素。

这对国际化尤为重要——若 Crowdin 上的翻译包含无效标记，可能导致 v2 站点构建失败，您可能需要清理翻译内容来修复这些问题。
...
为避免影响生产环境的 v1 站点，可复制原 v1 Crowdin 项目作为迁移策略。
...
首次尝试时很可能会遇到错误：预翻译可能会尝试翻译本不应翻译的内容（front matter、提示框、代码块等），且翻译后的MD文件可能不符合MDX解析器规范。
...
- 存在不良的v1翻译，导致v2中出现无效标记：翻译内容中的错误HTML元素和未闭合标签
...
Crowdin的Markdown解析器会随时间演进，且每个Crowdin项目使用不同的解析器版本，这可能导致预翻译无法处理所有字符串。

该解析器版本未公开，您需要联系Crowdin支持以获取项目使用的解析器版本并固定特定版本。
