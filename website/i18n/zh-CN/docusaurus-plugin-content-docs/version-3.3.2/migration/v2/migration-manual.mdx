...
Docusaurus 2中，我们将每项功能（博客、文档、页面）拆分为插件以实现模块化。预设集是插件的组合包，为保持向后兼容性，我们构建了`@docusaurus/preset-classic`预设集，其中包含Docusaurus 1中的大部分核心插件。
...
我们建议将 `docs` 文件夹移至 `website` 文件夹内，这也是 v2 版本的默认目录结构。[Vercel](https://vercel.com) 支持[开箱即用的 Docusaurus 项目部署](https://github.com/vercel/vercel/tree/main/examples/docusaurus)，前提是 `docs` 目录位于 `website` 内。通常将文档放在网站目录中也更合理，这样文档和网站其他代码可以集中存放在同一个 `website` 目录下。
...
已弃用。我们为 Docusaurus 2 开发了名为 [Infima](https://infima.dev/) 的自定义 CSS 框架，使用 CSS 变量实现主题功能。相关文档尚未完善，待完成后我们将在此更新。要覆盖 Infima 的 CSS 变量，请创建自己的 CSS 文件（如 `./src/css/custom.css`），并通过将其作为选项传递给 `@docusaurus/preset-classic` 来全局导入：
...
建议使用 [ColorBox](https://www.colorbox.io/) 工具为您选择的主色调查找不同色阶。

或者使用以下工具生成网站的不同色阶，并将变量复制到 `src/css/custom.css` 中。
...
网站元信息（如资源、SEO、版权信息）现在由主题处理。要自定义这些内容，请在 `docusaurus.config.js` 中使用 `themeConfig` 字段：
...
在 Docusaurus 1 中，标题图标和标题链接是 `siteConfig` 的根字段：
...
您的 Algolia DocSearch v1 配置（可在[此处](https://github.com/algolia/docsearch-configs/blob/master/configs)找到）需要针对 Docusaurus v2 进行更新（参见[示例](https://github.com/algolia/docsearch-configs/tree/master/configs/docusaurus-2.json)）。

您可以联系 DocSearch 团队 (@shortcuts, @s-pace) 获取支持。他们可以为您更新配置并触发站点重新爬取以恢复搜索功能（否则您需要等待最多 24 小时直到下次计划爬取）
...
已弃用。请在 `static` 文件夹中创建 `CNAME` 文件并填写您的自定义域名。执行构建命令时，`static` 文件夹中的文件将被复制到 `build` 文件夹的根目录下。
...
- `defaultVersionShown` - 版本功能尚未移植。若您使用版本功能，目前无法迁移至Docusaurus 2。请持续关注。
- `disableHeaderTitle`
- `disableTitleTagline`
...
- `facebookPixelId`
- `fonts`
- `highlight` - 现使用 [Prism](https://prismjs.com/) 替代 [highlight.js](https://highlightjs.org/)。
...
- `manifest`
- `onPageNav` - 现已默认开启。
- `separateCss` - 可通过与上文所述 `custom.css` 相同的方式导入。
- `scrollToTop`
- `scrollToTopOptions`
...
- `useEnglishUrl`
- `users`
- `usePrism` - 现使用 [Prism](https://prismjs.com/) 替代 [highlight.js](https://highlightjs.org/)。
- `wrapPagesHTML`
...
若您在v1中设置了 `cleanUrl: false`，可能存在用户发布过指向 `/installation.html` 的链接。

出于SEO考虑及避免链接失效，应在托管服务商处配置服务器端重定向规则。
...
旧版本中不允许嵌套侧边栏分类且分类项仅支持文档ID。而v2版本支持无限嵌套侧边栏，并提供多种[侧边栏项类型](../../guides/docs/sidebar/items.mdx)（不仅限于文档）。

若现有侧边栏包含分类结构需进行迁移：将`subcategory`重命名为`category`，`ids`改为`items`。
...
该操作会将当前主题使用的`<Footer />`组件复制到站点根目录下的`src/theme/Footer`目录中，您可在此自定义组件。

不要仅为在左侧添加logo而swizzle页脚组件。v2版本已特意移除该设计并将logo移至底部。直接在`docusaurus.config.js`中通过`themeConfig.footer`配置即可：
...
Docusaurus v2中改为通过`useDocusaurusContext`钩子获取该对象。

v2版本中需在每个页面外层应用主题布局组件。Layout组件接收元数据props。

v2弃用了`CompLibrary`，您需要自行编写React组件或使用Infima样式（相关文档即将发布，目前可查看V2官网或访问https://infima.dev/了解可用样式）。

可将CommonJS模块语法迁移至ES6的import/export语法。

以下是典型的Docusaurus v2页面结构：
...
Docusaurus 2采用[MDX](https://mdxjs.com/)语法，需更新现有文档中不兼容的语法。常见如HTML中的自闭合标签`<img>`和`<br>`，现在需显式闭合为`<img/>`和`<br/>`。MDX文档中所有标签必须符合JSX规范。
...
字段 `authorFBID` 和 `authorTwitter` 已被弃用。它们仅用于生成作者头像，现可通过 `authors` 字段实现相同功能。

## 部署 {#deployment}

GitHub Pages 使用的 `CNAME` 文件不再自动生成，若使用自定义域名请确保已在 `/static/CNAME` 路径下创建该文件。
