...
以下是通过 Git 将新初始化的英文 Docusaurus 网站翻译为法文的完整流程，假设您已遵循[国际化教程](./i18n-tutorial.mdx)。
...
`Hello` 将显示为 `(fr) Hello`，明确标识需要翻译的内容。
...
保持翻译文件与原始文件**同步具有挑战性**，特别是 Markdown 文档。

### Markdown 翻译 {#markdown-translations}

当未翻译的 Markdown 文档被修改时，**维护对应翻译文件是您的责任**，目前我们尚未提供有效的辅助方案。
...
您的翻译内容托管在 Git 上，可以使用文档和博客插件的 `editLocalizedFiles: true` 选项。

编辑按钮将直接链接到本地化文档 `i18n/fr/docusaurus-plugin-content-docs/current/doc1.md`。