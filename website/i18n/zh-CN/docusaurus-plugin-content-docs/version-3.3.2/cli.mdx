...
项目初始化后，网站源码中将包含可通过包管理器调用的 Docusaurus 脚本：
...
使用 [Webpack Dev Server](https://webpack.js.org/configuration/dev-server) 在本地构建并运行网站预览。
...
请注意部分功能（例如锚点链接）在开发模式下不可用，这些功能在生产环境中会正常运作。
...
有多种获取证书的方式，此处以 [mkcert](https://github.com/FiloSottile/mkcert) 为例：

1. 运行 `mkcert localhost` 生成 `localhost.pem` 和 `localhost-key.pem`

2. 运行 `mkcert -install` 将证书安装到信任库，并重启浏览器
...
对于 CSS 包的高级压缩，我们使用 [cssnano 高级预设](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-advanced)（配合多个 PostCSS 插件）和 [clean-css 二级优化](https://github.com/jakubpawlowicz/clean-css#level-2-optimizations)。若此高级 CSS 压缩导致样式异常，可通过环境变量 `USE_SIMPLE_CSS_MINIFIER=true` 改用 [cssnano 默认预设](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-default) 进行压缩。**如遇 CSS 压缩问题请[提交 issue](https://github.com/facebook/docusaurus/issues/new?labels=bug%2C+needs+triage&template=bug.md)**。
...
该交互式 CLI 将全程引导您完成[置换流程](./swizzling.mdx)。
...
将您的站点部署到 [GitHub Pages](https://pages.github.com/)。查看 [部署文档](deployment.mdx#deploying-to-github-pages) 获取更多细节。
...
我们建议在报告错误、升级版本后或遇到 Docusaurus 站点问题时运行此命令。
