...
若通过`displayed_sidebar`强制显示某个侧边栏，且该侧边栏不包含当前文档，则不会显示分页。
...
"windows"页面的分页下一篇指向"linux"并不合理：您可能希望读者安装后跳转至"getting started"。此时可手动设置分页：
...
当您需要从多个侧边栏链接到同一文档时，这个特性尤为实用。文档实际仅归属于一个侧边栏（通过 `type: 'doc'` 注册或通过自动生成目录），但其链接会出现在所有注册过的侧边栏中。
