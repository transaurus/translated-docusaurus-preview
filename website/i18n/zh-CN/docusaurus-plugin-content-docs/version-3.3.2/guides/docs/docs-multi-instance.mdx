...
如果您开发跨平台移动端SDK，可能需维护两套文档：

- Android SDK文档（`v1.0`、`v1.1`）
- iOS SDK文档（`v1.0`、`v2.0`）

此时可为每个移动端SDK文档集使用独立的插件实例。

:::warning

若每个文档集规模庞大，建议创建两个独立的Docusaurus站点。
...
每个与文档相关的[主题导航栏项目](../../api/themes/theme-configuration.mdx#navbar)都可选地接受一个`docsPluginId`属性。
