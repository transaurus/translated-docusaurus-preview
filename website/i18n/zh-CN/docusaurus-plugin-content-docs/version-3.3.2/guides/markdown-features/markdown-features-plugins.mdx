...
如果您使用过 [MDX 交互演示](https://mdxjs.com/playground/)，会注意到 MDX 转换有两个中间步骤：Markdown 抽象语法树（MDAST）和超文本抽象语法树（HAST），最终才输出 JSX。MDX 插件也分为两种形式：
...
使用插件为项目中最常用的 JSX 元素引入简写语法。我们提供的[告示语法](./markdown-features-admonitions.mdx)也是通过 Remark 插件生成的，您可以为自己的用例实现类似功能。
...
许多官方 Remark/Rehype 插件仅支持 [ES Modules](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)（JavaScript 模块系统），Docusaurus 对此提供了支持。我们建议使用 [ES Modules](https://flaviocopes.com/es-modules/) 配置文件以便更轻松地导入这类包。
...
某些插件支持配置并接受自定义选项。这种情况下，请使用 `[plugin, pluginOptions]` 语法，如下所示：
...
以下内容并非插件开发的完整指南，仅用于说明如何在Docusaurus中实现插件功能。如需深入了解插件工作机制，请参阅[Remark](https://github.com/remarkjs/remark/blob/main/doc/plugins.md#create-plugins)或[Rehype](https://github.com/rehypejs/rehype/blob/main/doc/plugins.md#create-plugins)官方文档。
...
Docusaurus默认插件会优先于自定义remark插件执行，这意味着图片/链接在被自定义插件处理前已被转换为带`require()`调用的JSX。例如上例中，虽然所有二级标题都添加了"第X节"前缀，但生成的目录仍保持原样，因为目录生成插件先于自定义插件执行。若需在默认插件前处理MDAST，请使用`beforeDefaultRemarkPlugins`和`beforeDefaultRehypePlugins`：
