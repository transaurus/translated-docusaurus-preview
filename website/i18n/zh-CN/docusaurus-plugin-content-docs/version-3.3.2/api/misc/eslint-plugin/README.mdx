...
[ESLint](https://eslint.org/) 是一款静态代码分析工具，通过编辑器提示和命令行报告问题或建议最佳实践。Docusaurus 提供了一个 ESLint 插件来强制执行 Docusaurus 的最佳实践。
...
如需更细粒度的控制，你也可以手动启用插件并直接配置想要使用的规则：
...
- 全部规则：启用**所有**规则。这会随着次要版本更新而变化，因此如果你希望避免意外的破坏性变更，不应使用此配置。
