...
如需在网站发布公告，可添加公告栏。这是位于导航栏上方非固定且可关闭的面板，所有配置均在`announcementBar`对象中。
...
标志可放置在[静态文件夹](static-assets.mdx)中。标志URL默认设为站点基础URL。虽然可指定自定义URL，但外部链接将在新标签页打开。此外，可覆盖标志链接的target属性值，这在文档网站作为主站子目录托管时特别有用（此时可能不需要标志链接在主站新标签页打开）。
...
React Router 通常会自动为链接应用活动链接样式，但在边缘情况下可以使用 `activeBasePath`。若某个链接需要在多个不同路径下保持活动状态（例如同一侧边栏下存在多个文档目录），可使用 `activeBaseRegex`。该参数是比 `activeBasePath` 更灵活的替代方案，且优先级更高——Docusaurus 会将其解析为正则表达式并与当前 URL 进行匹配测试。
...
需注意下拉菜单基项本身也是可点击链接，因此可接收[普通导航栏链接](#navbar-link)的所有参数。
...
无需硬编码文档 ID，即可将导航栏项目链接至指定侧边栏的第一个文档链接（可以是文档链接或生成的分类索引）。
...
用户可在保持当前文档不变的情况下切换版本（要求文档 ID 在各版本间保持一致）。
...
若使用版本化文档，此特殊导航栏项目类型会链接至当前浏览的文档版本（基于 URL），并默认回退至最新版本。
...
您也可以通过此导航栏项类型，在导航栏项中渲染自定义HTML标记。
...
默认使用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)作为语法高亮主题。您可以从[可用主题列表](https://github.com/FormidableLabs/prism-react-renderer/tree/master/packages/prism-react-renderer/src/themes)选择自定义主题，还可为暗色模式站点指定不同的语法高亮主题。
...
可为代码块设置默认语言（当开头的三个反引号后未指定语言时生效）。注意必须传入有效的[语言名称](https://prismjs.com/#supported-languages)。
...
可通过`themeConfig.footer`添加页脚logo和版权信息。logo文件可放置在[静态资源目录](static-assets.mdx)中，其URL处理方式与导航栏logo相同。
...
用于访问色彩模式的React钩子。该上下文包含设置浅色/深色模式的函数，并暴露当前使用模式的布尔变量。
