...
此插件在开发环境中始终处于未激活状态，**仅在生产环境生效**，因其功能基于构建输出实现。
...
本插件同时会读取[`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes)配置项，用于在多个文件生成至相同路径时调整日志级别。
...
"from"与"to"的概念是本插件的核心。"from"表示您希望创建的路径（即待生成的额外HTML文件）；"to"表示需要重定向的目标路径（通常是Docusaurus已识别的路由）。

因此可以存在多个"from"指向同一个"to"（生成多个重定向至同一目标的HTML文件），但一个"from"绝不能对应多个"to"（生成的HTML文件必须具有确定的重定向目标）。
