...
用于生成近乎理想图像（响应式、懒加载、低质量占位图）的Docusaurus插件。

:::info

默认情况下，该插件在开发环境中处于**非激活状态**，以便您始终可以查看完整尺寸的图像。如需调试理想图像行为，可将[`disableInDev`](#disableInDev)选项设为`false`。
...
- 修改前：`string`
- 修改后：`{preSrc: string, src: import("@theme/IdealImage").SrcImage}`
