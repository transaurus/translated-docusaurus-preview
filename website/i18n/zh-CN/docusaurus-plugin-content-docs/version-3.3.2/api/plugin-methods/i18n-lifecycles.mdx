...
- `path`：相对于插件本地化文件夹 `i18n/[locale]/[pluginName]` 的路径。省略 `.json` 扩展名以保持通用性。
- `content`：使用Chrome国际化JSON格式。
...
它应返回 `Record<string, string>` 格式的消息，其中键是翻译ID，值是使用站点当前语言环境本地化的消息（不包含描述）。
