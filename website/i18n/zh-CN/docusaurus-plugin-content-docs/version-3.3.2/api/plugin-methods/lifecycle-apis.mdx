...
插件应使用此生命周期从数据源（文件系统、远程API、无头CMS等）获取数据或进行一些服务器端处理。返回值即为其所需的内容。

例如，下面这个插件返回一个1到10之间的随机整数作为内容。
...
`contentLoaded`会在`loadContent`完成后调用。`loadContent()`的返回值将作为`content`传递给`contentLoaded`。
...
例如，下面这个插件创建了一个显示`你的朋友是：Yangshun, Sebastien`的`/friends`页面：
...
`configureWebpack` 在服务端构建和客户端构建时都会被调用。服务端构建接收 `true` 作为 `isServer` 参数，客户端构建接收 `false`。
...
- `preBodyTags` 将在 `<body>` 标签打开后插入，位于所有子元素之前。
