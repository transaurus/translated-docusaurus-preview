...
`options`是[插件使用时的第二个可选参数](../../using-plugins.mdx#configuring-plugins)。这些选项是插件特定的，由用户在`docusaurus.config.js`中配置时指定。如果导出了[`validateOptions`](./static-methods.mdx#validateOptions)函数，这些选项会先经过验证和规范化处理。
