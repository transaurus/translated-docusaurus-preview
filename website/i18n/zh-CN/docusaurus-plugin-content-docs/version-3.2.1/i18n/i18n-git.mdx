...
- **对专业译者困难**：他们习惯使用 SaaS 翻译软件和高级功能
- **维护困难**：必须保持翻译文件与未翻译文件**同步**

:::note

部分**大型技术项目**（React、Vue.js、MDN、TypeScript、Nuxt.js 等）采用 Git 进行翻译。

参考 [Docusaurus 国际化 RFC](https://github.com/facebook/docusaurus/issues/3317) 了解我们研究这些系统的笔记和链接。
...
保持翻译文件与原始文件**同步可能具有挑战性**，特别是 Markdown 文档。

### Markdown 翻译 {#markdown-translations}

当未翻译的 Markdown 文档被编辑时，**维护相应翻译文件是您的责任**，我们目前没有有效的方法来协助此过程。

为了保持翻译站点的一致性，当`website/docs/doc1.md`文档被编辑时，您需要**将这些修改反向移植**到`i18n/fr/docusaurus-plugin-content-docs/current/doc1.md`中。

### JSON翻译维护 {#json-translations}

为帮助维护JSON翻译文件，您可以重新运行[write-translations](../cli.mdx#docusaurus-write-translations-sitedir) CLI命令：
...
您的翻译内容存放在Git中，可以使用文档和博客插件的`editLocalizedFiles: true`选项。
