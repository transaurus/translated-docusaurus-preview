...
不过您通常不需要逐个安装常见插件：它们可以以[预设包](#using-presets)的形式分发。对于大多数用户，插件是通过预设配置进行管理的。

我们维护了[官方插件列表](./api/plugins/overview.mdx)，社区也开发了一些[非官方插件](/community/resources#community-plugins)。如需添加任何功能（自动生成文档页面、执行自定义脚本、集成第三方服务等），请务必查阅这些列表：可能已有现成实现！

如果您精力充沛，也可以阅读[插件开发指南](./advanced/plugins.mdx)或[插件方法参考文档](./api/plugin-methods/README.mdx)来自行开发插件。

## 安装插件 {#installing-a-plugin}

插件通常是 npm 包，因此可以通过 npm 像安装其他包一样进行安装。
...
但插件支持通过"Babel风格"的二元组配置来指定选项——即在配置中用数组包裹插件名称和选项对象。
...
所有 Docusaurus 内容插件都支持多实例运行。例如，可以配置[多个文档插件实例](./guides/docs/docs-multi-instance.mdx)或[多个博客](./blog.mdx#multiple-blogs)。每个插件实例必须分配唯一ID，默认插件ID为`default`。
...
主题的加载方式与插件完全相同。从使用者角度看，在安装和配置时，`themes`和`plugins`配置项可以互换使用。唯一区别在于主题会在插件之后加载，且[主题可以覆盖插件的默认主题组件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes`和`plugins`选项会导致不同的[简写解析规则](#module-shorthands)，如需使用简写形式，请确保使用正确的配置项！
...
预设（Presets）是插件和主题的集合包。例如，与其让您在配置文件中逐个注册和配置 `@docusaurus/plugin-content-docs`、`@docusaurus/plugin-content-blog` 等插件，我们提供了 `@docusaurus/preset-classic` 预设，允许您在一个集中的位置配置它们。

### `@docusaurus/preset-classic` {#docusauruspreset-classic}

经典预设默认随新创建的 Docusaurus 网站（通过 [`create-docusaurus`](./installation.mdx#scaffold-project-website) 创建）一起提供。它包含以下主题和插件：
...
预设通常是一个 npm 包，因此您可以使用 npm 像安装其他 npm 包一样安装它们。
...
预设是一个与[插件构造函数](./api/plugin-methods/README.mdx#plugin-constructor)具有相同形状的函数。它应该返回一个 `{ plugins: PluginConfig[], themes: PluginConfig[] }` 对象，与站点配置中接受的形式相同。例如，您可以指定一个包含以下主题和插件的预设：
...
Docusaurus 支持插件、主题和预设的简写形式。当它看到一个插件/主题/预设名称时，它会尝试按以下顺序加载：
...
如果名称是有作用域的（以 `@` 开头），名称会首先通过第一个斜杠分割为作用域和包名：
...
如果名称不包含具体包名（例如`@jquery`），则会加载`[scope]/docusaurus-[moduleType]`（即`@jquery/docusaurus-plugin`）。否则将尝试以下解析方式：

- `[scope]/[name]`（例如`@jquery/content-docs`）
- `[scope]/docusaurus-[moduleType]-[name]`（例如`@jquery/docusaurus-plugin-content-docs`）

以下是在`plugins`字段中注册插件时的示例。请注意，与[ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins)或[Babel](https://babeljs.io/docs/en/options#name-normalization)强制要求插件命名规范不同，Docusaurus允许更自由的命名方式，因此解析结果并非绝对，但会遵循上述优先级规则。
