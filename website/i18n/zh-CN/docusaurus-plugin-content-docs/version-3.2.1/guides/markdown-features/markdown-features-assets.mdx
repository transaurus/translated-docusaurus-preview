...
同理，您可以通过 `require` 引入现有资源文件，并将返回的 URL 用于 `video` 视频标签、`a` 锚点链接等场景。
...
使用 Markdown 图片或链接语法时，所有资源路径都会被 Docusaurus 作为文件路径解析并自动转换为 `require()` 调用。除非使用 JSX 语法（此时需自行处理），否则无需在 Markdown 中显式使用 `require()`。

:::

## 内联 SVG {#inline-svgs}

Docusaurus 原生支持内联 SVG 图像。
...
当需要通过 CSS 修改 SVG 局部样式时（例如根据当前主题切换 SVG 颜色），此功能非常实用。
...
Docusaurus 支持主题化图片：内置的 `ThemedImage` 组件（包含在主题中）允许根据当前主题切换图片源。
...
要通过路径片段控制图片显隐（GitHub 使用 `#gh-dark-mode-only` 和 `#gh-light-mode-only`），请在自定义 CSS 中添加以下代码（若不希望与 GitHub 耦合，也可使用自定义后缀）：
...
若 Markdown 链接或图片使用绝对路径，该路径将被视为文件路径并从静态资源目录解析。例如配置了[静态目录](../../static-assets.mdx)为 `['public', 'static']` 时，对于以下图片：

```md title="my-doc.md"
![An image from the static](/img/docusaurus.png)
```

Docusaurus 会依次在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 路径查找。该链接最终会被转换为 `require()` 调用而非保留为 URL，这种设计在以下两方面具有优势：

1. 您无需关心基础URL，Docusaurus会在提供资源时自动处理；
2. 图片会进入Webpack的构建流程，其文件名会被附加哈希值，这使得浏览器能积极缓存图片并提升网站性能。

如需直接使用URL链接，可通过`pathname://`协议禁用自动资源链接处理。
