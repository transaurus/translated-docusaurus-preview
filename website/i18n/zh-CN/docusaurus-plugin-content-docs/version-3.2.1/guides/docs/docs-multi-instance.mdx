...
此特性仅对[版本化文档](./versioning.mdx)有用。建议在阅读本页前先熟悉文档版本化功能。若仅需[多侧边栏](./sidebar/multiple-sidebars.mdx)，可在单个插件内实现。

:::

## 使用场景 {#use-cases}

有时您可能希望在一个Docusaurus站点中托管两套（或更多）独立的文档集。
...
当有人编辑iOS文档时，重新构建包括未变更的Android文档在内的全部内容是否真的必要？
...
例如，若需要为每个移动端SDK（iOS和Android）单独配置版本下拉菜单，可采用如下方式：
