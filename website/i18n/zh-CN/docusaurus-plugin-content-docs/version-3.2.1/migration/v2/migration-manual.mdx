...
在Docusaurus 1中，所有构建产物位于`website/build/<项目名称>`目录。

Docusaurus 2将其简化为`website/build`目录。请确保更新部署配置以从正确的构建目录读取生成文件。
...
D1网站可能已有README文件。可修改该文件反映D2变更，或复制默认的Docusaurus v2版README模板。
...
Docusaurus 2将各项功能（博客、文档、页面）拆分为插件以实现模块化。预设集是插件的组合包，为保持向后兼容性，我们构建了`@docusaurus/preset-classic`预设集，其中包含Docusaurus 1中的大部分核心插件。
...
已弃用。我们为 Docusaurus 2 开发了名为 [Infima](https://infima.dev/) 的自定义 CSS 框架，使用 CSS 变量实现主题化。相关文档尚未完善，后续会更新说明。要覆盖 Infima 的 CSS 变量，请创建自定义 CSS 文件（如 `./src/css/custom.css`）并通过 `@docusaurus/preset-classic` 的选项全局引入：
...
也可使用下方工具生成网站所需的色阶变量，并复制到 `src/css/custom.css` 文件中：
...
网站元信息（如图标资源、SEO、版权信息）现由主题处理。如需自定义，请在 `docusaurus.config.js` 的 `themeConfig` 字段中配置：
...
在 Docusaurus 1 中，顶部图标和链接是 `siteConfig` 的根字段：
...
可联系 DocSearch 团队 (@shortcuts, @s-pace) 获取支持。他们可协助更新配置并触发站点重新爬取以恢复搜索功能（否则需等待最多 24 小时的下次计划爬取）
...
- `defaultVersionShown` - 版本功能尚未移植。若您使用版本功能，暂时无法迁移至Docusaurus 2。请保持关注。
- `disableHeaderTitle`
- `disableTitleTagline`
...
- `markdownOptions` - v2改用MDX而非Remarkable，需将Markdown选项转换为Remark/Rehype插件。
- `markdownPlugins` - v2改用MDX而非Remarkable，需将Markdown插件转换为Remark/Rehype插件。
- `manifest`
- `onPageNav` - 现已默认开启。
...
在v1中，所有页面均可通过带或不带 `.html` 后缀的URL访问。
...
出于SEO和避免链接失效的考虑，应在托管服务商处配置服务端重定向规则。
...
不再需要`website/core/Footer.js`文件。如需修改Docusaurus默认页脚，可通过[swizzle](../../swizzling.mdx)实现：

```bash npm2yarn
npm run swizzle @docusaurus/theme-classic Footer
```

此操作会将当前主题使用的`<Footer />`组件复制到站点根目录下的`src/theme/Footer`文件夹中，随后可对该组件进行自定义编辑。

请勿仅为在左侧添加Logo而swizzle页脚组件。v2版本已刻意移除该设计并将Logo移至底部。直接在`docusaurus.config.js`中通过`themeConfig.footer`配置即可：
...
请参阅[创建页面](guides/creating-pages.mdx)了解Docusaurus 2的页面机制。特别注意需将v1版本中的`pages/en`文件迁移至`src/pages`目录。
...
v2已弃用`CompLibrary`，需自行编写React组件或使用Infima样式（相关文档即将发布，目前可查看V2官网或访问https://infima.dev/ 了解可用样式）。
