...
相比之下，Docusaurus 2的`website/versioned_docs/version-1.0.0/hello.md`文件内容如下（与原文件完全相同）
...
在v2中，我们采用快照机制实现文档版本控制。**每个版本的文档不依赖其他版本**。可能出现`foo.md`存在于`version-1.0.0`但不存在于`version-1.2.0`的情况。由于Docusaurus v1的回退功能(https://v1.docusaurus.io/docs/en/versioning#fallback-functionality)，这在之前版本中是不可能的。

例如，如果v1中的`versions.json`如下所示

```json title="versions.json"
["1.1.0", "1.0.0"]
```

Docusaurus v1仅在文档内容发生变化时才会创建版本化文档。如果从v1.0.0到v1.1.0只有`hello.md`发生变更，您的文档结构可能如下所示。
