...
项目初始化完成后，网站源码目录中将包含可通过包管理器调用的 Docusaurus 脚本：
...
## Docusaurus CLI 命令列表 {#docusaurus-cli-commands}

以下是 Docusaurus CLI 命令及其用途说明：
...
当需要从远程服务器或虚拟机（如 GitHub Codespaces）转发 3000 端口时，可通过指定 `0.0.0.0` 让开发服务器监听本地 IP 地址。
...
2. 执行 `mkcert -install` 将证书安装到信任存储区，并重启浏览器
...
对于 CSS 包的高级压缩，我们使用 [cssnano 高级预设](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-advanced)（配合多个 PostCSS 插件）和 [clean-css 二级优化](https://github.com/jakubpawlowicz/clean-css#level-2-optimizations)。若发现由此导致 CSS 异常，可通过设置环境变量 `USE_SIMPLE_CSS_MINIFIER=true` 改用 [cssnano 默认预设](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-default) 进行压缩。**如遇 CSS 压缩问题请[提交 issue](https://github.com/facebook/docusaurus/issues/new?labels=bug%2C+needs+triage&template=bug.md)**。

设置环境变量 `SKIP_HTML_MINIFICATION=true` 可跳过 HTML 压缩。
...
我们建议在报告问题、升级版本后或遇到 Docusaurus 站点异常时运行此命令。

### `docusaurus write-translations [siteDir]` {#docusaurus-write-translations-sitedir}

生成待翻译的 JSON 翻译文件。

默认情况下，文件将写入 `website/i18n/<defaultLocale>/...` 目录。
