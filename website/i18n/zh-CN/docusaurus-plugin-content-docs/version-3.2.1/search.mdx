...
DocSearch每周会爬取您的网站（可通过网页界面配置周期），将所有内容聚合到Algolia索引中。这些内容随后通过Algolia API直接从您的前端进行查询。

如果您的网站[不符合](https://docsearch.algolia.com/docs/who-can-apply)免费托管版DocSearch的准入条件，或是网站部署在防火墙后非公开访问，您可以[自行运行](https://docsearch.algolia.com/docs/run-your-own/)DocSearch爬虫。
...
当您的申请通过并部署后，将会收到包含所有配置详情的邮件。您可以通过[网页界面](https://crawler.algolia.com/)编辑和管理爬取任务。索引在部署后立即可用，通常无需手动配置。

:::danger[使用推荐的爬虫配置]

强烈建议使用我们官方的[**Docusaurus v3爬虫配置模板**](https://docsearch.algolia.com/docs/templates/#docusaurus-v3-template)。若选择其他爬虫配置，我们将无法提供支持。
...
若更新了`initialIndexSettings`配置，虽然可以通过界面手动更新索引，但[Algolia团队建议删除原有索引后重新爬取](https://github.com/facebook/docusaurus/issues/9200#issuecomment-1667338492)，以完全使用新设置重新初始化。

:::

### 连接Algolia {#connecting-algolia}

Docusaurus自带的`@docusaurus/preset-classic`已支持Algolia DocSearch集成。若使用经典预设，则无需额外安装。
...
在 Algolia 完成站点爬取前，搜索功能可能无法稳定运行。

若重大更新后搜索失效，请通过 Algolia 控制台**手动触发重新爬取**。
...
假设您维护两个文档版本（**v1** 和 **v2**）及两种语言（`en` 和 `fr`）。

当浏览 v2 文档时，展示 v1 版本的搜索结果显然不合逻辑。某些情况下 v1 与 v2 内容高度相似，这将导致同一查询返回重复结果（每个版本各显示一次）。

同理，在法语站点中展示英文文档结果也非最佳体验。

上下文搜索功能通过动态创建查询过滤器，自动识别当前浏览的文档版本与语言环境。
...
如需特殊配置，可关闭 `contextualSearch` 并自定义 `facetFilters`：
...
若关闭上下文搜索才能获取结果，通常表明存在[索引配置问题](#algolia-no-search-results)。

:::

### 定制 Algolia 搜索样式 {#styling-your-algolia-search}

DocSearch 默认提供符合无障碍标准的主题，所有颜色对比度均通过标准检测。
...
如需直接修改 Algolia 搜索的 React 组件，可通过[置换功能](swizzling.mdx)处理 `@docusaurus/theme-search-algolia` 中的 `SearchBar` 组件：
...
以下是Docusaurus用户在使用Algolia DocSearch时最常遇到的问题。
...
Algolia DocSearch团队可协助排查您站点的搜索问题。
...
[Typesense](https://typesense.org) DocSearch的工作方式与Algolia DocSearch类似，区别在于您的网站内容会被索引到Typesense搜索集群中。
...
- [typesense-docsearch-scraper](https://github.com/typesense/typesense-docsearch-scraper) - 用于抓取网站内容并将数据索引到Typesense集群
- [docusaurus-theme-search-typesense](https://github.com/typesense/docusaurus-theme-search-typesense) - 可集成到网站的搜索栏UI组件

请参阅[运行typesense-docsearch-scraper的逐步指南](https://typesense.org/docs/latest/guide/docsearch.html#step-1-set-up-docsearch-scraper)以及[在Docusaurus站点安装搜索栏的教程](https://typesense.org/docs/latest/guide/docsearch.html#option-a-docusaurus-powered-sites)。

## 👥 使用本地搜索 {#using-local-search}

对于搜索索引较小且可被用户浏览器下载的网站，可使用本地搜索插件方案。
...
这将在项目文件夹中创建`src/theme/SearchBar`文件。重启开发服务器并编辑该组件后，Docusaurus将开始使用您的自定义`SearchBar`组件。
