...
此插件在开发环境下始终处于非活跃状态，**仅在生产环境生效**，因其功能基于构建输出实现。
...
"from"与"to"的概念是本插件的核心。"from"表示您希望创建的路径（即待写入的额外HTML文件）；"to"表示您希望重定向至的路径（通常是Docusaurus已识别的路由）。

因此可以存在多个"from"指向同一个"to"：我们将创建多个HTML文件均重定向至同一目标。但一个"from"绝不能对应多个"to"：写入的HTML文件必须具有确定的重定向目标。
