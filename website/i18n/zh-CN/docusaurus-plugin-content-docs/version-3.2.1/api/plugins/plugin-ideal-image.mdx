...
默认情况下，该插件在开发环境中处于**非活动状态**，以便您始终可以查看完整尺寸的图像。如需调试理想图像行为，可将[`disableInDev`](#disableInDev)选项设为`false`。
...
本插件会注册一个[Webpack加载器](https://webpack.js.org/loaders/)，改变导入/require图像的类型：
