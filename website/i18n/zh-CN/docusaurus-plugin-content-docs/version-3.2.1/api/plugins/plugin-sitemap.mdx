...
该插件为您的站点生成站点地图，使搜索引擎爬虫能更精准地抓取网站内容。

:::warning[仅生产环境]

此插件在开发环境中始终处于非激活状态，**仅在生产环境生效**（作用于构建输出阶段）。
...
`lastmod`选项仅在插件提供[路由元数据](../plugin-methods/lifecycle-apis.mdx#addRoute)属性`sourceFilePath`或`lastUpdatedAt`时输出站点地图的`<lastmod>`标签。
