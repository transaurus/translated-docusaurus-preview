...
`configureWebpack` 会在服务端构建和客户端构建时被调用。服务端构建传入 `true`，客户端构建传入 `false` 作为 `isServer` 参数。
...
`injectHtmlTags` 会被调用，同时传入插件加载的内容。
...
- `headTags` 将在 `</head>` 关闭标签之前插入，位于配置添加的脚本之后。
- `preBodyTags` 将在 `<body>` 开始标签之后插入，位于任何子元素之前。
- `postBodyTags` 将在 `</body>` 关闭标签之前插入，位于所有子元素之后。

## `getClientModules()` {#getClientModules}

返回要导入客户端包的[客户端模块](../../advanced/client.mdx#client-modules)路径数组。

例如，为了让您的主题加载用户通过 `options` 传入的 `customCss` 或 `customJs` 文件路径：
