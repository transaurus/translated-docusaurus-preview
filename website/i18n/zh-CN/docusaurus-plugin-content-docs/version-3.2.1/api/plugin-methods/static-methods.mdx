...
静态方法不属于插件实例——它们被附加到构造函数上。这些方法用于验证和规范化插件选项及主题配置，随后这些经过处理的参数将作为构造函数参数来初始化插件实例。

## `validateOptions({options, validate})` {#validateOptions}

返回经过验证和规范化的插件选项。此方法在插件初始化前被调用。必须返回选项对象，因为它们将在初始化时传递给插件。

### `options` {#options}

`validateOptions` 方法接收插件传入的 `options` 参数进行验证和规范化处理。
