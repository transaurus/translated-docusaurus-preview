...
以下是通过 Git 将新初始化的英文 Docusaurus 网站翻译为法文的步骤指南，假设您已遵循[国际化教程](./i18n-tutorial.mdx)。
...
当未翻译的 Markdown 文档被编辑时，**维护相应翻译文件是您的责任**，遗憾的是我们目前无法提供有效协助。

为确保翻译站点的一致性，当 `website/docs/doc1.md` 文档被编辑时，您需要**将这些修改反向移植**到 `i18n/fr/docusaurus-plugin-content-docs/current/doc1.md` 中。
...
若翻译内容托管在 Git 仓库，可通过 docs 和 blog 插件的 `editLocalizedFiles: true` 配置项。

编辑按钮将指向本地化文档路径 `i18n/fr/docusaurus-plugin-content-docs/current/doc1.md`。