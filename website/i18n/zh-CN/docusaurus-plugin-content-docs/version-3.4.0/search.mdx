...
DocSearch 每周会爬取您的网站（可通过网页界面调整频率），将所有内容聚合到 Algolia 索引中。这些内容随后通过 Algolia API 直接从您的前端进行查询。

如果您的网站[不符合](https://docsearch.algolia.com/docs/who-can-apply)免费托管版 DocSearch 的准入条件，或是部署在防火墙后的非公开网站，您可以[自行运行](https://docsearch.algolia.com/docs/run-your-own/) DocSearch 爬虫程序。
...
您可以通过[博客文章](/blog/2021/11/21/algolia-docsearch-migration)或[迁移文档](https://docsearch.algolia.com/docs/migrating-from-legacy)了解从旧版基础设施迁移的更多信息。

:::

### 索引配置 {#algolia-index-configuration}

申请通过并部署后，您将收到包含配置详情的邮件。通过[网页界面](https://crawler.algolia.com/)即可编辑和管理爬取任务。索引在部署后立即可用，通常无需手动配置。
...
配置中的`initialIndexSettings`仅用于初始化不存在的 Algolia 索引。

若更新了爬虫配置中的`initialIndexSettings`，虽然可以通过界面手动更新索引，但[Algolia 团队建议删除索引后重新爬取](https://github.com/facebook/docusaurus/issues/9200#issuecomment-1667338492)，以完全使用新设置重新初始化。

:::

### 连接 Algolia {#connecting-algolia}

Docusaurus 自带的`@docusaurus/preset-classic`已支持 Algolia DocSearch 集成。若使用经典预设，无需额外安装。
...
同理，浏览法语站点时展示英语文档结果也不符合预期。

上下文搜索功能通过动态创建搜索查询过滤器，确保结果始终匹配当前浏览的文档版本和语言。

- 访问 `/en/docs/v1/myDoc` 时，搜索结果仅包含 **v1** 版本的**英语**文档（及未版本化的页面）
- 访问 `/fr/docs/v2/myDoc` 时，搜索结果仅包含 **v2** 版本的**法语**文档（及未版本化的页面）
...
要使用自定义搜索，请对`@docusaurus/theme-classic`中的`SearchBar`组件进行 swizzle 操作
...
**注意**：您也可以[从 Algolia SearchBar 进行 swizzle](#editing-the-algolia-search-component)，并基于此创建自己的搜索组件。