...
[Vercel Analytics](https://vercel.com/docs/analytics) 提供全面的网站访问者洞察，可追踪热门页面、流量来源以及地理位置、操作系统和浏览器信息等用户画像数据。

:::warning[仅生产环境]

该插件在开发环境中始终处于非激活状态，**仅在生产环境** (`docusaurus build`) 下生效，以避免污染分析统计数据。
