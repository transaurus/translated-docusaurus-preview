...
如果您仍在使用带有`UA-*`跟踪ID的该插件，应尽快创建Google Analytics 4账户，并使用[`@docusaurus/plugin-google-gtag`](./plugin-google-gtag.mdx)替代本插件。更多详情请参阅[此链接](https://github.com/facebook/docusaurus/issues/7221)。

:::

:::warning[仅生产环境]

该插件在开发环境中始终处于非活动状态，**仅在生产环境中生效**，以避免污染分析统计数据。
