...
在构建过程中，插件会并行加载以获取自身内容并将其渲染为路由。插件还可以配置 webpack 或对生成的文件进行后处理。

## `async loadContent()` {#loadContent}

插件应使用此生命周期从数据源（文件系统、远程API、无头CMS等）获取数据或进行一些服务器端处理。返回值是插件所需的内容。
...
在`contentLoaded`中会消费`loadContent`加载的数据。这些数据可以被渲染为路由、注册为全局数据等。
...
一个声明式回调，用于创建静态数据（通常是JSON或字符串），这些数据稍后可以作为props提供给您的路由。接收文件名和要存储的数据，返回实际数据文件的路径。
...
修改内部webpack配置。如果返回值是一个JavaScript对象，它将使用[`webpack-merge`](https://github.com/survivejs/webpack-merge)合并到最终配置中。如果是一个函数，它将被调用，并接收`config`作为第一个参数，`isServer`标志作为第二个参数。
...
`configureWebpack` 在服务端构建和客户端构建时都会被调用。服务端构建接收 `true` 作为 `isServer` 参数，客户端构建则接收 `false`。
...
阅读 [webpack-merge 策略文档](https://github.com/survivejs/webpack-merge#merging-with-strategies) 了解更多详情。
