...
当需要在网站发布公告时，可添加公告栏。这是一个非固定且可选关闭的面板，位于导航栏上方。所有配置均在`announcementBar`对象中。
...
标志可放置在[静态文件夹](static-assets.mdx)中。标志URL默认设置为站点根路径。虽然可以自定义标志URL，但如果是外部链接将在新标签页打开。此外可覆盖标志链接的target属性值，当文档网站作为主站点的子目录托管时特别有用，这种情况下可能不需要标志链接在主站新标签页打开。
...
根据`type`字段不同，导航栏项可具有不同行为。以下章节将介绍所有可用的导航栏项类型。

#### 导航栏链接 {#navbar-link}

默认情况下，导航栏项是常规链接（内部或外部）。

React Router 通常会自动为链接应用活动链接样式，但在边缘情况下可以使用 `activeBasePath`。若需使链接在多个不同路径下保持活动状态（例如同一侧边栏下存在多个文档目录），可使用 `activeBaseRegex`。该参数作为 `activeBasePath` 的更灵活替代方案，会被解析为正则表达式来匹配当前 URL，且优先级高于后者。
...
类型为 `dropdown` 的导航栏项目需额外配置 `items` 字段，即嵌套的导航栏项目数组。

导航栏下拉菜单仅接受以下**类链接项目类型**：
...
注意下拉菜单基项本身也是可点击链接，因此可接收[普通导航栏链接](#navbar-link)的所有参数。
...
若需链接至特定文档，此特殊类型会通过 `docId` 渲染文档链接。只要浏览同一侧边栏下的文档，该链接就会保持 `navbar__link--active` 类。
...
#### 导航栏侧边栏链接 {#navbar-doc-sidebar}

可将导航栏项目关联至指定侧边栏的首个文档链接（可以是文档链接或生成的分类索引），无需硬编码文档 ID。
...
若使用版本化文档，此特殊类型会渲染包含所有可用版本的下拉菜单。

用户可在不同版本间切换，并保持停留在同一文档（需文档 ID 跨版本保持一致）。
...
若使用版本化文档，此特殊类型会链接至当前浏览的文档版本（根据 URL 判断），并默认回退至最新版本。
...
若启用了[i18n国际化功能](../../i18n/i18n-introduction.mdx)，此特殊导航栏项类型将渲染一个下拉菜单，展示站点所有可用语言版本。
...
目前支持两种样式选项：`dark`（暗色）和`primary`（基于`--ifm-color-primary`主色调）。样式预览参见[Infima文档](https://infima.dev/docs/components/navbar/)。
...
若使用行高亮Markdown语法，可能需要为暗色模式语法高亮主题指定不同的高亮背景色。具体指导参见[相关文档](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)。
...
通过`themeConfig.footer`可为页脚添加Logo和版权信息。Logo需放置在[静态文件夹](static-assets.mdx)中，其URL配置方式与导航栏Logo相同。
...
您可以通过 `themeConfig.footer.links` 添加页脚链接。页脚配置有两种类型：**多列页脚**和**简易页脚**。

多列页脚链接为每列设置一个 `title` 和一组 `FooterItem` 列表。
...
- **多实例路径**: 不适用
- **JSON文件**: 通过 [`docusaurus write-translations`](../../cli.mdx#docusaurus-write-translations-sitedir) 提取
- **Markdown文件**: 不适用
