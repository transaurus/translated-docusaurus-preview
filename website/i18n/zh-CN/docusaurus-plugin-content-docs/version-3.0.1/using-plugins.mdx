...
不过您通常不需要逐个安装常见插件：它们可以以[预设包](#using-presets)的形式分发。对于大多数用户而言，插件是通过预设配置进行配置的。

我们维护了一份[官方插件列表](./api/plugins/overview.mdx)，社区也开发了一些[非官方插件](/community/resources#community-plugins)。如果您需要添加任何功能：自动生成文档页面、执行自定义脚本、集成第三方服务...请务必查阅该列表：可能已经有人为您实现了！

如果您精力充沛，也可以阅读[插件开发指南](./advanced/plugins.mdx)或[插件方法参考文档](./api/plugin-methods/README.mdx)来学习如何开发自己的插件。
...
路径可以是绝对路径或相对于配置文件的相对路径。

## 配置插件 {#configuring-plugins}

最基本的插件使用方式是在配置中仅提供插件名称或路径。

但插件可以通过"Babel风格"的配置方式支持选项——在配置中将插件名称和选项对象包装成一个二元数组。
...
最多只能有一个插件实例作为"默认插件实例"，通过省略`id`属性或使用`id: 'default'`实现。

:::

## 使用主题 {#using-themes}

主题的加载方式与插件完全相同。从使用者角度看，在安装和配置插件时，`themes`和`plugins`条目可以互换使用。唯一的细微差别在于主题会在插件之后加载，且[主题可以覆盖插件的默认主题组件](./advanced/client.mdx#theme-aliases)。

:::tip

`themes`和`plugins`选项会导致不同的[简写解析规则](#module-shorthands)，因此如果您想利用简写功能，请确保使用正确的配置条目！
...
预设（Presets）是插件和主题的集合包。例如，无需在配置文件中逐一注册和配置`@docusaurus/plugin-content-docs`、`@docusaurus/plugin-content-blog`等插件，通过`@docusaurus/preset-classic`预设，您可以在一个集中位置配置它们。
...
这在某些插件和主题需要一起使用时特别有用。您甚至可以将它们的选项链接起来，例如将一个选项传递给多个插件。
...
其中`moduleType`是`'preset'`、`'theme'`或`'plugin'`之一，具体取决于模块名称声明在哪个字段中。第一个成功找到的模块名称会被加载。

如果名称是作用域化的（以`@`开头），名称会首先通过第一个斜杠分割为作用域和包名：
...
如果名称不带作用域（如`@jquery`），则会加载`[scope]/docusaurus-[moduleType]`（即`@jquery/docusaurus-plugin`）。否则将尝试以下路径：

- `[scope]/[name]`（如`@jquery/content-docs`）
- `[scope]/docusaurus-[moduleType]-[name]`（如`@jquery/docusaurus-plugin-content-docs`）

以下是在`plugins`字段中注册插件的一些示例。请注意，与[ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins)或[Babel](https://babeljs.io/docs/en/options#name-normalization)强制要求插件命名规范不同，Docusaurus允许更自由的命名方式，因此解析路径并不绝对，但会遵循上述定义的优先级。
