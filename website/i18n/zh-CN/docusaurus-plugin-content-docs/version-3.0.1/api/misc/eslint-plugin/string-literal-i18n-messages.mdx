...
Docusaurus提供了[`docusaurus write-translations`](../../../cli.mdx#docusaurus-write-translations-sitedir) API，该接口会静态提取标记为可翻译的文本标签。在`<Translate>`或`translate()`调用中使用的动态值将无法被提取。本规则将确保所有翻译调用都是可静态提取的。
...
如果您没有使用[国际化功能](../../../i18n/i18n-introduction.mdx)，可以禁用此规则。
