...
本插件会读取[`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes)配置，当多个文件将输出至相同位置时调整日志级别。
...
"from"与"to"概念是本插件的核心逻辑：
- "from"表示您希望创建的路径（即待生成的额外HTML文件）
- "to"表示重定向目标路径（通常是Docusaurus已有路由）

因此允许为同一个"to"配置多个"from"（生成多个指向相同目标的HTML文件），但单个"from"不能对应多个"to"（生成的HTML文件必须具有确定的重定向目标）。
