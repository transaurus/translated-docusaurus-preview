...
标志可放置在[静态文件夹](static-assets.mdx)中。标志URL默认设置为站点基础URL。虽然可以指定自定义URL，但如果是外部链接将在新标签页打开。此外可覆盖标志链接的target属性值，当文档网站托管在主站点的子目录时特别有用——这种情况下可能不需要标志链接在主站新标签页打开。
...
项目行为根据`type`字段而异。以下章节将介绍所有可用的导航栏项目类型。

#### 导航栏链接 {#navbar-link}

默认情况下，导航栏项目是常规链接（内部或外部）。

React Router 通常会自动为链接应用活动链接样式，但在边缘情况下可以使用 `activeBasePath`。当某个链接需要在多个不同路径下保持活动状态时（例如同一侧边栏下存在多个文档目录），可以使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更灵活的替代方案，且优先级更高——Docusaurus 会将其解析为正则表达式来匹配当前 URL。

外部链接会自动添加 `target="_blank" rel="noopener noreferrer"` 属性。
...
若需链接至特定文档，此特殊导航栏项目类型会渲染指向 `docId` 指定文档的链接。只要浏览同一侧边栏下的文档，该链接就会获得 `navbar__link--active` 类。
...
无需硬编码文档ID，即可将导航栏项目链接至指定侧边栏的第一个文档链接（可以是文档链接或生成的分类索引）。
...
用户可在保持当前文档不变的情况下切换版本（要求文档ID在不同版本间保持一致）。
...
若使用版本化文档，此特殊导航栏项目类型会链接至当前浏览的文档版本（取决于URL），并默认回退至最新版本。
...
若您启用了[i18n国际化功能](../../i18n/i18n-introduction.mdx)，此特殊导航栏项目类型将渲染一个下拉菜单，展示您站点支持的所有语言环境。
...
#### 自定义HTML导航项 {#navbar-with-custom-html}

您也可以通过此导航栏项目类型，在导航项中渲染自定义HTML标记。
...
您可以启用这个酷炫的UI特性：当用户向下滚动页面时自动隐藏导航栏，向上滚动时重新显示。
...
您可以在保留主题切换功能的同时设置静态导航栏样式。所选样式将始终生效，不受用户选择的主题影响。

目前支持两种样式选项：`dark`（暗色）和基于`--ifm-color-primary`颜色的`primary`（主色）。样式预览可参见[Infima文档](https://infima.dev/docs/components/navbar/)。
...
默认使用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)作为语法高亮主题。您可以从[可用主题列表](https://github.com/FormidableLabs/prism-react-renderer/tree/master/packages/prism-react-renderer/src/themes)选择自定义主题，还可为暗色模式指定不同的语法高亮主题。
...
若使用行高亮Markdown语法，可能需要为暗色模式语法高亮主题指定不同的高亮背景色。具体请参考[行高亮文档](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)。

:::

### 默认语言 {#default-language}

可为未标注语言的代码块（即仅以```开头的代码块）设置默认语言。注意必须传入有效的[语言名称](https://prismjs.com/#supported-languages)。
...
通过`themeConfig.footer`可添加页脚logo和版权信息。logo文件可放置在[静态资源目录](static-assets.mdx)，其URL处理方式与导航栏logo相同。
...
多栏页脚链接需要为每列配置 `title` 和一组 `FooterItem` 列表。
...
用于访问色彩模式的React钩子。该上下文包含设置浅色/深色模式的函数，并暴露表示当前模式的布尔变量。
