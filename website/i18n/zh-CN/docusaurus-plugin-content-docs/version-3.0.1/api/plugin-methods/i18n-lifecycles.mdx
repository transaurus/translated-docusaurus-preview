...
- `path`：相对于插件本地化文件夹`i18n/[locale]/[pluginName]`的路径。应省略扩展名`.json`以保持通用性。
- `content`：使用Chrome i18n JSON格式。

这些文件将由[`write-translations` CLI](../../cli.mdx#docusaurus-write-translations-sitedir)写入插件的i18n子文件夹，并在调用[`translateContent()`](#translateContent)和[`translateThemeConfig()`](#translateThemeConfig)之前读取适当的本地化版本。
...
`contentLoaded()`生命周期将使用`translateContent()`返回的本地化插件内容调用。
...
使用本地化的翻译文件翻译站点`themeConfig`中的标签。
...
它应返回`Record<string, string>`格式的消息，其中键是翻译ID，值是根据站点当前本地化设置的消息（不包含描述）。
