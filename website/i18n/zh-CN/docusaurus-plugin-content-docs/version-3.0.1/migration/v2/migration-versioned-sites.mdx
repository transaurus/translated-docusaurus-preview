...
与v1不同，每个版本化文档的Markdown头部不再使用`version-${version}-${original_id}`作为实际ID字段的值。具体差异请参见下方示例。

例如，假设您有一个`docs/hello.md`文件。
...
由于我们采用快照机制且允许用户在版本内自由移动（和编辑）文档，`id` front matter不再被修改且保持原样。内部实际ID会被设置为`version-${version}/${id}`。
...
由于v1和v2都不允许在front matter中使用`/`字符，冲突发生的概率会降低。
...
在v2中，我们采用快照机制实现文档版本控制。**每个版本的文档不依赖其他版本**。可能出现`foo.md`存在于`version-1.0.0`但不存在于`version-1.2.0`的情况。由于Docusaurus v1的回退功能(https://v1.docusaurus.io/docs/en/versioning#fallback-functionality)，这种场景在之前版本中无法实现。
...
Docusaurus v1**仅当文档内容发生变化时**才会创建版本化文档。如果从v1.0.0到v1.1.0只有`hello.md`发生变更，您的文档结构可能如下所示：
