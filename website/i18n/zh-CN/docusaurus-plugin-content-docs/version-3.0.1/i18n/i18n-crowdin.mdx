...
欢迎在 **[社区驱动的 GitHub 讨论区](https://github.com/facebook/docusaurus/discussions/4052)** 交流 Docusaurus 与 Crowdin 集成的相关问题。
...
本教程将演示如何使用 Crowdin 将新建的英文版 Docusaurus 网站翻译为法文版，假设您已完成[国际化教程](./i18n-tutorial.mdx)的学习。
...
切勿提交该令牌，建议为您的公司创建**专用Crowdin账户**而非使用个人账号。
...
确保您的网站现已翻译为法语，可通过[`http://localhost:3000/fr/`](http://localhost:3000/fr/)访问。
...
Crowdin**官方不支持MDX**，但已添加**对`.mdx`扩展名的支持**，会将这些文件视为Markdown（而非纯文本）处理。

#### MDX问题 {#mdx-problems}

Crowdin会将JSX语法误认为嵌入式HTML，在下载翻译时可能破坏JSX标记，导致因无效JSX而构建失败。

使用简单字符串属性的JSX片段（如`<Username name="Sebastien"/>`）可以正常工作；而使用对象/数组属性的复杂JSX片段（如`<User person={{name: "Sebastien"}}/>`）则更容易因语法不符合HTML规范而失败。
...
不使用`Hide`会导致配额中的`源字符串`数量激增，直接影响计费。
...
理论上这能实现Git与Crowdin的双向编辑同步，建立双向工作流。

但实际运行中存在可靠性问题：

- Crowdin→Git同步正常（通过PR机制）
- Git→Crowdin需手动触发（需点击按钮）
- Crowdin匹配现有Markdown翻译的启发式算法并非100%可靠，同步后需人工校验
- Git与Crowdin的并发编辑可能导致翻译丢失
...
Crowdin会将Markdown字符串替换为`crowdin:id12345`等技术ID，但替换过程过于激进，会破坏front matter、admonitions和JSX等隐藏字符串的结构。
...
您可以通过使用 `editUrl` 函数按语言环境自定义编辑链接，使编辑按钮指向 Crowdin 界面。
...
**Docusaurus 配置文件**是同时使用版本控制和多实例功能的典型示例：
