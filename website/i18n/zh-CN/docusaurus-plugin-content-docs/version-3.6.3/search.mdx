...
该服务对开发者文档和技术博客**完全免费**：只需阅读[准入清单](https://docsearch.algolia.com/docs/who-can-apply/)并[申请加入DocSearch计划](https://docsearch.algolia.com/apply)。

DocSearch会每周抓取您的网站（可通过网页界面配置周期），将所有内容聚合到Algolia索引中。这些内容随后通过Algolia API从前端直接查询。

如果您的网站[不符合](https://docsearch.algolia.com/docs/who-can-apply)免费托管版DocSearch的条件，或网站部署在防火墙后非公开访问，您可以[自行运行](https://docsearch.algolia.com/docs/run-your-own/)DocSearch爬虫。
...
您可以通过[我们的博客文章](/blog/2021/11/21/algolia-docsearch-migration)或[DocSearch迁移文档](https://docsearch.algolia.com/docs/migrating-from-legacy)了解从旧版基础设施迁移的更多信息。

:::

### 索引配置 {#algolia-index-configuration}

申请通过并部署后，您将收到包含项目接入DocSearch所有详情的邮件。可通过[网页界面](https://crawler.algolia.com/)编辑和管理爬取任务。索引在部署后立即可用，通常无需手动配置。
...
爬虫配置中的`initialIndexSettings`仅用于初始化尚未存在的Algolia索引。

若更新`initialIndexSettings`爬虫配置，虽然可通过界面手动更新索引，但[Algolia团队建议删除索引后重新爬取](https://github.com/facebook/docusaurus/issues/9200#issuecomment-1667338492)，以全新初始化新配置。

:::

### 接入Algolia {#connecting-algolia}
...
当浏览 v2 文档时，若返回 v1 版本的搜索结果会显得不合逻辑。由于不同版本文档内容可能高度相似，这将导致同一查询返回重复结果（每个版本各出现一次）。

同理，在法语站点浏览时返回英文文档结果也不符合预期。

上下文搜索功能通过动态创建搜索查询过滤器，确保结果始终匹配当前浏览的文档版本和语言。

- 在 `/en/docs/v1/myDoc` 页面，搜索结果仅包含 **v1 版本英文文档**（及未版本化的页面）
- 在 `/fr/docs/v2/myDoc` 页面，搜索结果仅包含 **v2 版本法语文档**（及未版本化的页面）
...
若仅在禁用上下文搜索时才能获取结果，通常是由于[索引配置问题](#algolia-no-search-results)导致。
...
[Typesense](https://typesense.org) DocSearch与Algolia DocSearch工作原理相似，区别在于您的网站内容会被索引到Typesense搜索集群中。
...
对于搜索索引较小且可随网站加载到用户浏览器的情况，可使用本地搜索插件。
...
这将在项目文件夹中创建`src/theme/SearchBar`文件。重启开发服务器并编辑该组件，您会发现Docusaurus现在使用您的自定义`SearchBar`组件。
