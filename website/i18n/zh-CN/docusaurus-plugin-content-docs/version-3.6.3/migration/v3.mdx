...
Docusaurus v3 是一个**主版本更新**，包含需要您相应调整站点的**破坏性变更**。我们将在此过程中为您提供指引，并提及一些可选建议。
...
查阅 [**Docusaurus v3.0.0**](https://github.com/facebook/docusaurus/releases/tag/v3.0.0) 的发布说明，并浏览相关 pull-requests 以获取更多有用信息及本文提及的每项变更背后的动机。
...
此命令也是评估使您的内容兼容所需工作量的好方法。请记住，大部分工作可以通过[为 Docusaurus v3 准备您的内容](/blog/preparing-your-site-for-docusaurus-v3)在升级前完成。
...
MDX Playground将成为您的新朋友。它可以帮助您理解内容如何**被编译为React组件**，并隔离地排查编译或渲染问题。
...
`{`字符用于打开[JavaScript表达式](https://mdxjs.com/docs/what-is-mdx/#expressions)。如果放在`{expression}`中的内容不是有效表达式，MDX现在将失败。
...
`<` 符号用于开启 [JSX 标签](https://mdxjs.com/docs/what-is-mdx/#jsx)。若 MDX 认为你的 JSX 无效，现在会直接报错。
...
> 在 `paragraph` 结束前期望出现 `</T>` 闭合标签 (1:6-1:9) end-tag-mismatch mdast-util-mdx-jsx

> 在 `paragraph` 结束前期望出现 `</YOUR_MINOR_VERSION>` 闭合标签 (134:19-134:39)
...
> 名称中出现意外字符 `@` (U+0040)，期望是字母、数字、`$` 或 `_` 等名称字符，或属性前的空白，或标签结束符（注意：在 MDX 中创建链接请使用 `[文本](网址)`）

> 在本地名称前出现意外字符 `/` (U+002F)，期望是字母、`$` 或 `_` 等可开头字符（注意：在 MDX 中创建链接请使用 `[文本](网址)`）
...
对于自定义 [`MDXComponent` 映射](../guides/markdown-features/markdown-features-react.mdx#mdx-component-scope)的用户，组件现在会被"沙盒化"：

- 为 `h1` 定义的 `MDXComponent` 映射仅适用于 `# 标题` 语法，不适用于 `<h1>标题</h1>`
...
在 MDX v3 中，现在可以更轻松地交错使用 JSX 和 Markdown，而无需额外的换行符。在多行上编写内容也可能产生预期之外的 `<p>` 标签。

:::danger[视觉差异]

查看以下内容在 MDX v1 和 v3 中的渲染差异。
...
如果不希望产生额外的 `<p>` 标签，可以逐个案例重构内容，使用单行 JSX 标签。
...
如果尚未打算在该内容中使用 Markdown 语法，也可以用 `{` 和 `}` 包裹以避免额外的 `<p>` 标签。
...
新的MDX解析器现在严格遵循CommonMark规范。自v0.14起，CommonMark规范引入了关于空格和标点符号周围强调文本的规则，这对不使用空格分隔单词的语言（如中日韩文本）影响尤为显著。

日语和中文受此影响最大，但其他一些语言（如泰语和高棉语）也可能受到影响，例如当您尝试强调内联代码或链接时。使用空格分隔单词的语言受影响程度要小得多。

以下示例中的`**`（除了`` `**` ``）在Docusaurus 2中能按预期解析，但在Docusaurus 3中不再有效。
...
MDX生态系统中的所有官方包（Unified、Remark、Rehype等）现在都[仅支持ES模块](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c)，不再支持[CommonJS](https://nodejs.org/api/modules.html#modules-commonjs-modules)。

这意味着您不能再使用`require("remark-plugin")`这样的语法。

:::tip[升级指南]

Docusaurus v3现在支持[ES模块](https://flaviocopes.com/es-modules/)配置文件。我们建议您将配置文件迁移至ES模块，这样可以轻松导入Remark插件：
...
如果您希望继续使用CommonJS模块，可以使用动态导入作为变通方案，这允许您在CommonJS模块中导入ES模块。幸运的是，[Docusaurus配置支持异步函数语法](/docs/configuration#syntax-to-declare-docusaurus-config)来实现这一点。
...
如果您创建了自定义的Remark或Rehype插件，由于新AST的结构变化，您可能需要重构这些插件，甚至完全重写。我们创建了[专门的讨论帖](https://github.com/facebook/docusaurus/discussions/9337)来帮助插件开发者升级代码。

:::

### 代码格式化工具

目前最常用的格式化工具Prettier仅支持旧版MDX v1，截至Docusaurus v3.0.0尚未支持v3。您可以在代码不兼容部分前添加`{/* prettier-ignore */}`来使其与Prettier兼容。
...
如果`{/* prettier-ignore */}`注释过多影响使用，您可以考虑在Prettier支持MDX v3之前，通过将以下内容添加到`.prettierignore`文件来禁用MDX格式化：
...
除了MDX v3升级外，以下是Docusaurus v3带来的完整破坏性变更列表。
...
您还可以更新站点 `package.json` 文件以防止使用旧版不受支持的 Node.js：
...
Docusaurus 对这些特性的支持被视为实验性的。未来我们可能需要调整集成方式，导致运行时行为发生变化。
...
这是一个包含破坏性变更的新主版本库，我们无法保证严格的向后兼容性。[`prism-react-renderer` v2 发布说明](https://github.com/FormidableLabs/prism-react-renderer/releases/tag/prism-react-renderer%402.0.0)并不十分详尽，但 Docusaurus 用户需要注意 3 个主要变化。
...
由于历史原因，我们曾支持未在文档中声明的红色`:::warning`警告提示框。

:::danger[警告]

这是Docusaurus v2版本的`:::warning`红色警告提示框。

:::

但其颜色和图标始终存在偏差。Docusaurus v3正式重新引入`:::warning`提示框，完善文档说明，并修正了颜色与图标。

:::warning

这是Docusaurus v3版本的`:::warning`黄色警告提示框。

:::

:::info[如何升级]

若您曾使用过未文档化的`:::warning`提示框，请逐一检查黄色是否适合当前场景。如需保持红色警示，请改用`:::danger`。

Docusaurus v3同时[弃用了`:::caution`提示框](https://github.com/facebook/docusaurus/pull/9308)。请将原`:::caution`（黄色）重构为`:::warning`（黄色）或`:::danger`（红色）。
...
此变更仅影响**在v2.0.0-beta.10版本（2021年12月）前**启用文档版本化的早期用户。

当创建v1.0.0版本时，侧边栏文件包含`version-v1.0.0/`前缀，该语法[已被Docusaurus v3弃用](https://github.com/facebook/docusaurus/pull/9310)。
...
若不希望启用此限制，可通过`limit: false`配置恢复原有行为：
...
对于通过swizzle定制过文档主题组件（如`@theme/DocPage`）的用户，这些组件已进行重大重构以提升可定制性。

严格来说**这并非破坏性变更**，因为这些组件**本就不建议swizzle**。但许多站点仍进行了定制，需注意原有修改可能导致兼容问题。

:::info[如何升级]

删除所有swizzle过的组件，重新执行swizzle操作，并在新版组件基础上重新应用定制。

或参考[PR说明](https://github.com/facebook/docusaurus/pull/7966)了解新版主题组件结构，尝试手动迁移现有定制。

:::

## 可选变更项

以下变更非强制要求，但了解这些特性可充分发挥Docusaurus v3的优势。
...
当您在 Markdown 文件中使用 JSX、`import` 或 `export`（即 MDX 特性）时，建议使用 `.mdx` 扩展名。这更符合语义规范，并能提升与外部工具（IDE、格式化程序、linter 等）的兼容性。

未来版本的 Docusaurus 将把 `.md` 文件解析为标准 [CommonMark](https://commonmark.org/)（不支持上述特性）。在 Docusaurus v3 中，`.md` 文件仍会被编译为 MDX 文件，但后续可[选择启用 CommonMark 解析](https://github.com/facebook/docusaurus/issues/3018)。
...
此选项允许在 MDX 中使用 HTML 注释，而官方标准已不再支持 HTML 注释。
...
此选项支持 Docusaurus v2 的[警告框标题](../guides/markdown-features/markdown-features-admonitions.mdx#specifying-title)语法：
...
建议逐步采用新的 Markdown 指令标签语法，然后关闭此兼容选项。

#### `headingIds` 选项

此选项支持 Docusaurus v2 的[显式标题 ID](../guides/markdown-features/markdown-features-toc.mdx#heading-ids) 语法：

<Code language="md">{'### Hello World \u007B#my-explicit-id}\n'}</Code>

该语法现为无效 MDX，需转义 `{` 字符：`{#my-explicit-id}`。
