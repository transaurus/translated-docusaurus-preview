...
[Front Matter](./guides/markdown-features/markdown-features-intro.mdx#front-matter)可用于添加文章元数据（如作者信息），但Docusaurus也能自动推断必要元数据。完整字段说明请参阅[API文档](api/plugins/plugin-content-blog.mdx#markdown-front-matter)。

## 博客列表 {#blog-list}

博客首页（默认路径为`/blog`）即_博客列表页_，集中展示所有文章。

在文章中使用`<!--truncate-->`标记定义摘要内容，该标记前的所有内容将作为摘要显示。请注意摘要部分必须是独立可渲染的Markdown内容。例如：
...
默认每页显示10篇文章，可通过插件配置中的`postsPerPage`参数控制分页。设为`postsPerPage: 'ALL'`将禁用分页并首屏显示所有文章。还可为博客列表页添加meta描述以优化SEO：
...
博客侧边栏默认展示5篇最新文章，通过`blogSidebarCount`参数可调整数量。设为`blogSidebarCount: 0`将完全禁用侧边栏（容器也会移除）以增加主内容区宽度。特殊值`blogSidebarCount: 'ALL'`会展示_所有_文章。
...
此命名约定是可选的，您也可以通过 front matter 提供日期。由于 front matter 遵循支持日期时间标记的 YAML 语法，当需要更精确的发布日期时可以使用 front matter。例如，若有多篇博文在同一天发布，您可以根据具体时间进行排序：
...
使用 `authors` front matter 字段声明博文作者。作者至少需包含 `name` 或 `image_url` 属性。Docusaurus 会使用 `url`、`email` 和 `title` 等信息，但也允许添加其他任意信息。
...
无论是通过 front matter 还是作者映射表声明的作者，都必须具有名称或头像（或两者兼备）。若某篇博文的所有作者均未设置名称，Docusaurus 将以紧凑形式显示其头像。效果可参见[此测试文章](/tests/blog/2022/01/20/image-only-authors)。
...
- `Releases`是内联标签，因其不存在于`blog/tags.yml`
...
`readingTime`回调接收三个参数：博客内容文本字符串、front matter键值记录，以及默认阅读时长函数。返回数字（分钟数）或`undefined`（禁用该页面的阅读时长显示）。

默认阅读时长函数可接受额外选项：`wordsPerMinute`（数字，默认300）和`wordBound`（字符串到布尔值的函数）。当传入字符串应被视为单词边界（默认为空格、制表符和换行符）时，函数应返回`true`。
...
通过配置`feedOptions`可生成RSS/Atom/JSON订阅源。默认生成RSS和Atom源，设为`null`可禁用此功能。
...
您可以让博客文章列表页替代独立首页作为站点入口。将`routeBasePath`设为`'/'`后，博客将通过根路由`example.com/`提供服务，而非子路径`example.com/blog/`。
...
若禁用文档插件，请同步移除配置文件中所有相关引用，特别注意需删除导航栏中的文档相关条目。

:::

:::tip

针对仅需文档功能的用户，我们还提供"纯文档模式"。详见[纯文档模式](./guides/docs/docs-introduction.mdx)获取详细配置说明及`routeBasePath`参数的深入解析。

:::

### 多博客系统 {#multiple-blogs}

经典主题默认单博客架构，仅包含一个博客插件实例。若需在单站点部署多个博客，可通过在`docusaurus.config.js`的`plugins`选项中配置额外博客插件实现。

为第二个博客设置专属的`routeBasePath`路由路径，注意需与首个博客路径区分以避免冲突。同时通过`path`参数指定第二个博客内容的存储目录。

如[多实例插件](./using-plugins.mdx#multi-instance-plugins-and-plugin-ids)文档所述，需为各插件分配唯一ID。
