...
同样地，您可以通过 `require` 引用现有资源，并将返回的 URL 用于 `video` 视频标签、`a` 锚点链接等场景。
...
如果使用 Markdown 图片或链接语法，Docusaurus 会将所有资源路径解析为文件路径并自动转换为 `require()` 调用。除非使用 JSX 语法（此时需自行处理），否则无需在 Markdown 中显式使用 `require()`。
...
当您需要通过 CSS 修改 SVG 图片部分样式时，这个功能非常有用。例如，可以根据当前主题动态改变 SVG 的某些颜色。
...
要通过路径片段切换图片显示（GitHub 使用 `#gh-dark-mode-only` 和 `#gh-light-mode-only`），请将以下代码添加到自定义 CSS 中（若不希望与 GitHub 耦合，也可使用自定义后缀）：
...
当 Markdown 链接或图片使用绝对路径时，该路径会被视为文件路径，并从静态资源目录中解析。例如，若您配置的[静态目录](../../static-assets.mdx)为 `['public', 'static']`，则对于以下图片：

```md title="my-doc.md"
![An image from the static](/img/docusaurus.png)
```

Docusaurus 会依次在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 路径下查找。该链接最终会被转换为 `require()` 调用而非保留为 URL。这种处理方式在以下两方面具有优势：
