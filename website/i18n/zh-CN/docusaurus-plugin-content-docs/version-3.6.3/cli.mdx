...
Docusaurus 提供了一系列脚本，帮助您生成、运行和部署网站。
...
使用 [Webpack 开发服务器](https://webpack.js.org/configuration/dev-server) 本地构建并运行网站预览。
...
当需要从远程服务器或虚拟机（如 GitHub Codespaces）转发 3000 端口时，可指定开发服务器监听本地 IP 地址：
...
获取证书有多种方式，此处以 [mkcert](https://github.com/FiloSottile/mkcert) 为例：

1. 执行 `mkcert localhost` 生成 `localhost.pem` 和 `localhost-key.pem`

2. 运行 `mkcert -install` 将证书安装至信任库，并重启浏览器

3. 通过环境变量启动 Docusaurus HTTPS 服务：
...
对于 CSS 的高级压缩，我们使用 [cssnano 高级预设](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-advanced)（配合多个 PostCSS 插件）和 [clean-css 二级优化](https://github.com/jakubpawlowicz/clean-css#level-2-optimizations)。若因此导致 CSS 异常，可通过环境变量 `USE_SIMPLE_CSS_MINIFIER=true` 切换至 [cssnano 默认预设](https://github.com/cssnano/cssnano/tree/master/packages/cssnano-preset-default)。**如遇 CSS 压缩问题请[提交 issue](https://github.com/facebook/docusaurus/issues/new?labels=bug%2C+needs+triage&template=bug.md)**。

通过环境变量 `SKIP_HTML_MINIFICATION=true` 可跳过 HTML 压缩。

:::

### `docusaurus swizzle [themeName] [componentName] [siteDir]` {#docusaurus-swizzle}

通过[组件替换](./swizzling.mdx)来自定义主题组件。
...
该交互式 CLI 将全程引导您完成[替换流程](./swizzling.mdx)。
...
将您的站点部署到 [GitHub Pages](https://pages.github.com/)。更多详情请查阅 [部署文档](deployment.mdx#deploying-to-github-pages)。
...
为站点 Markdown 文档添加 [显式标题 ID](./guides/markdown-features/markdown-features-toc.mdx#heading-ids)。
