...
[订阅功能](../../blog.mdx#feed)通过解析构建输出实现，**仅在生产环境激活**。
...
用于通过 [XSLT](https://developer.mozilla.org/en-US/docs/Web/XSLT) 样式化博客 XML 订阅源，使其在浏览器中优雅渲染。

- 设为 `true` 使用插件内置的 `.xsl` 和 `.css` 文件样式化订阅源
- 设为假值（`undefined | null | false`）禁用此功能
- 设为 `string` 可指定博客内容目录下自定义 `.xsl` 文件路径（需配套同名 `.css` 文件）
...
该文件可包含预定义的[全局博客作者](../../blog.mdx#global-authors)，在 Markdown 文件中可通过 [`authors` 前置元数据](#markdown-front-matter)引用这些作者。
