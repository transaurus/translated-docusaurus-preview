...
该文件可包含预定义的标签列表。您可以通过 Markdown 文件中的 [`tags` 前置元数据](#markdown-front-matter) 来引用这些标签键。

:::tip[保持标签一致性]

使用标签文件可确保插件内容集中的标签使用保持一致。通过 [`onInlineTags: 'throw'`](#onInlineTags) 插件选项可强制执行此一致性，并防止使用临时声明的行内标签。
