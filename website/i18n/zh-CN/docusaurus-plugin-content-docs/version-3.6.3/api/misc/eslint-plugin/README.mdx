...
这将启用 `@docusaurus` ESLint 插件并使用 `recommended` 配置。请参阅下面的[支持规则](#supported-rules)了解此配置将启用的规则列表。

### 手动配置

如需更精细的控制，你也可以手动启用插件并直接配置要使用的规则：
...
- 推荐(recommended)：适用于大多数 Docusaurus 站点的推荐规则集，应作为基础配置扩展。
- 全部(all)：启用**所有**规则。这会在次要版本之间发生变化，因此如果你希望避免意外的破坏性变更，不应使用此配置。
