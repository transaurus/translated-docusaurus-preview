...
静态方法不属于插件实例——它们附加在构造函数上。这些方法用于验证和规范化插件选项及主题配置，这些参数随后会作为构造函数参数来初始化插件实例。

## `validateOptions({options, validate})` {#validateOptions}

返回经过验证和规范化的插件选项。该方法在插件初始化前调用，必须返回选项对象，因为它们会在初始化时传递给插件。

### `options` {#options}

`validateOptions` 方法接收传入插件的 `options` 参数进行验证和规范化。
...
如果不使用 **[Joi](https://www.npmjs.com/package/joi)** 进行验证，可以在选项无效时抛出错误，在验证通过时返回选项。
...
`validateThemeConfig` 方法接收 `docusaurus.config.js` 中提供的 `themeConfig` 进行验证和规范化。
...
如果不使用 **[Joi](https://www.npmjs.com/package/joi)** 进行验证，可以在选项无效时抛出错误。
