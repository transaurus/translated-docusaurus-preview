...
插件应使用此生命周期从数据源（文件系统、远程API、无头CMS等）获取数据或进行一些服务器端处理。返回值是它所需的内容。
...
`contentLoaded`将在`loadContent`完成后调用。`loadContent()`的返回值将作为`content`传递给`contentLoaded`。
...
一个声明式回调，用于创建静态数据（通常是JSON或字符串），稍后可以作为props提供给您的路由。接收文件名和要存储的数据，并返回实际数据文件的路径。

例如，下面的插件创建一个显示`你的朋友是：Yangshun, Sebastien`的`/friends`页面：
...
修改内部webpack配置。如果返回值是一个JavaScript对象，它将使用[`webpack-merge`](https://github.com/survivejs/webpack-merge)合并到最终配置中。如果是一个函数，它将被调用并接收`config`作为第一个参数，`isServer`标志作为第二个参数。

:::warning

`configureWebpack`的API将在未来修改为接收一个对象（`configureWebpack({config, isServer, utils, content})`）
...
`configureWebpack` 会在服务端构建和客户端构建时被调用。服务端构建接收 `true` 作为 `isServer` 参数值，客户端构建则接收 `false`。
...
您可以使用这些工具函数有条件地返回您的 webpack 配置。

例如，下方插件通过修改 webpack 配置来转译 `.foo` 文件。
...
参阅 [webpack-merge 策略文档](https://github.com/survivejs/webpack-merge#merging-with-strategies) 了解更多细节。

### 配置开发服务器 {#configuring-dev-server}

可通过返回 `devServer` 字段来配置开发服务器。
...
向 Docusaurus 生成的 HTML 中注入头部和/或正文 HTML 标签。

`injectHtmlTags` 会同时传入插件加载的内容。
...
- `headTags` 将在 `</head>` 闭合标签前插入，位于配置添加的脚本之后。
- `preBodyTags` 将在 `<body>` 开始标签后插入，位于所有子元素之前。
- `postBodyTags` 将在 `</body>` 闭合标签前插入，位于所有子元素之后。

## `getClientModules()` {#getClientModules}

返回要导入客户端包的[客户端模块](../../advanced/client.mdx#client-modules)路径数组。

例如，要使您的主题加载用户通过 `options` 传入的 `customCss` 或 `customJs` 文件路径：
