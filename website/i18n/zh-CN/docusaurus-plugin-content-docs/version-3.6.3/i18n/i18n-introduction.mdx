...
借助国际化（[i18n](https://en.wikipedia.org/wiki/Internationalization_and_localization)）支持，**翻译 Docusaurus 网站非常简单**。

## 目标 {#goals}

理解 Docusaurus 国际化支持背后的**设计决策**非常重要。

更多背景信息可查阅最初的 [RFC](https://github.com/facebook/docusaurus/issues/3317) 和 [PR](https://github.com/facebook/docusaurus/pull/3325)。
...
- **灵活的翻译流程**：支持 Git（单体仓库、分支或子模块）、SaaS 软件、FTP 等多种方式
- **灵活的部署选项**：支持单域名、多域名或混合部署
- **模块化**：允许插件开发者提供国际化支持
- **低运行时开销**：文档主要是静态内容，无需重型 JS 库或 polyfill
- **可扩展的构建时间**：支持独立构建和部署本地化站点
- **资源本地化**：站点图片中可能包含需要翻译的文本
- **无耦合**：不强制使用任何 SaaS，但支持集成
- **与 [Crowdin](https://crowdin.com/) 易集成**：许多 Docusaurus v1 站点使用 Crowdin 并应能迁移到 v2
- **良好的 SEO 默认设置**：自动设置有用的 SEO 头信息如 [`hreflang`](https://developers.google.com/search/docs/advanced/crawling/localized-versions)
- **RTL 支持**：支持从右向左阅读的语言（阿拉伯语、希伯来语等）且易于实现
- **默认翻译**：经典主题标签已为[多种语言](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-translations/locales)提供翻译

### 非目标 {#i18n-non-goals}

我们不提供以下支持：

- **自动语言检测**：这属于主观决策，最好由[服务器（您的托管服务商）](../deployment.mdx)处理
- **翻译 SaaS 软件**：您需要自行了解所选外部工具
...
JSON文件可通过 [`docusaurus write-translations`](../cli.mdx#docusaurus-write-translations-sitedir) CLI命令初始化。各插件在对应目录下管理自身翻译内容，而 `code.json` 则定义React代码中使用的文本标签。

不同内容插件或主题的**翻译文件存放规则各异**：
