...
本手动迁移流程应在[自动化迁移流程](./migration-automated.mdx)之后执行，用于补全缺失部分或调试迁移CLI输出的问题。
...
这为官方包与社区维护包提供了明确区分。换言之，所有官方包都以`@docusaurus/`作为命名空间。
...
CLI命令现已重命名为`docusaurus <command>`（原为`docusaurus-command`）。
...
Docusaurus 2中简化为`website/build`目录。请确保部署配置已更新至正确读取新构建目录。

若部署至GitHub Pages，请运行`yarn deploy`命令而非原`yarn publish-gh-pages`脚本。
...
D1网站可能已有README文件，可修改其内容以适配D2变更，或直接复制[Docusaurus v2默认README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。
...
Docusaurus 2将各功能模块（博客、文档、页面）拆分为插件实现模块化。预设集是插件的组合包，为保持向后兼容性，我们提供了`@docusaurus/preset-classic`预设集，其中包含Docusaurus 1的大部分核心插件。
...
我们建议将 `docs` 文件夹移至 `website` 文件夹内，这也是 v2 版本的默认目录结构。[Vercel](https://vercel.com) 支持[开箱即用的 Docusaurus 项目部署](https://github.com/vercel/vercel/tree/main/examples/docusaurus)，前提是 `docs` 目录位于 `website` 内部。将文档与网站代码集中存放在同一个 `website` 目录下通常也更利于项目管理。

如果您正在迁移 Docusaurus v1 网站且存在待处理的文档拉取请求，可暂时保留 `/docs` 文件夹的原始位置以避免冲突。
...
已弃用。我们为 Docusaurus 2 开发了名为 [Infima](https://infima.dev/) 的 CSS 框架，采用 CSS 变量实现主题化。文档尚未完善，待就绪后将更新此处说明。要覆盖 Infima 的 CSS 变量，请创建自定义 CSS 文件（如 `./src/css/custom.css`）并通过 `@docusaurus/preset-classic` 选项全局引入：
...
或使用下方工具生成网站色阶变量，复制到 `src/css/custom.css` 中：
...
站点元信息（如资源、SEO、版权信息）现由主题处理。需通过 `docusaurus.config.js` 中的 `themeConfig` 字段进行定制：
...
您的 Algolia DocSearch v1 配置（参见[此处](https://github.com/algolia/docsearch-configs/blob/master/configs)）需针对 Docusaurus v2 进行更新（[示例](https://github.com/algolia/docsearch-configs/tree/master/configs/docusaurus-2.json)）。

可联系 DocSearch 团队 (@shortcuts, @s-pace) 获取支持。他们可协助更新配置并触发站点重新爬取以恢复搜索功能（否则需等待最长 24 小时的下次计划爬取）
...
已弃用。请在项目的 `static` 文件夹中创建 `CNAME` 文件并填写您的自定义域名。执行构建命令时，`static` 文件夹中的文件会被复制到 `build` 文件夹的根目录下。
...
- `defaultVersionShown` - 版本功能尚未移植。若您正在使用版本功能，暂时无法迁移至Docusaurus 2。请保持关注。
- `disableHeaderTitle`
- `disableTitleTagline`
...
- `markdownOptions` - v2采用MDX替代Remarkable，需将Markdown配置转换为Remark/Rehype插件。
- `markdownPlugins` - v2采用MDX替代Remarkable，需将Markdown插件转换为Remark/Rehype插件。
- `manifest`
- `onPageNav` - 现已默认开启。
- `separateCss` - 可参照前文 `custom.css` 的方式导入。
- `scrollToTop`
- `scrollToTopOptions`
...
当 [`cleanUrl`](https://v1.docusaurus.io/docs/en/site-config#cleanurl-boolean) 设置为：
...
若您在v1中设置了 `cleanUrl: false`，可能存在用户发布过 `/installation.html` 的链接。

出于SEO考虑和避免链接失效，应在托管服务商处配置服务端重定向规则。
...
旧版本中不允许嵌套侧边栏分类且分类仅能包含文档ID。而v2版本支持无限嵌套侧边栏，并提供多种[侧边栏项类型](../../guides/docs/sidebar/items.mdx)（不仅限于文档）。
...
该操作会将当前主题使用的`<Footer />`组件复制到站点根目录下的`src/theme/Footer`目录中，随后可编辑此组件进行自定义。

不要仅为在左侧添加Logo而swizzle页脚组件。v2版本已刻意移除左侧Logo并移至底部，直接在`docusaurus.config.js`中通过`themeConfig.footer`配置即可：
...
请参阅[创建页面](guides/creating-pages.mdx)了解Docusaurus 2的页面机制。注意需将v1版本中`pages/en`目录下的文件移至`src/pages`目录。
...
v2已弃用`CompLibrary`，需自行编写React组件或使用Infima样式（相关文档即将发布，目前可检查V2网站或查看https://infima.dev/了解可用样式）。
...
Docusaurus 2中Markdown语法已改为[MDX](https://mdxjs.com/)，因此需更新现有文档中可能存在的无效语法。常见示例如HTML中有效的自闭合标签`<img>`和`<br>`，现在需显式闭合为`<img/>`和`<br/>`。MDX文档中所有标签必须符合JSX规范。
...
参见[多语言代码块支持](../../guides/markdown-features/markdown-features-code-blocks.mdx#multi-language-support-code-blocks)章节。

### Front matter 元数据 {#front-matter}

博客的 Docusaurus front matter 字段已从驼峰式改为蛇形命名法，与文档规范保持一致。

字段 `authorFBID` 和 `authorTwitter` 已弃用。这些字段仅用于生成作者头像，现可通过 `authors` 字段实现。
