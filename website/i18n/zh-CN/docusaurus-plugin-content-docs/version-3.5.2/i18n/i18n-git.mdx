...
以下是通过 Git 将新初始化的英文 Docusaurus 网站翻译为法文的逐步指南，假设您已按照[国际化教程](./i18n-tutorial.mdx)操作。
...
使用 `--messagePrefix '(fr) '` 选项使未翻译的字符串突出显示。
...
保持翻译文件与原始文件**同步可能具有挑战性**，尤其是 Markdown 文档。

### Markdown 翻译 {#markdown-translations}

当未翻译的 Markdown 文档被编辑时，**您有责任维护相应的翻译文件**，遗憾的是我们目前无法提供有效帮助。
...
若翻译文件托管在 Git 上，可通过文档和博客插件的 `editLocalizedFiles: true` 选项进行配置。
