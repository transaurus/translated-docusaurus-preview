...
本节内容与[样式与布局](./styling-layout.mdx)类似，但这次我们将定制React组件本身，而非其外观样式。我们将讨论Docusaurus的核心概念——**组件置换(Swizzling)**，该功能允许实现**更深层次的站点定制**。
...
置换组件后，**需重启开发服务器**以使Docusaurus识别新组件。

:::

:::warning[建议保持安全边界]

请务必理解[哪些组件可安全置换](#what-is-safe-to-swizzle)。部分组件属于主题的**内部实现细节**。

:::

:::info

`docusaurus swizzle`仅是辅助置换的自动化工具。您也可以手动创建`src/theme/SomeComponent.js`文件，Docusaurus会[自动解析](./advanced/client.mdx#theme-aliases)。该命令背后并无特殊机制！

:::

### 弹出(Ejecting) {#ejecting}

弹出主题组件是指**创建原始组件的副本**，从而进行**完全自定义和覆盖**的过程。

要弹出主题组件，可通过交互式swizzle CLI或使用`--eject`选项：
...
该操作会将当前`<Footer />`组件的实现复制到站点的`src/theme`目录。此后Docusaurus将使用该副本而非原始组件。您现在可以完全重新实现`<Footer>`组件。
...
弹出[**不安全**](#what-is-safe-to-swizzle)组件可能导致复制大量需自行维护的内部代码。这会使Docusaurus升级更困难，若组件接收的props或使用的内部主题API发生变化，您需要手动迁移定制内容。

**尽可能选择[包装(Wrapping)](#wrapping)**：需维护的代码量更少。

:::

:::tip[重新置换]

Docusaurus升级后，可通过重新运行弹出命令并使用`git diff`比对变更来保持组件更新。建议在文件顶部添加注释说明您的修改内容，以便在重新弹出后快速重应用改。
...
包装主题是**[弹出组件](#ejecting)的绝佳替代方案**，能在不破坏原始组件的情况下**扩展功能**。例如轻松为每篇博客添加评论系统：
...
部分主题组件属于主题的**内部实现细节**。虽然Docusaurus允许替换，但存在**潜在风险**。
...
不必对**替换不安全组件过度担忧**，只需注意**破坏性变更**可能发生，在次版本升级时需要手动调整定制内容。
...
## 组件选择策略 {#which-component-should-i-swizzle}

有时难以确定具体应替换哪个组件才能实现预期效果。提供大多数主题组件的`@docusaurus/theme-classic`包含约[100个组件](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)！
...
2. **组件命名**。官方主题组件采用语义化命名，通常可通过名称推断功能。使用交互式swizzle CLI时，输入组件名称片段即可筛选相关选项。例如执行`yarn swizzle @docusaurus/theme-classic`后输入`Doc`，将仅列出与文档相关的组件。
3. **从高层级组件入手**。组件以树状结构相互引用，每个路由会关联一个顶级渲染组件（多数列于[内容插件路由](./advanced/routing.mdx#routing-in-content-plugins)）。例如所有博客文章页面的最顶层组件都是`@theme/BlogPostPage`。可先swizzle该组件，再沿组件树向下定位具体目标组件。定位后请删除其他临时swizzle的组件文件，避免维护过多组件。
...
2. **利用国际化机制**。虽然看似意外，但翻译本质上是定制文本标签的有效方式。例如站点默认语言为`en`时，仍可执行`yarn write-translations -l en`后编辑生成的`code.json`文件，详见[i18n教程](./i18n/i18n-tutorial.mdx)。

:::tip

**组件越小越好**。若必须swizzle，请仅修改相关部分并尽量减少自主维护的代码量。swizzle小型组件通常能降低升级时的**破坏性变更**风险。
...
`<Root>`组件在React树的最顶层渲染，位于主题`<Layout>`之上且**永不卸载**，是添加跨导航状态逻辑（用户认证状态、购物车状态等）的理想位置。
