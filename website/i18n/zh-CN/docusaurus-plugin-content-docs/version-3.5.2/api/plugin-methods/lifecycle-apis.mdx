...
在构建过程中，插件会并行加载以获取其自身内容并将其渲染为路由。插件还可以配置 webpack 或对生成的文件进行后处理。
...
例如，下面的插件创建了一个 `/friends` 页面，显示 `你的朋友是：Yangshun, Sebastien`：
...
`configureWebpack` 的 API 将在未来修改为接收一个对象（`configureWebpack({config, isServer, utils, content})`）
...
`configureWebpack` 会在服务端构建和客户端构建时被调用。服务端构建时传入 `true`，客户端构建时传入 `false` 作为 `isServer` 参数。
...
您可以使用这些工具函数来有条件地返回 webpack 配置。

例如，下面的插件通过修改 webpack 配置来转译 `.foo` 文件。
...
向 Docusaurus 生成的 HTML 中注入 head 和/或 body 标签。
...
- `headTags` 会在 `</head>` 闭合标签前插入，位于配置添加的脚本之后。
- `preBodyTags` 会在 `<body>` 开始标签后插入，位于所有子元素之前。
- `postBodyTags` 会在 `</body>` 闭合标签前插入，位于所有子元素之后。

## `getClientModules()` {#getClientModules}

返回要导入客户端包的[客户端模块](../../advanced/client.mdx#client-modules)路径数组。

例如，要让您的主题加载用户通过 `options` 传入的 `customCss` 或 `customJs` 文件路径：
