...
这些文件将由 [`write-translations` CLI](../../cli.mdx#docusaurus-write-translations-sitedir) 写入插件i18n子文件夹，并在调用 [`translateContent()`](#translateContent) 和 [`translateThemeConfig()`](#translateThemeConfig) 之前读取对应语言环境的翻译内容。
...
应返回 `Record<string, string>` 格式的消息，其中键是翻译ID，值是根据站点当前语言环境本地化的消息（不包含描述）。
