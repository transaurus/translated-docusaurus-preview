...
[订阅功能](../../blog.mdx#feed)通过解析构建输出实现，**仅在生产环境生效**。
...
用于通过 [XSLT](https://developer.mozilla.org/en-US/docs/Web/XSLT) 美化博客 XML 订阅源的浏览器渲染样式。

- 设为 `true` 时使用插件内置的 `.xsl` 和 `.css` 文件
- 设为假值（`undefined | null | false`）时禁用此功能
- 设为字符串时指定博客内容目录下自定义 `.xsl` 文件路径（需配套同名 `.css` 文件）
...
## Markdown 扉页元数据 {#markdown-front-matter}

Markdown 文档可使用以下被 `---` 分隔线包裹的 [扉页元数据](../../guides/markdown-features/markdown-features-intro.mdx#front-matter) 字段。
...
默认会在博客内容目录根路径查找 `authors.yml` 文件。

该文件可包含预定义的 [全局博客作者](../../blog.mdx#global-authors)，在 Markdown 文件中可通过 [`authors` 扉页元数据](#markdown-front-matter) 引用这些作者键名。

### 类型定义 {#authors-file-types}

作者文件的 YAML 内容需符合以下结构：
