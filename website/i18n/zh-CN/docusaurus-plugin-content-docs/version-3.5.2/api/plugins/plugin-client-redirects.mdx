...
该插件会向静态站点写入额外的HTML页面，这些页面通过JavaScript将用户重定向至现有的Docusaurus页面。
...
本插件还会读取[`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes)配置，当多个文件将生成至相同位置时调整其日志级别。
...
"from"与"to"的概念是本插件的核心。"from"表示您希望创建的路径（即待生成的额外HTML文件）；"to"表示您希望重定向至的路径（通常是Docusaurus已识别的路由）。

因此可以存在多个"from"指向同一个"to"（生成多个重定向至相同目标的HTML文件），但一个"from"绝不能对应多个"to"（生成的HTML文件必须具有确定的重定向目标）。
