...
这将启用 `@docusaurus` ESLint 插件并使用 `recommended` 配置。参阅下方的[支持规则](#supported-rules)了解该配置启用的规则列表。

### 手动配置

如需更细粒度的控制，你也可以手动启用插件并直接配置需要使用的规则：
...
- 全部规则：启用**所有**规则。该配置会在次要版本间发生变化，若希望避免意外破坏性变更，不建议使用。
