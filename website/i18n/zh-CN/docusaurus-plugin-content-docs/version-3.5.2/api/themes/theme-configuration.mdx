...
当`respectPrefersColorScheme: true`时，`defaultMode`会被用户系统偏好设置覆盖。
...
标志可放置在[静态文件夹](static-assets.mdx)中。标志URL默认设置为站点根路径。虽然可以自定义标志URL，但如果是外部链接将在新标签页打开。此外可覆盖标志链接的target属性值，当文档网站作为主站点的子目录时特别有用——这种情况下可能不需要标志链接在主站新标签页打开。
...
根据`type`字段不同，项目可具有不同行为。以下章节将介绍所有可用的导航栏项目类型。

#### 导航栏链接 {#navbar-link}

默认情况下，导航栏项目是常规链接（内部或外部）。

React Router 应自动为链接应用活动链接样式，但在边缘情况下可使用 `activeBasePath`。若某链接需在多个不同路径下保持活动状态（例如同一侧边栏下有多个文档文件夹），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更灵活的替代方案且优先级更高——Docusaurus 会将其解析为正则表达式来匹配当前URL。
...
若使用带版本控制的文档，此特殊导航栏项目类型会渲染包含所有可用版本的下拉菜单。

用户可在保持当前文档不变的情况下切换版本（前提是文档ID在不同版本间保持一致）。
...
若使用带版本控制的文档，此特殊导航栏项目类型会链接至当前浏览的文档版本（取决于URL），并回退至最新版本。
...
若启用了[国际化功能](../../i18n/i18n-introduction.mdx)，此特殊导航栏项目类型将渲染包含所有可用语言的下拉菜单。
...
您也可以使用此导航栏项目类型，在导航项中渲染自定义HTML标记。
...
当前支持两种样式选项：`dark`（暗色）和基于`--ifm-color-primary`颜色的`primary`（主色）。样式预览可参见[Infima文档](https://infima.dev/docs/components/navbar/)。
...
若使用行高亮Markdown语法，可能需要为暗色模式语法高亮主题指定不同的高亮背景色。具体请参考[行高亮指南文档](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)。

:::

### 默认语言 {#default-language}

可为未标注语言的代码块（即仅以```开头的代码块）设置默认语言。注意必须传递有效的[语言名称](https://prismjs.com/#supported-languages)。
...
用于访问颜色模式的React钩子。该上下文包含设置浅色/深色模式的函数，并暴露当前使用模式的布尔值变量。
