...
但插件可以通过在配置中使用"Babel风格"的双元素元组（包含插件名称和选项对象）来指定选项。
...
所有 Docusaurus 内容插件都支持多实例运行。例如，使用[多个文档插件实例](./guides/docs/docs-multi-instance.mdx)或[多个博客](./blog.mdx#multiple-blogs)可能很有用。必须为每个插件实例分配唯一ID，默认情况下插件ID为 `default`。
...
`themes` 和 `plugins` 选项会导致不同的[模块简写解析规则](#module-shorthands)，因此如果您想利用简写功能，请确保使用正确的条目！
...
预设（Presets）是插件和主题的集合包。例如，无需在配置文件中逐个注册和配置`@docusaurus/plugin-content-docs`、`@docusaurus/plugin-content-blog`等插件，通过`@docusaurus/preset-classic`预设，您可以在一个集中位置统一配置它们。

### `@docusaurus/preset-classic` {#docusauruspreset-classic}

经典预设默认随[`create-docusaurus`](./installation.mdx#scaffold-project-website)创建的新建Docusaurus网站提供，包含以下主题和插件：
...
然后，在您站点的`docusaurus.config.js`文件的`presets`选项中添加：
...
预设是一个与[插件构造函数](./api/plugin-methods/README.mdx#plugin-constructor)结构相同的函数。它应返回一个`{ plugins: PluginConfig[], themes: PluginConfig[] }`对象，其格式与站点配置中接受的格式相同。例如，您可以指定一个包含以下主题和插件的预设：
...
当某些插件和主题需要配合使用时，这种方式特别有用。您甚至可以将它们的配置选项关联起来，例如将一个选项传递给多个插件。

## 模块简写 {#module-shorthands}

Docusaurus支持插件、主题和预设的简写形式。当它识别到一个插件/主题/预设名称时，会按以下顺序尝试加载：

- `[name]`（如`content-docs`）
- `@docusaurus/[moduleType]-[name]`（如`@docusaurus/plugin-content-docs`）
- `docusaurus-[moduleType]-[name]`（如`docusaurus-plugin-content-docs`）

其中`moduleType`根据模块名称声明字段的不同，可能是`'preset'`、`'theme'`或`'plugin'`。系统会加载第一个成功找到的模块名称。

如果名称带有作用域（以`@`开头），系统会首先通过第一个斜杠将名称拆分为作用域和包名：
...
如果名称不带作用域（如`@jquery`），则会加载`[scope]/docusaurus-[moduleType]`（即`@jquery/docusaurus-plugin`）。否则将尝试以下加载顺序：

- `[scope]/[name]`（如`@jquery/content-docs`）
- `[scope]/docusaurus-[moduleType]-[name]`（如`@jquery/docusaurus-plugin-content-docs`）

以下是在`plugins`字段中注册插件的示例。请注意，与[ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins)或[Babel](https://babeljs.io/docs/en/options#name-normalization)强制要求插件命名规范不同，Docusaurus允许更自由的命名方式，因此解析结果并非绝对，但会遵循上述优先级规则。
