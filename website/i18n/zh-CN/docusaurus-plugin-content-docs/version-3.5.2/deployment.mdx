...
后续章节将介绍常见托管服务商及其高效部署 Docusaurus 站点的配置方法。Docusaurus 与这些服务无商业关联，本文档仅作信息参考。部分配置说明由第三方提供，可能未及时同步服务商API变更。若发现内容过时，欢迎提交PR更新。

由于我们仅能提供有限维护，现已停止接受新增托管方案的PR。您可将部署指南发布在独立站点（如个人博客或服务商官网），并联系我们添加相关链接。
...
若未预先配置，可在站点创建后通过"Site settings" -> "Build & deploy"补设。

正确配置后，站点将在合并到部署分支（默认为`main`）时自动部署/重新部署。

:::warning

部分Docusaurus项目采用非标准目录结构（常见于v1版本）：
...
若将`website`设为Netlify基础目录，更新`docs`文件夹时不会触发构建，需配置[自定义忽略命令](https://docs.netlify.com/configure-builds/common-configurations/ignore-builds/)：
...
默认情况下Netlify会为URL添加尾部斜杠。

建议关闭Netlify设置`Post Processing > Asset Optimization > Pretty Urls`以避免URL小写化、多余重定向及404错误。

**特别注意**：全局复选框`Disable asset optimization`存在缺陷，实际不会禁用`Pretty URLs`设置。请务必单独取消勾选该选项。

如需保留Netlify的`Pretty Urls`设置，请相应调整Docusaurus的`trailingSlash`配置。

更多信息参考[slorber/trailing-slash-guide](https://github.com/slorber/trailing-slash-guide)。

:::

## 部署至 Vercel {#deploying-to-vercel}

将项目部署到 [Vercel](https://vercel.com/) 可获得[性能优化](https://vercel.com/)和易用性方面的多重优势。

通过 [Vercel Git集成功能](https://vercel.com/docs/concepts/git)部署前，请确保代码已推送至Git仓库。
...
通常发布流程涉及两个仓库（至少两个分支）：包含源文件的分支，以及包含构建输出供 GitHub Pages 服务使用的分支。下文教程中分别称为 **"源分支"** 和 **"部署分支"**。

每个 GitHub 仓库都关联一个 GitHub Pages 服务。若部署仓库名为 `my-org/my-project`（`my-org`为组织名或用户名），部署后的站点将出现在 `https://my-org.github.io/my-project/`。若部署仓库名为 `my-org/my-org.github.io`（组织专属 GitHub Pages 仓库），站点将位于 `https://my-org.github.io/`。
...
GitHub Pages 会从默认分支（通常为 `master`/`main`）或 `gh-pages` 分支的根目录/`docs` 文件夹获取部署就绪的文件（`docusaurus build` 的输出）。可通过仓库的 `Settings > Pages` 进行配置，该分支称为"部署分支"。
...
GitHub Pages 默认会通过 [Jekyll](https://jekyllrb.com/) 处理发布文件。由于 Jekyll 会忽略以下划线 `_` 开头的文件，建议在 `static` 目录添加名为 `.nojekyll` 的空文件来禁用此功能。
...
GitHub 企业版安装与 github.com 操作方式相同，只需将组织的 GitHub Enterprise 主机设为环境变量：
...
自 2021 年 8 月起，GitHub 要求所有命令行登录必须使用 **个人访问令牌** 而非密码。当 GitHub 提示输入密码时，请改为输入 PAT。更多信息请参阅 [GitHub 文档](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token)。

或者，您也可以使用 SSH（`USE_SSH=true`）登录。

:::

### 使用 GitHub Actions 触发部署 {#triggering-deployment-with-github-actions}

[GitHub Actions](https://help.github.com/en/actions) 允许您在代码仓库中自动化、定制和执行软件开发工作流。

以下工作流示例假设您的网站源代码位于仓库的 `main` 分支（_源分支_ 为 `main`），并且您的 [发布源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site) 已配置为 [使用自定义 GitHub Actions 工作流发布](https://docs.github.com/en/pages/getting-started-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site#publishing-with-a-custom-github-actions-workflow)。

我们的目标是：

1. 当向 `main` 分支提交新的拉取请求时，有一个动作确保站点构建成功，但不会实际部署。此任务称为 `test-deploy`。
2. 当拉取请求合并到 `main` 分支或有人直接推送到 `main` 分支时，将构建并部署到 GitHub Pages。此任务称为 `deploy`。

以下是使用 GitHub Actions 部署文档的两种方法。根据部署仓库的位置，选择下方相关标签：

- 源仓库和部署仓库为 **同一** 仓库。
- 部署仓库为 **远程** 仓库，与源仓库不同。此场景的说明假设 [发布源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site) 为 `gh-pages` 分支。
...
持续集成（CI）服务通常用于在提交新代码到源代码控制时执行例行任务。这些任务可以是运行单元测试和集成测试、自动化构建、将包发布到 npm 以及将更改部署到您的网站的任何组合。要自动化网站的部署，您只需在网站更新时调用 `yarn deploy` 脚本。以下部分介绍如何使用流行的持续集成服务提供商 [Travis CI](https://travis-ci.com/) 实现这一点。

1. 访问 https://github.com/settings/tokens 并生成一个新的 [个人访问令牌](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)。创建令牌时，授予其 `repo` 范围以确保其拥有所需权限。
2. 使用您的 GitHub 账户，[添加 Travis CI 应用](https://github.com/marketplace/travis-ci) 到您想要激活的仓库。
3. 打开您的 Travis CI 仪表板（URL 类似 `https://travis-ci.com/USERNAME/REPO`），导航到仓库的 `More options > Setting > Environment Variables` 部分。
4. 创建一个名为 `GH_TOKEN` 的新环境变量，其值为您新生成的令牌，然后创建 `GH_EMAIL`（您的电子邮件地址）和 `GH_NAME`（您的 GitHub 用户名）。
5. 在仓库根目录创建一个 `.travis.yml` 文件，内容如下：
...
1. 访问 https://github.com/settings/tokens 生成新的[个人访问令牌](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)，创建时需授予 `repo` 权限范围。
2. 登录 Buddy 账户并创建新项目。
...
完成该简单流水线创建后，每次向选定分支推送新提交时，系统将通过 `yarn deploy` 将网站部署至 GitHub Pages。查阅[本指南](https://buddy.works/guides/react-docusaurus)了解如何为 Docusaurus 设置 CI/CD 流水线。
...
4. 在项目页面（形如 `https://dev.azure.com/ORG_NAME/REPO_NAME/_build`）中，使用以下文本创建新流水线。点击编辑添加名为 `GH_TOKEN` 的环境变量（值为新生成的令牌），以及 `GH_EMAIL`（您的邮箱）和 `GH_NAME`（GitHub 用户名），务必标记为加密变量。也可在仓库根目录创建 `azure-pipelines.yml` 文件。
...
1. 生成新的 SSH 密钥作为项目[部署密钥](https://docs.github.com/en/free-pro-team@latest/developers/overview/managing-deploy-keys#deploy-keys)。
...
5. 点击仓库激活后，添加名为 `git_deploy_private_key` 的加密变量并填入私钥值。
...
此后每当向 GitHub 推送新标签时，将触发 Drone CI 任务发布网站。

## 部署到 Flightcontrol {#deploying-to-flightcontrol}

[Flightcontrol](https://www.flightcontrol.dev/?ref=docusaurus) 服务可直接从 Git 仓库将 Web 应用构建并部署至 AWS Fargate，在摆脱传统 PaaS 限制的同时提供完整的基础设施检查与修改权限。
...
[Render](https://render.com) 提供 [免费的静态网站托管服务](https://render.com/docs/static-sites)，包括全托管的 SSL、自定义域名、全球 CDN 以及从 Git 仓库持续自动部署。按照 [Render 的 Docusaurus 部署指南](https://render.com/docs/deploy-docusaurus) 只需几分钟即可开始。

## 部署到 Qovery {#deploying-to-qovery}

[Qovery](https://www.qovery.com) 是一个完全托管的云平台，运行在您的 AWS、Digital Ocean 和 Scaleway 账户上，您可以在一个地方托管静态网站、后端 API、数据库、定时任务和所有其他应用程序。
...
4. 添加一个应用程序。
   - 点击 **创建应用程序**，命名并选择您的 GitHub 或 GitLab 仓库（Docusaurus 应用所在位置）。
   - 定义主分支名称和应用程序根路径。
   - 点击 **创建**。应用程序创建完成后：
   - 导航到您的应用程序 **设置**
   - 选择 **端口**
...
完成。查看状态并等待应用程序部署完成。要在浏览器中打开应用程序，请在应用程序概览中点击 **操作** 和 **打开**。
...
2. 选择要部署的项目。
   ...
   - 如果您使用 GitHub、GitLab 或 Bitbucket 账户登录 Hostman，您将看到包含您项目的代码库，包括私有项目。

   - 选择您想要部署的项目。该项目必须包含项目文件的目录（例如 `website`）。

   - 如需访问其他代码库，请点击**连接其他代码库**。
...
3. 配置构建设置。
   ...
   - **应用目录**指向构建后包含项目文件的目录。如果在步骤 2 中选择了包含网站内容（或 `my_website`）目录的代码库，可以留空。
...
Surge 是一个[静态网站托管平台](https://surge.sh/help/getting-started-with-surge)，您可以通过命令行在几秒内部署 Docusaurus 项目。部署到 Surge 简单且免费（包括自定义域名和 SSL 证书）。

通过以下步骤，您可以在几秒内使用 Surge 部署应用：
...
您的站点现已免费部署在 `subdomain.surge.sh` 或 `your-domain.com`，具体取决于您选择的方法。
...
更多部署到 QuantCDN 的示例和用例，请参阅[文档](https://docs.quantcdn.io/docs/cli/continuous-integration)和[博客](https://www.quantcdn.io/blog)。

## 部署到 Layer0 {#deploying-to-layer0}

[Layer0](https://www.layer0.co) 是一个一体化平台，用于开发、部署、预览、实验、监控和运行无头前端。它专注于大型动态网站，通过 EdgeJS（基于 JavaScript 的内容分发网络）、预测性预取和性能监控提供一流的性能。Layer0 提供免费套餐。按照 [Layer0 的 Docusaurus 部署指南](https://docs.layer0.co/guides/docusaurus)，几分钟即可开始使用。

## 部署到 Cloudflare Pages {#deploying-to-cloudflare-pages}

[Cloudflare Pages](https://pages.cloudflare.com/) 是一个面向前端开发者的 Jamstack 平台，用于协作和部署网站。按照[这篇文章](https://dev.to/apidev234/deploying-docusaurus-to-cloudflare-pages-565g)，几分钟即可开始使用。

## 部署到 Azure Static Web Apps {#deploying-to-azure-static-web-apps}

[Azure Static Web Apps](https://docs.microsoft.com/en-us/azure/static-web-apps/overview) 是一项服务，可直接从代码仓库自动构建和部署全栈 Web 应用到 Azure，简化了 CI/CD 的开发者体验。Static Web Apps 将 Web 应用的静态资源与其动态（API）端点分离。静态资源从全球分布的内容服务器提供，使客户端能够使用附近的服务器更快地检索文件。动态 API 使用基于事件驱动的无服务器架构进行扩展，这种方法更具成本效益并按需扩展。按照[这份分步指南](https://dev.to/azure/11-share-content-with-docusaurus-azure-static-web-apps-30hc)，几分钟即可开始使用。

## 部署到 Kinsta {#deploying-to-kinsta}

[Kinsta 静态站点托管](https://kinsta.com/static-site-hosting) 允许您免费部署多达 100 个静态站点，提供自定义域名和 SSL、每月 100 GB 带宽以及 260 多个 Cloudflare CDN 位置。

按照我们的 [Kinsta 上的 Docusaurus](https://kinsta.com/docs/docusaurus-example/) 文章，只需点击几下即可开始使用。