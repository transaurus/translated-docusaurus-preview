...
1. _如何在侧边栏X生成文档Y的链接，但不在Y页面显示侧边栏X？_ 例如当文档Y被同时添加到多个侧边栏时，如何明确指定Docusaurus显示特定侧边栏？
...
Docusaurus利用侧边栏在文档底部生成"上一篇/下一篇"分页链接。它严格使用当前显示的侧边栏：若未关联侧边栏，则不会生成分页。但被列为"上一篇/下一篇"的文档可能显示不同侧边栏：它们虽属于当前侧边栏，但其front matter可能设置了不同的`displayed_sidebar`。
...
"windows"页面的下一篇链接指向"linux"并不合理：您可能希望读者在安装后跳转至"getting started"。此时可手动设置分页：
