...
有时您需要直接从 Markdown 文件链接到静态资源（如 docx 文档、图片等），将资源文件与使用它的 Markdown 文件放在同一目录下会非常方便。
...
如果使用 [@docusaurus/plugin-ideal-image](../../api/plugins/plugin-ideal-image.mdx) 插件，需按照文档使用专用图片组件。
...
当需要通过 CSS 修改 SVG 局部样式时（例如根据当前主题切换 SVG 某部分颜色），此功能非常实用。
...
GitHub 采用其特有的[图片主题化方案](https://github.blog/changelog/2021-11-24-specify-theme-context-for-images-in-markdown/)，通过路径片段实现，您可以轻松实现类似效果。

要通过路径片段（GitHub 使用 `#gh-dark-mode-only` 和 `#gh-light-mode-only`）控制图片显隐，请在自定义 CSS 中添加以下代码（若不希望与 GitHub 强耦合，也可使用自定义后缀）：
...
若 Markdown 链接或图片使用绝对路径，该路径将被视为文件路径并从静态资源目录解析。例如配置[静态目录](../../static-assets.mdx)为 `['public', 'static']` 时，对于以下图片：
...
2. 图片会进入Webpack构建流程，其文件名会被附加哈希值，这使得浏览器能积极缓存图片并提升网站性能。
