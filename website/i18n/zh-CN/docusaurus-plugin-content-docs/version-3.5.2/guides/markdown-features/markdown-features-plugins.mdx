...
使用插件为项目中常用的 JSX 元素引入简写语法。我们提供的[告示语法](./markdown-features-admonitions.mdx)也是通过 Remark 插件生成的，您可以为自己的用例实现类似功能。
...
许多官方 Remark/Rehype 插件仅支持 [ES Modules](https://gist.github.com/sindresorhus/a39789f98801d908bbc7ff3ecc99d99c) 模块系统（Docusaurus 支持该标准）。我们建议使用 [ES Modules 配置文件](https://flaviocopes.com/es-modules/)以便更轻松地导入这类包。
...
某些插件支持配置并接受专属选项。此时需使用 `[plugin, pluginOptions]` 语法，例如：
...
例如，我们创建一个为所有二级标题添加"第X节"前缀的插件。首先在任意位置创建插件源文件（也可发布为独立npm包并通过上述方式安装），此处我们将插件放在`src/remark/section-prefix.js`。remark/rehype插件本质上是一个接收`options`参数并返回AST转换器的函数。
...
Docusaurus默认插件会优先于自定义remark插件执行，这意味着图片/链接在被自定义插件处理前已通过`require()`转换为JSX。例如上例中，虽然所有二级标题都被添加了"第X节"前缀，但生成的目录仍保持原样，因为目录生成插件先于我们的自定义插件执行。若需在默认插件前处理MDAST，请使用`beforeDefaultRemarkPlugins`和`beforeDefaultRehypePlugins`：
