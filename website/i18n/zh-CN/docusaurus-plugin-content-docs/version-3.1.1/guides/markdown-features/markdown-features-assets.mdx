...
使用 Markdown 图片或链接语法时，所有资源路径都会被 Docusaurus 解析为文件路径并自动转换为 `require()` 调用。除非使用 JSX 语法（此时需自行处理），否则无需在 Markdown 中显式使用 `require()`。
...
当需要通过 CSS 修改 SVG 部分元素时（例如根据当前主题切换 SVG 颜色），这种方式非常有用。
...
Docusaurus 支持主题化图片：内置的 `ThemedImage` 组件允许根据当前主题切换图片源。
...
### GitHub 风格主题化图片 {#github-style-themed-images}

GitHub 采用独特的[图片主题化方案](https://github.blog/changelog/2021-11-24-specify-theme-context-for-images-in-markdown/)，通过路径片段实现，您可以轻松复现该方案。

要通过路径片段（GitHub 使用 `#gh-dark-mode-only` 和 `#gh-light-mode-only`）控制图片显隐，请在自定义 CSS 中添加以下代码（若不希望与 GitHub 耦合，也可使用自定义后缀）：
...
当 Markdown 链接或图片使用绝对路径时，该路径会被视为文件路径，并从静态资源目录解析。例如，若您配置的[静态目录](../../static-assets.mdx)为 `['public', 'static']`，则对于以下图片：

```md title="my-doc.md"
![An image from the static](/img/docusaurus.png)
```

Docusaurus 会依次在 `static/img/docusaurus.png` 和 `public/img/docusaurus.png` 路径查找。该链接最终会被转换为 `require()` 调用而非保留为 URL，这种机制在以下两方面具有优势：

1. 您无需担心基础URL问题，Docusaurus在提供资源时会自动处理；
2. 图片会进入Webpack的构建流程，其名称会被附加哈希值，这使得浏览器能够积极缓存图片并提升网站性能。
