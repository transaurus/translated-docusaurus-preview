...
Docusaurus v2 的国际化机制在概念上与 v1 版本较为相似，但存在若干关键差异。

新版不再与 Crowdin 深度耦合，您可以选择 Git 或其他 SaaS 方案替代。
...
针对使用 Crowdin 的 Docusaurus v1 站点，存在多种迁移策略，各有利弊。

:::warning

本文档为最佳实践指南，如您发现更优方案，请协助我们改进！

:::

在开始前，我们建议您：

- 先完成无翻译内容的 v1 至 v2 基础迁移
...
遗憾的是，Crowdin 不提供"项目复制/克隆"功能，这使得操作过程较为复杂。
...
- 首次尝试选择"100%匹配"（比"完全匹配"能翻译更多内容），对源文件进行预翻译
- 下载Crowdin翻译到本地
- 尝试运行/构建站点并检查是否存在错误

首次尝试时很可能会出现错误：预翻译可能试图翻译不应翻译的内容（front matter、告示块、代码块等），且翻译后的MD文件可能不符合MDX解析器规范。
...
该解析器版本未公开文档，您需要联系Crowdin支持以确认项目使用的解析器版本并固定特定版本。
