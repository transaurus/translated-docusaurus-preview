...
这为官方包与社区维护包提供了明确区分。换言之，所有Docusaurus官方包都归属于`@docusaurus/`命名空间。
...
`package.json`中的`"scripts"`部分应按如下方式更新：
...
Docusaurus 2中已简化为`website/build`。请确保更新部署配置以从正确的`build`目录读取生成文件。
...
D1网站可能已有README文件。可修改其内容以反映D2变更，或复制默认的[Docusaurus v2 README](https://github.com/facebook/docusaurus/blob/main/packages/create-docusaurus/templates/shared/README.md)。
...
Docusaurus 2将各功能（博客、文档、页面）拆分为插件以实现模块化。预设是插件的组合包，为保持向后兼容性，我们构建了`@docusaurus/preset-classic`预设，其中包含Docusaurus 1中的大部分核心插件。
...
已弃用。我们为 Docusaurus 2 开发了名为 [Infima](https://infima.dev/) 的 CSS 框架，使用 CSS 变量实现主题功能。相关文档尚未完善，后续会更新说明。要覆盖 Infima 的 CSS 变量，请创建自定义 CSS 文件（如 `./src/css/custom.css`）并通过 `@docusaurus/preset-classic` 的配置选项全局引入：
...
也可使用下方工具生成网站配色方案，将变量复制到 `src/css/custom.css` 中：
...
站点元信息（如图标资源、SEO、版权信息）现由主题处理。要自定义这些内容，请在 `docusaurus.config.js` 中使用 `themeConfig` 字段：
...
可联系 DocSearch 团队 (@shortcuts, @s-pace) 获取支持。他们能协助更新配置并触发站点重新爬取以恢复搜索功能（否则需等待最长 24 小时的下次定时爬取）
...
已弃用。请在 `static` 文件夹中创建 `CNAME` 文件并填写您的自定义域名。执行构建命令时，`static` 文件夹中的文件将被复制到 `build` 文件夹的根目录中。
...
- `disableHeaderTitle`
- `disableTitleTagline`
- `docsSideNavCollapsible` 已移至 `docsPluginOptions.sidebarCollapsible`，且默认启用。
- `facebookAppId`
- `facebookComments`
- `facebookPixelId`
- `fonts`
...
- `manifest`
- `onPageNav` - 现已默认启用。
- `separateCss` - 可采用与上文 `custom.css` 相同的方式导入。
- `scrollToTop`
- `scrollToTopOptions`
...
若现有侧边栏包含分类类型需迁移：将`subcategory`重命名为`category`，`ids`改为`items`。
...
该操作会将当前主题使用的`<Footer />`组件复制到站点根目录下的`src/theme/Footer`目录，您可在此目录中编辑组件实现定制化。
...
请参阅[创建页面](guides/creating-pages.mdx)了解Docusaurus 2的页面机制。注意需将v1版本中`pages/en`目录下的文件迁移至`src/pages`目录。
...
Docusaurus 2采用[MDX](https://mdxjs.com/)语法，需更新现有文档中不兼容的语法。常见示例如HTML中有效的自闭合标签`<img>`和`<br>`，现在必须显式闭合（`<img/>`和`<br/>`）。MDX文档中所有标签必须符合JSX规范。

Front matter 由 [gray-matter](https://github.com/jonschlinkert/gray-matter) 解析。若 front matter 包含特殊字符（如 `:`），现在需用引号包裹：`title: Part 1: my part1 title` → `title: "Part 1: my part1 title"`。

**提示**：可使用 [HTML 转 JSX](https://transform.tools/html-to-jsx) 等在线工具简化迁移流程。

### 多语言代码标签 {#language-specific-code-tabs}

参阅[多语言代码块支持](../../guides/markdown-features/markdown-features-code-blocks.mdx#multi-language-support-code-blocks)章节。

### Front matter 规范 {#front-matter}

博客的 Docusaurus front matter 字段已从驼峰式更改为下划线式，以保持与文档的一致性。

字段 `authorFBID` 和 `authorTwitter` 已被弃用。它们仅用于生成作者头像，现可通过 `authors` 字段实现。

## 部署配置 {#deployment}

GitHub Pages 使用的 `CNAME` 文件不再自动生成，若使用自定义域名，请确保在 `/static/CNAME` 中手动创建。

博客 RSS 订阅地址变更为 `/blog/rss.xml`（原为 `/blog/feed.xml`）。建议配置服务端重定向以保持用户订阅有效性。
