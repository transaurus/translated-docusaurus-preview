...
更多背景信息可参阅初始 [RFC](https://github.com/facebook/docusaurus/issues/3317) 和 [PR](https://github.com/facebook/docusaurus/pull/3325)。
...
- **与 [Crowdin](https://crowdin.com/) 易用**：许多 Docusaurus v1 站点使用 Crowdin 并应能迁移到 v2
- **良好的 SEO 默认设置**：自动设置有用的 SEO 头如 [`hreflang`](https://developers.google.com/search/docs/advanced/crawling/localized-versions)
- **RTL 支持**：原生支持从右向左阅读的语言（阿拉伯语、希伯来语等）
- **默认翻译**：经典主题标签已提供[多种语言](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-translations/locales)的翻译

### 非目标 {#i18n-non-goals}

我们不提供以下支持：

- **自动语言检测**：这属于主观决策，最好由[服务器（托管服务商）](../deployment.mdx)处理
- **翻译 SaaS 软件**：您需自行了解所选外部工具
- **URL 别名翻译**：技术复杂且 SEO 价值有限
...
部分插件会从需要整体本地化的外部数据文件中读取内容。例如博客插件使用的[`authors.yml`](../blog.mdx#global-authors)文件，可通过在`i18n/[locale]/docusaurus-plugin-content-blog/`路径下创建副本实现翻译。
...
为简单Docusaurus网站添加法语翻译后，目录结构示例如下：
