...
如需帮助，请查阅 **[Crowdin 官方文档](https://support.crowdin.com/)** 或联系 **[Crowdin 支持团队](mailto:support@crowdin.com)**。
...
[`crowdin.yml` 配置文件](https://support.crowdin.com/configuration-file/)能与 Docusaurus 良好配合，确保**将本地化翻译文件下载至预期位置**（存放于 `i18n/[locale]/..` 目录）。

阅读**[官方文档](https://support.crowdin.com/)**了解高级功能与不同翻译工作流。

## Crowdin 教程 {#crowdin-tutorial}

本教程将演示如何使用 Crowdin 将新建的英文版 Docusaurus 网站翻译为法文版，假设您已完成[国际化教程](./i18n-tutorial.mdx)的配置。
...
- `preserve_hierarchy`：在Crowdin界面上保留文档的文件夹层级结构而非扁平化显示
...
可临时在`crowdin.yml`中硬编码个人令牌：`api_token: 'MY-TOKEN'`。
...
使用简单字符串属性的简单JSX片段（如`<Username name="Sebastien"/>`）可以正常工作；而使用对象/数组属性的复杂JSX片段（如`<User person={{name: "Sebastien"}}/>`）更可能因语法不符合HTML规范而失败。
...
- Crowdin匹配现有Markdown翻译的算法并非100%可靠，同步后需人工校验
...
Crowdin会将Markdown字符串替换为`crowdin:id12345`等技术ID，但替换过程过于激进，会影响front matter、admonitions和JSX等本应隐藏的内容。
