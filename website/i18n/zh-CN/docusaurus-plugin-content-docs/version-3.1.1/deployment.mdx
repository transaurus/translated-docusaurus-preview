...
Docusaurus 站点是静态渲染的，通常无需 JavaScript 即可工作！
...
- Jamstack 提供商（例如 [Netlify](#deploying-to-netlify) 和 [Vercel](#deploying-to-vercel)）。我们将以它们为参考，但同样的推理可以适用于其他提供商。
...
与静态托管服务/CDN相比，此方案并非最优选择。

:::

:::warning

后续章节将介绍几种常见托管服务商及其高效部署 Docusaurus 站点的配置方法。Docusaurus 与这些服务无商业关联，本文档仅作信息参考。部分配置说明由第三方提供，可能未及时同步服务商API变更。若发现内容过时，欢迎提交PR修正。
...
若未预先配置这些构建选项，站点创建后仍可前往"站点设置"→"构建与部署"进行设置。

完成上述配置后，站点将自动部署，并在代码合并至部署分支（默认为 `main`）时触发重新部署。

:::warning

部分 Docusaurus 项目将 `docs` 目录置于 `website` 外（常见于 Docusaurus v1 项目结构）：
...
若选择将 `website` 目录作为 Netlify 的基础目录，则更新 `docs` 目录时 Netlify 不会触发构建，此时需配置 [自定义忽略命令](https://docs.netlify.com/configure-builds/common-configurations/ignore-builds/)：
...
建议关闭 Netlify 设置中的 `后期处理 > 资源优化 > 美化URL` 功能，以避免小写URL转换、不必要的重定向及404错误。

**特别注意**：全局复选框 `禁用资源优化` 存在缺陷，实际不会禁用 `美化URL` 设置。请务必单独取消勾选该选项。

如需保留 Netlify 的 `美化URL` 设置，请相应调整 Docusaurus 的 `trailingSlash` 配置。
...
将 Docusaurus 项目部署至 [Vercel](https://vercel.com/) 可显著提升性能表现并简化运维流程。
...
项目导入后，所有推送到分支的操作都会生成 [预览部署](https://vercel.com/docs/platform/deployments#preview)，而对 [生产分支](https://vercel.com/docs/git-integrations#production-branch)（通常为"main"或"master"）的更改将触发 [生产环境部署](https://vercel.com/docs/platform/deployments#production)。
...
每个 GitHub 仓库都关联 GitHub Pages 服务。若部署仓库名为 `my-org/my-project`（`my-org`为组织名或用户名），部署后的站点将出现在 `https://my-org.github.io/my-project/`。若部署仓库名为 `my-org/my-org.github.io`（组织级 GitHub Pages 仓库），站点将位于 `https://my-org.github.io/`。

:::info

如需为 GitHub Pages 使用自定义域名，请在 `static` 目录创建 `CNAME` 文件。`static` 目录下的所有内容都会被复制到构建目录的根路径。使用自定义域名时，可将配置从 `baseUrl: '/projectName/'` 改回 `baseUrl: '/'`，并将 `url` 设为您的自定义域名。

更多细节请参阅 GitHub Pages 文档 [用户、组织和项目页面](https://help.github.com/en/articles/user-organization-and-project-pages)。

:::

GitHub Pages 会从默认分支（通常为 `master`/`main`）或 `gh-pages` 分支的根目录/`docs` 文件夹中获取部署就绪的文件（`docusaurus build` 的输出）。可通过仓库的 `Settings > Pages` 进行配置，该分支称为"部署分支"。

我们提供 `docusaurus deploy` 命令，可一键完成从源分支到部署分支的克隆、构建和提交操作。
...
这些字段也支持优先级更高的环境变量：`ORGANIZATION_NAME`、`PROJECT_NAME` 和 `DEPLOYMENT_BRANCH`。

:::warning

GitHub Pages 默认会在 Docusaurus URL 末尾添加斜杠。建议明确设置 `trailingSlash` 配置（`true` 或 `false`，不要设为 `undefined`）。
...
GitHub 企业版安装方式与 github.com 相同，只需将组织的 GitHub Enterprise 主机地址设为环境变量：
...
[GitHub Actions](https://help.github.com/en/actions) 允许您直接在代码仓库中自动化、定制和执行软件开发工作流程。

以下工作流示例假设您的网站源代码位于仓库的 `main` 分支（_源分支_为 `main`），且您的[发布源](https://help.github.com/en/github/working-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site)配置为 `gh-pages` 分支（_部署分支_为 `gh-pages`）。

我们的目标是：

1. 当向 `main` 分支提交新的拉取请求时，执行一个确保站点能成功构建但不实际部署的 Action。该任务称为 `test-deploy`。
2. 当拉取请求被合并到 `main` 分支或有人直接向 `main` 分支推送代码时，将构建并部署到 `gh-pages` 分支。之后，新的构建输出将在 GitHub Pages 站点上生效。该任务称为 `deploy`。

以下是使用 GitHub Actions 部署文档的两种方法。根据部署分支（`gh-pages`）的位置，选择下方对应的标签页：

- 源仓库和部署仓库为**同一**仓库。
- 部署仓库是一个**远程**仓库，与源仓库不同。
...
持续集成（CI）服务通常用于在源代码提交时执行例行任务。这些任务可以是运行单元测试和集成测试、自动化构建、将包发布到 npm 以及将更改部署到您的网站等操作的任意组合。要自动化网站的部署，只需在网站更新时调用 `yarn deploy` 脚本即可。以下部分介绍如何使用流行的持续集成服务提供商 [Travis CI](https://travis-ci.com/) 来实现这一点。

1. 访问 https://github.com/settings/tokens 生成一个新的[个人访问令牌](https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/)。创建令牌时，授予其 `repo` 权限以确保其具备所需权限。
2. 使用您的 GitHub 账户，将 [Travis CI 应用](https://github.com/marketplace/travis-ci) 添加到要激活的仓库。
3. 打开您的 Travis CI 仪表盘（URL 类似 `https://travis-ci.com/USERNAME/REPO`），导航至仓库的 `More options > Setting > Environment Variables` 部分。
4. 创建一个名为 `GH_TOKEN` 的环境变量，其值为新生成的令牌，然后创建 `GH_EMAIL`（您的电子邮件地址）和 `GH_NAME`（您的 GitHub 用户名）。
...
现在，每当有新的提交推送到 `main` 分支时，Travis CI 将运行您的测试套件，如果一切通过，您的网站将通过 `yarn deploy` 脚本自动部署。
...
2. 登录Buddy账户并创建新项目。
3. 选择GitHub作为git托管服务提供商，并选中包含网站代码的仓库。
4. 通过左侧导航栏切换到`Pipelines`视图。
5. 创建新流水线：定义名称，将触发模式设为`On push`，选择触发流水线执行的分支。
6. 添加`Node.js`操作。
7. 在操作终端中输入以下命令：
...
完成该简单流水线创建后，每次向选定分支推送新提交时，系统都会通过`yarn deploy`将网站部署至GitHub Pages。查阅[本指南](https://buddy.works/guides/react-docusaurus)了解如何为Docusaurus设置CI/CD流水线。
...
4. 在项目页面（形如`https://dev.azure.com/ORG_NAME/REPO_NAME/_build`）中，使用以下文本创建新流水线。点击编辑添加名为`GH_TOKEN`的环境变量（值为新生成令牌），以及`GH_EMAIL`（您的邮箱）和`GH_NAME`（GitHub用户名）。务必将其标记为机密。也可在仓库根目录创建`azure-pipelines.yml`文件。
...
1. 创建用作项目[部署密钥](https://docs.github.com/en/free-pro-team@latest/developers/overview/managing-deploy-keys#deploy-keys)的新SSH密钥。
2. 为私钥和公钥命名以区分其他[SSH密钥](https://docs.github.com/en/free-pro-team@latest/github/authenticating-to-github/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent)。
3. 访问`https://github.com/USERNAME/REPO/settings/keys`，粘贴刚生成的公钥添加新部署密钥。
4. 打开Drone.io控制台（形如`https://cloud.drone.io/USERNAME/REPO`）并登录。
5. 点击仓库激活，添加名为`git_deploy_private_key`的机密（值为刚生成的私钥）。
...
[Flightcontrol](https://www.flightcontrol.dev/?ref=docusaurus)服务可直接从Git仓库将Web应用自动构建并部署至AWS Fargate。该服务提供完整的基础设施检查与修改权限，突破传统PaaS的限制。

按照[Flightcontrol的Docusaurus分步指南](https://www.flightcontrol.dev/docs/reference/examples/docusaurus/?ref=docusaurus)开始使用。

## 部署到 Koyeb {#deploying-to-koyeb}

[Koyeb](https://www.koyeb.com) 是一个对开发者友好的无服务器平台，用于全球部署应用程序。该平台允许您无缝运行 Docker 容器、Web 应用和 API，支持基于 Git 的部署、原生自动扩展、全球边缘网络以及内置的服务网格和发现功能。查看 [Koyeb 的 Docusaurus 部署指南](https://www.koyeb.com/tutorials/deploy-docusaurus-on-koyeb) 开始使用。
...
4. 添加应用程序。
   - 点击 **创建应用程序**，命名并选择您的 GitHub 或 GitLab 仓库，其中包含您的 Docusaurus 应用。
   - 定义主分支名称和应用程序根路径。
   - 点击 **创建**。应用程序创建后：
   - 导航到应用程序的 **设置**
   - 选择 **端口**
...
3. 配置构建设置。
   ...
   - **应用目录**指向构建后包含项目文件的目录。如果在步骤2中选择了包含网站内容（或 `my_website` 目录）的代码库，可留空此项。
...
确认待发布站点位于 `build` 目录。系统会自动分配随机子域名 `*.surge.sh`（可编辑）。
...
未来部署变更时只需运行 `surge` 命令。

## 部署到 QuantCDN {#deploying-to-quantcdn}

1. 安装 [Quant CLI](https://docs.quantcdn.io/docs/cli/get-started)
2. [注册](https://dashboard.quantcdn.io/register)创建 QuantCDN 账户
3. 运行 `quant init` 初始化项目并填写凭证：
   ```bash
   quant init
   ```
4. 部署站点：
...
[Layer0](https://www.layer0.co) 是一个一体化平台，用于开发、部署、预览、实验、监控和运行无头前端。它专注于大型动态网站，通过EdgeJS（基于JavaScript的内容分发网络）、预测性预取和性能监控提供一流的性能。Layer0提供免费套餐，只需几分钟即可开始，按照[Layer0的Docusaurus部署指南](https://docs.layer0.co/guides/docusaurus)操作即可。
...
[Azure Static Web Apps](https://docs.microsoft.com/en-us/azure/static-web-apps/overview) 是一项服务，可直接从代码仓库自动构建和部署全栈Web应用到Azure，简化了CI/CD的开发者体验。Static Web Apps将Web应用的静态资源与动态（API）端点分离。静态资源通过全球分布的内容服务器提供，使客户端能够通过附近的服务器更快地检索文件。动态API通过基于事件驱动的无服务器架构进行扩展，这种方法更具成本效益且按需扩展。按照[这篇分步指南](https://dev.to/azure/11-share-content-with-docusaurus-azure-static-web-apps-30hc)，几分钟内即可开始。

## 部署到Kinsta {#deploying-to-kinsta}

[Kinsta静态站点托管](https://kinsta.com/static-site-hosting) 允许您免费部署多达100个静态站点，支持自定义域名和SSL，每月100 GB带宽，以及260多个Cloudflare CDN节点。
