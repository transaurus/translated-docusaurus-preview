...
默认的[全局站点标签(gtag.js)](https://developers.google.com/tag-platform/gtagjs)插件。这是一个JavaScript标签框架和API，允许您将事件数据发送到Google Analytics（分析）、Google Ads和Google Marketing Platform。本节介绍如何配置Docusaurus站点以启用Google Analytics的全局站点标签。

:::tip

您可以使用[Google的Tag Assistant](https://tagassistant.google.com/)工具来检查gtag是否正确设置！
