...
默认情况下，该插件在开发环境中处于**非激活状态**，以便您始终可以查看全尺寸图像。如需调试理想图像行为，可将 [`disableInDev`](#disableInDev) 选项设为 `false`。
...
该插件注册的 [Webpack 加载器](https://webpack.js.org/loaders/)会改变导入/请求图像的类型：
