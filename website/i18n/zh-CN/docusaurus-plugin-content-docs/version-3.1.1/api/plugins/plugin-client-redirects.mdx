...
该插件会向静态站点写入额外的 HTML 页面，通过 JavaScript 将用户重定向至现有的 Docusaurus 页面。

:::warning[仅生产环境生效]

本插件在开发环境中始终处于未激活状态，**仅在生产环境生效**，因其功能基于构建输出实现。
...
本插件会读取 [`siteConfig.onDuplicateRoutes`](../docusaurus.config.js.mdx#onDuplicateRoutes) 配置，当多个文件将输出至相同位置时，据此调整日志级别。
...
因此可以存在多个"from"指向同一个"to"：我们将创建多个HTML文件来重定向至同一目标。但一个"from"绝不能对应多个"to"：写入的HTML文件必须具有确定的重定向目标。
