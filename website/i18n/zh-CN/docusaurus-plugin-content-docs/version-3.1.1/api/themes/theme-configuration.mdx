...
Logo可放置在[静态文件夹](static-assets.mdx)中。Logo URL默认设为站点基础URL。虽然可以指定自定义URL，但如果是外部链接会在新标签页打开。此外可覆盖logo链接的target属性值，这在文档网站作为主站子目录托管时特别有用——此时可能不需要logo链接在主站新标签页打开。
...
React Router 应自动为链接应用活动链接样式，但在边缘情况下可使用 `activeBasePath`。当链接需要在多个不同路径下保持活动状态时（例如同一侧边栏下存在多个文档目录），可使用 `activeBaseRegex`。`activeBaseRegex` 是比 `activeBasePath` 更灵活的替代方案且优先级更高——Docusaurus 会将其解析为正则表达式来匹配当前 URL。
...
注意下拉菜单基项本身也是可点击链接，因此该类型项目可接收[普通导航栏链接](#navbar-link)的所有属性。
...
若需链接至特定文档，此特殊导航栏项目类型会通过提供的 `docId` 渲染文档链接。只要浏览同一侧边栏下的文档，该链接就会获得 `navbar__link--active` 类。
...
若使用带版本控制的文档，此特殊导航栏项目类型会链接至当前浏览的文档版本（取决于 URL），并回退至最新版本。
...
如果启用了[i18n功能](../../i18n/i18n-introduction.mdx)，此特殊导航栏项目类型会渲染一个下拉菜单，显示站点所有可用语言版本。
...
Docusaurus使用[Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)实现代码块高亮，所有配置都在`prism`对象中。
...
默认使用[Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts)作为语法高亮主题。您可以从[可用主题列表](https://github.com/FormidableLabs/prism-react-renderer/tree/master/packages/prism-react-renderer/src/themes)选择自定义主题，还可以为暗色模式站点指定不同的语法高亮主题。
...
可以为代码块设置默认语言（当开头的三个反引号```后未指定语言时生效）。注意必须传入有效的[语言名称](https://prismjs.com/#supported-languages)。
...
通过`themeConfig.footer`可添加页脚logo和版权信息。logo文件可放置在[静态文件夹](static-assets.mdx)中，其URL处理方式与导航栏logo相同。
